...
需要等待已启动 task 的输出或终态时使用 `tmux_read`；`tmux_run` 的 `resume: true` 会由 MCP 负责固定 60 分钟挂起与 Workspace 恢复。
...
MCP 调用 `tmux_run` 时传入 `resume: true`，会先固定挂起模型最多 60 分钟。任务在 60 分钟内结束则直接返回；仍运行时返回 `detached: true` 并建立 durable Wait。`timeout` 从任务启动开始计算，任务结束或绝对截止时间先到达时都会触发 Workspace resume；失败退出码也会触发恢复。
