...
- `[mcp].contextMode`：选择 MCP Context selector。`explicit`（默认）要求模型显式传 `ctxId`；`openai-session` 使用 ChatGPT tool call `_meta["openai/session"]` 解析到内部 Context，并从模型可见 schema/result 中移除 `ctxId`。两种模式内部都继续使用 `ctxId` 作为 runtime key；
