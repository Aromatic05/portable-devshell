...
- `openai-session`：仅用于会在每次 model-facing tool call 的 `_meta["openai/session"]` 中提供 ChatGPT session 标识的 Host。`environ_info` 不向模型返回 `ctxId`，model-facing 工具的输入 schema 也不包含 `ctxId`；服务端使用 `openai/session` 选择内部 Context。Workspace App 自己的 app-only 协议仍可携带内部 `ctxId`。`openai/session` 只用于模型调用的 Context 选择，不用于授权。
...
`environ_info` 是正常调用链上唯一的 Context / Workspace bootstrap：它在准备 workspace 环境的同时签发隐藏的 Workspace App capability，并通过同一个 tool result 呈现 Live Workspace。模型不再需要先调用 `workspace_open`。`workspace_open` 仅保留为显式重新呈现/恢复入口，例如用户关闭了 App、iframe remount 后需要重新挂载，或 `workspace_ask` / `workspace_goal start` 明确报告 App 已失活。App 视图附着在产生它的 tool result / 对话消息上，并不是跨后续消息常驻的独立 ChatGPT 面板；Context、durable Wait、Goal、Todo 等服务端 Workspace 状态继续存在。MCP 边界先用当前 selector 得到内部 Context，再返回该 Context 的 authoritative snapshot；model-facing `openai-session` 调用由 Host metadata 选择 Context，而 Workspace App 的 app-only helper 继续显式携带内部 `ctxId` 与隐藏 app token，以便 remount、Live route 和对象归属校验不依赖模型参数。可见 UI 固定按“需要用户处理的事件 → 当前 Goal → 后台等待”排列。`currentEvent` 只允许 Question 与 Approval，waiting/detached 的 tmux Wait 永远属于后台状态，不会抢占人工操作。相同优先级的人工事件按最早待处理者优先；detached Question 低于仍有原始 held call 的 waiting Question。
...
iframe remount 或 MCP/Control 重启后，App 从 tool result / widget state 恢复内部 `ctxId`，并从 metadata 获得新的 app token；旧 token 不需要持久化。`ctxId` 在这里属于 App-only 状态，不重新变成 `openai-session` 模式下的模型参数。ChatGPT remount 时只有存在 `toolResponseMetadata.mcp_tool_result` / `call_tool_result` envelope 时才接受 `window.openai.toolOutput` 覆盖其中可能缓存的旧 structured output，不能把一个孤立的全局 `toolOutput` 当成 Workspace 身份来源。App 在 connect 后短暂等待真实 initial tool result，再回退到 widget state。`workspace_answer`、`workspace_interrupt`、`workspace_resume`、`workspace_stop` 和 `workspace_approval` 属于人工写操作，必须携带当前 `ctxId` 与隐藏 app token；服务端最终仍以内部 `ctxId` 校验目标对象归属。当前 ToolDefinition 名称统一遵循单一 `namespace_operation` 分隔符；v0.6.15 已挂载 iframe 使用过的多段 app-only 名只在隐藏 wire compatibility decoder 中接受，用于跨 MCP/Control 热升级，不进入 model-facing catalog。
