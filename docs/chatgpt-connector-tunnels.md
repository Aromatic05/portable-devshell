...
此项目的 OIDC provider 通过 HTTP 上游运行时，发现文档中的 endpoint URL 可能被生成为 `http://`。为保证 ChatGPT 始终发现 HTTPS endpoint，在同一 `server` 块中为两份发现文档加入以下精确 location；它们会把本域名的内部 HTTP URL 改写成 HTTPS：
...
location = /.well-known/openid-configuration {
    ...
    sub_filter 'http://dev.example.com' 'https://dev.example.com';
    sub_filter_once off;
}
...
此方式在运行 `portable-devshell` 的同一台机器运行 `cloudflared`，不需要 VPS 或开放入站端口。域名必须托管在 Cloudflare。仍然需要一个只监听本机回环地址的 Nginx，用于保持 OIDC discovery 中的 URL 为 HTTPS。
...
让本机 Nginx 监听 `127.0.0.1:17891`，并反代到 control。Nginx 不需要证书，因为 TLS 在 Cloudflare 终止：
...
创建 `~/.cloudflared/config.yml`，让 Tunnel 指向该本机 Nginx：
...
- discovery 中出现 `http://`：检查 Nginx 的两个 `sub_filter` location；Cloudflare Tunnel 必须指向该本机 Nginx，而不是直接指向 control 的 HTTP 端口。
