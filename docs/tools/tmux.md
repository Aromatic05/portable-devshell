...
`timeout` 是 `block` 从 task 启动开始计算的总等待截止时间，不停止 task。MCP 在 task 启动后立即建立 Workspace Wait，因此用户可以随时 `Stop waiting`。当前所有 Context 模式使用同一个 **180 秒**同步窗口；超过同步窗口但尚未到 `timeout` 时转为 detached，由 Workspace 接管。

MCP HTTP host 使用 request-scoped SSE，并每 15 秒发送 keepalive，因此这 180 秒是真正保持同一个 `tools/call` 阻塞，而不是在约两分钟的 HTTP idle timeout 之前被迫提前 handoff。
...
相同 `ctxId + operationId` 的副作用请求使用 replay protection，不会因为 transport 重试同一个 worker operation 而重复发送按键。`operationId` 由 Core 为一次 worker operation 分配，并随同一 RPC envelope 的重放保持不变；它不是 MCP request ID。
