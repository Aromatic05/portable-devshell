...
export function parseAgentWorkerTarget(value: string): AgentWorkerTarget {
    if (value.length === 0 || value.trim() !== value) {
        throw new TypeError(
            "Agent target must not be empty or surrounded by whitespace.",
        );
    }
    ...
    if (delimiter <= 0) {
        throw new TypeError(
            "Agent target must use <worker-instance>:<workspace>.",
        );
    }
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/u.test(instance)) {
        throw new TypeError(
            "Agent target has an invalid worker instance name.",
        );
    }
