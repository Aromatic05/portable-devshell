...
export function parseAgentProviderManifest(
    value: unknown,
): AgentProviderManifest {
    if (!isRecord(value))
        throw new TypeError("Agent provider manifest must be an object.");
    ...
    if (
        ...
        throw new TypeError(
            "Agent provider manifest has unknown or missing fields.",
        );
    }
    if (value.schemaVersion !== 1)
        throw new TypeError(
            "Unsupported Agent provider manifest schema version.",
        );
    if (!Number.isInteger(value.apiVersion) || Number(value.apiVersion) < 1) {
        throw new TypeError(
            "Agent provider apiVersion must be a positive integer.",
        );
    }
...
function readString(value: unknown, name: string): string {
    if (typeof value !== "string" || value.trim().length === 0) {
        throw new TypeError(
            `Agent provider ${name} must be a non-empty string.`,
        );
    }
    return value.trim();
}
