...
const AGENT_MODEL_TOOL_PROJECTIONS = new Map<
    string,
    AgentModelToolProjectionSpec
>([
    [
        "bash_run",
        {
            description:
                "Run a short, bounded, non-interactive shell command. Prefer this when the work fits the command timeout and does not need a PTY; use tmux_run for long-running or interactive work. If stdoutRecovery or stderrRecovery is returned, use file_read on that path for omitted output.",
    ...
    [
        ...
        {
            description:
                "Discover files and directories by exact path or glob pattern. Use this for path discovery rather than content search, and continue paged results with cursor alone.",
    ...
    [
        ...
        {
            description:
                "Search text in files, directories, or globs. Use this to locate relevant lines and establish edit coverage before file_edit; continue a truncated file with startLine and paged traversal with cursor alone.",
    ...
    [
        ...
        {
            description:
                "Send terminal input to a managed task or persistent pane. Address managed executions by task id and persistent interactions by pane; use tmux_read for task transcript output and tmux_inspect for pane screen state. Caret notation sends control keys.",
    ...
    [
        ...
        {
            description:
                "Observe pane terminal state without consuming a managed task transcript. Use this for main or persistent panes and curses/TUI screen state; use tmux_read when the durable managed-task transcript is the source of truth.",
    ...
    [
        ...
        {
            description:
                "Manage tmux resources: command=list discovers panes and active tasks, command=create opens a persistent interactive pane, and command=close terminates a task or closes a persistent pane. Use force only when intentionally terminating a running or busy resource.",
    ...
    [
        ...
        {
            description:
                "Wait for or consume a managed task transcript using its task id. Use this for durable task output; use tmux_inspect for non-consuming pane screen state. Positive line consumes the oldest unread lines. Negative line also consumes all unread transcript data, discards the earlier portion, and returns only the requested tail after waiting.",
            projectResult: renderTmuxTaskModelToolResult,
        },
    ],
    [
        ...
        {
            description:
                "Start long-running or PTY/interactive work as a managed task. Prefer wait=block when completion is on the current critical path and there is no useful parallel work; use wait=nonblock when intentionally continuing other work. Continue task output with tmux_read and interact with tmux_input.",
            projectResult: renderTmuxTaskModelToolResult,
        },
    ],
]);
...
>([
    [
        ...
        {
            timeoutMs:
                "Hard command timeout in milliseconds, up to 100000. Use tmux_run instead when work may exceed this bound.",
        },
    ],
    [
        "tmux_run",
        {
            timeout:
                "Maximum block-wait deadline from task start; reaching it leaves the task running. Set it to cover the expected critical-path wait.",
    ...
    return tools
        ...
        .map((tool) => {
            ...
            if (projection === undefined)
                throw new Error(
                    `Missing Agent model tool projection: ${tool.name}`,
                );
            return {
                description: projection.description,
                inputSchema: projectAgentModelInputSchema(
                    tool.name,
                    tool.inputSchema,
                ),
                name: tool.name,
...
export function prepareAgentModelToolInput(
    toolName: string,
    params: unknown,
): ExtensionJsonValue {
...
export function projectAgentModelToolResult(
    toolName: string,
...
function projectAgentModelInputSchema(
    ...
    const propertyDescriptions =
        AGENT_MODEL_INPUT_PROPERTY_DESCRIPTIONS.get(toolName);
    if (propertyDescriptions !== undefined) {
        for (const [property, description] of Object.entries(
            ...
            if (isRecord(propertySchema))
                properties[property] = { ...propertySchema, description };
        }
    }
...
function renderAgentModelToolResult(
    toolName: string,
    value: ExtensionJsonValue,
): string {
    if (!isRecord(value)) return renderToolResult(value);
    return (
        AGENT_MODEL_TOOL_PROJECTIONS.get(toolName)?.projectResult(value) ??
        renderToolResult(value)
    );
}

function renderFileEditModelToolResult(
    value: Record<string, ExtensionJsonValue>,
): string {
        ...
        .map((operation) => {
            ...
            const removed =
                typeof operation.removedLines === "number"
                    ? `-${operation.removedLines}`
                    : undefined;
            const movedFrom =
                typeof operation.movedFrom === "string"
                    ? `from=${operation.movedFrom}`
                    : undefined;
            const summary = [action, path, status, added, removed, movedFrom]
                .filter(Boolean)
                .join(" ");
            const errorRecord = isRecord(operation.error)
                ? operation.error
                : undefined;
            const error =
...
function renderBashModelToolResult(
    value: Record<string, ExtensionJsonValue>,
): string {
    const stdout = typeof value.stdout === "string" ? value.stdout : "";
    const stderr = typeof value.stderr === "string" ? value.stderr : "";
    const stdoutPath =
        typeof value.stdoutPath === "string" ? value.stdoutPath : undefined;
    const stderrPath =
        typeof value.stderrPath === "string" ? value.stderrPath : undefined;
    const streams = [stdout.length > 0, stderr.length > 0].filter(
        Boolean,
    ).length;
    const streamBudget = Math.floor(
        MAX_SEMANTIC_CONTENT_CHARACTERS / Math.max(streams, 1),
    );
    const lines = [
        compactFields([
    ...
    if (stdout.length > 0) {
        ...
        lines.push(
            `stdout${streamLabel(value.stdoutBytes, value.stdoutTruncated)}:\n${preview.text}`,
        );
        if (preview.clipped && stdoutPath === undefined)
            lines.push(
                "stdoutProjectionClipped=true; rerun with narrower output if omitted text is required",
            );
    }
    if (stderr.length > 0) {
        const preview = previewText(stderr, streamBudget);
        lines.push(
            `stderr${streamLabel(value.stderrBytes, value.stderrTruncated)}:\n${preview.text}`,
        );
        if (preview.clipped && stderrPath === undefined)
            lines.push(
                "stderrProjectionClipped=true; rerun with narrower output if omitted text is required",
            );
    }
...
function renderFileReadModelToolResult(
    ...
    const contentBudget = Math.max(
        80,
        Math.floor(MAX_SEMANTIC_CONTENT_CHARACTERS / files.length),
    );
    return files
        .map((file) => {
            const path =
                typeof file.path === "string" ? file.path : "<unknown>";
            ...
            return [
                header,
                preview.text,
                file.truncated === true ? "workerTruncated=true" : undefined,
                continuation,
            ]
                .filter(
                    (line): line is string =>
...
function renderFileGlobModelToolResult(
    ...
    const rendered = entries
        .map((entry) => {
            const path =
                typeof entry.path === "string" ? entry.path : "<unknown>";
    ...
    if (preview.clipped)
        lines.push(
            "projectionClipped=true; rerun file_glob with narrower patterns if omitted paths are required",
        );
...
function renderTmuxTaskModelToolResult(
    ...
    const taskId =
        task !== undefined && typeof task.id === "string" ? task.id : undefined;
    const paneId =
        pane !== undefined && typeof pane.id === "string" ? pane.id : undefined;
    const lines = [
        compactFields([
            field("task", taskId),
    ...
    if (output.length > 0) {
        ...
        if (preview.clipped) {
            if (taskId !== undefined)
                lines.push(
                    `projectionClipped=true; continue with tmux_read task=${taskId}`,
                );
            else if (paneId !== undefined)
                lines.push(
                    `projectionClipped=true; inspect pane=${paneId} for omitted terminal content`,
                );
        }
    }
...
function renderTmuxInspectModelToolResult(
    ...
    if (panes.length === 0)
        return [
            ...renderTmuxWarnings(value.warnings),
            "No panes returned.",
        ].join("\n");
    const lineBudget = Math.max(
        80,
        Math.floor(MAX_SEMANTIC_CONTENT_CHARACTERS / panes.length),
    );
...
function renderTmuxManageModelToolResult(
    value: Record<string, ExtensionJsonValue>,
): string {
    ...
    if (
        typeof value.closedTaskId === "string" ||
...
function renderTmuxCreateModelToolResult(
    value: Record<string, ExtensionJsonValue>,
): string {
    const pane = isRecord(value.pane) ? value.pane : undefined;
    if (pane === undefined) return renderToolResult(value);
    return [
        compactFields([field("pane", pane.id), field("name", pane.name)]),
        ...renderTmuxWarnings(value.warnings),
    ]
...
function renderTmuxCloseModelToolResult(
    value: Record<string, ExtensionJsonValue>,
): string {
    const closed =
        typeof value.closedTaskId === "string"
...
function streamLabel(
    bytes: ExtensionJsonValue | undefined,
    truncated: ExtensionJsonValue | undefined,
): string {
    const attributes = [
        typeof bytes === "number" ? `${bytes} bytes` : undefined,
        truncated === true ? "worker-truncated" : undefined,
    ].filter(Boolean);
...
function asJsonValue(value: unknown): ExtensionJsonValue {
    if (!isJsonValue(value))
        throw new TypeError("Agent tool arguments are not JSON serializable.");
    return value;
}
