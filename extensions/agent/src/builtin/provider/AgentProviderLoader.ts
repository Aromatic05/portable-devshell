...
export class AgentProviderLoader {
    ...
    constructor(
        ...
        importer: (url: string) => Promise<unknown> = async (url) =>
            (await import(url)) as unknown,
        registry = new AgentProviderRegistryStore(
    ...
    async loadSelected(): Promise<AgentProvider[]> {
        ...
        for (const [id, entry] of Object.entries(registry.providers).sort(
            ...
            for (const generation of candidates) {
                try {
                    ...
                    this.#context.logger.warn(
                        `Agent provider ${id} generation ${generation} failed to load.`,
                        {
                            error:
            ...
            if (loaded !== undefined) providers.push(loaded.provider);
                ...
                this.#context.logger.error(
                    `Agent provider ${id} has no loadable generation.`,
                    {
                        error:
    ...
    async inspectGeneration(
        id: string,
        generation: string,
    ): Promise<AgentProviderManifest> {
        assertProviderSegment(id, "id");
        const manifest = await this.inspectBundle(generation);
        if (manifest.id !== id) {
            throw new Error(
                `Agent provider generation ${generation} declares id ${manifest.id}, expected ${id}.`,
            );
        }
        return manifest;
    }
    ...
    async inspectBundle(generation: string): Promise<AgentProviderManifest> {
        ...
        await assertPlainDirectory(
            generationDirectory,
            `Agent provider generation ${generation}`,
        );
        const manifestPath = join(
            generationDirectory,
            "devshell-agent-provider.json",
        );
        await assertPlainFile(
            manifestPath,
            `Agent provider manifest ${generation}`,
        );
        const manifest = parseAgentProviderManifest(
    ...
    async loadGeneration(
        ...
        await assertPlainFile(
            entryPath,
            `Agent provider entry ${id}/${generation}`,
        );
        const module = readProviderModule(
        ...
        if (provider.id !== manifest.id) {
            throw new Error(
                `Agent provider module returned id ${provider.id}, expected ${manifest.id}.`,
            );
        }
    ...
    async #generationDirectory(generation: string): Promise<string> {
        ...
        if (bundle === undefined) {
            throw new Error(
                `Agent provider generation ${generation} is not installed.`,
            );
        }
...
function readProviderModule(value: unknown, id: string): AgentProviderModule {
    if (!isRecord(value) || typeof value.createAgentProvider !== "function") {
        throw new TypeError(
            `Agent provider ${id} entry must export createAgentProvider().`,
        );
    }
...
function resolveContainedPath(root: string, candidate: string): string {
    if (isAbsolute(candidate))
        throw new TypeError(
            "Agent provider entry must be relative to its generation.",
        );
    ...
    if (child === "" || (!child.startsWith("..") && !isAbsolute(child)))
        return resolved;
    throw new TypeError(
        "Agent provider entry must stay inside its generation.",
    );
}
...
async function assertPlainFile(path: string, label: string): Promise<void> {
    ...
    if (metadata.isSymbolicLink() || !metadata.isFile())
        throw new TypeError(`${label} must be a plain file.`);
}
