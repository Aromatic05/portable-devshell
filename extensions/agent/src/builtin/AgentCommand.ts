    ...
    switch (argv[0]) {
        ...
        case "show": {
            ...
            const record = runtime.get(
                required(argv[1], "agent id is required"),
            );
        ...
        case "stop": {
            ...
            return json(
                recordToJson(
                    await runtime.stop({
                        agentId: required(argv[1], "agent id is required"),
                    }),
                ),
            );
        }
...
async function providerCommand(
    ...
    switch (argv[0]) {
        ...
        case "default": {
            if (argv.length === 1) {
                return json({
                    provider: (await providers.getDefault()) ?? null,
                });
            }
            requireLocalOwner(context);
            expectLength(argv, 2, "agent provider default <id>");
            return json({
                provider: await providers.setDefault(
        ...
        case "disable": {
            ...
            const record =
                argv[0] === "enable"
                    ? await providers.enable(id)
                    : await providers.disable(id);
            return json(providerRecordToJson(record));
        }
        case "remove":
            requireLocalOwner(context);
...
async function start(
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (value === "--provider") {
            ...
            provider = required(
                argv[++index],
                "agent --provider requires <id>",
            );
            continue;
        }
        if (value.startsWith("-"))
            throw usageError(`Unknown agent option: ${value}`);
        if (target !== undefined)
            throw usageError(
                "agent start accepts one <instance:/workspace> target",
            );
        target = value;
    }
...
function requireLocalOwner(context: CliNativeCommandInvocationContext): void {
    ...
    throw new Error(
        "Agent provider mutations are restricted to the local owner CLI.",
    );
}
