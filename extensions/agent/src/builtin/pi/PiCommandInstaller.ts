...
export async function ensureBundledPiCommand(
    ...
    const launcher = join(context.paths.codeDirectory, "dist", "builtin", "pi", "PiLauncher.js");
    const launcherMetadata = await lstat(launcher).catch((error: unknown) => {
        if (isMissing(error)) return undefined;
        throw error;
    });
    const binDirectory = environment.PORTABLE_DEVSHELL_BIN_DIR ?? resolve(homeDirectory, ".local", "bin");
    const command = join(binDirectory, platform === "win32" ? "pi.cmd" : "pi");
    if (launcherMetadata === undefined || launcherMetadata.isSymbolicLink() || !launcherMetadata.isFile()) {
        return { command, installed: false, reason: "launcher-missing" };
    }

    const existing = await inspectExistingCommand(command);
    if (existing === "foreign") return { command, installed: false, reason: "collision" };
...
async function replaceFile(path: string, source: string, mode: number): Promise<void> {
...
async function inspectExistingCommand(path: string): Promise<"absent" | "foreign" | "owned"> {
    ...
    if (source.includes("portable-devshell-pi-launcher.mjs") && source.includes("portable-devshell")) return "owned";
    return "foreign";
}

function isLegacyCorePiLauncher(target: string): boolean {
    return basename(target) === "portable-devshell-pi-launcher.mjs" && target.includes("portable-devshell");
}
