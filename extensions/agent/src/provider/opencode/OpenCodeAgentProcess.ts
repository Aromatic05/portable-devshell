...
export interface OpenCodeAgentRuntimeFactory {
    start(
        options: OpenCodeAgentProcessStartOptions,
...
class OpenCodeManagedProcess {
    ...
    constructor(
        ...
        this.#ownerHeartbeat = setInterval(() => {
            if (this.#stopped) return;
            void this.#send({ type: "owner.heartbeat" }).catch((error) =>
                this.#fail(toError(error)),
            );
    ...
    async initialize(): Promise<void> {
        ...
        await this.#send({
            ...
            modelTools: this.#options.tools.modelTools.map((tool) => ({
                ...tool,
            })),
    ...
    async command(
        ...
        if (this.#stopped && command !== "stop")
            throw new Error("OpenCode provider child is already stopped.");
        await this.#request({
            command,
            id: randomUUID(),
    ...
    terminate(): void {
        ...
        this.#controller.abort(
            new Error("OpenCode provider child was terminated."),
        );
        this.#rejectPending(
            new Error("OpenCode provider child was terminated."),
        );
    ...
    async #request(
        message: Extract<OpenCodeParentMessage, { type: "command" }>,
    ): Promise<void> {
    ...
    async #onMessage(value: unknown): Promise<void> {
        ...
        if (message?.type === "ready") {
            if (message.ok) this.#readyResolve?.();
                ...
                this.#readyReject?.(
                    new Error(
                        message.error ??
                            "OpenCode provider child initialization failed.",
                    ),
                );
        ...
        if (message?.type === "result") {
            ...
            if (message.ok) pending.resolve();
                ...
                pending.reject(
                    new Error(
                        message.error ?? "OpenCode provider command failed.",
                    ),
                );
            return;
        }
    ...
    async #handleToolCall(
        message: Extract<OpenCodeChildMessage, { type: "tool.call" }>,
    ): Promise<void> {
        try {
            const input = prepareAgentModelToolInput(
                message.toolName,
                message.input,
            );
...
function assertToolTarget(
    ...
    if (
        ...
        throw new Error(
            "OpenCode Agent tool session target does not match the Agent target.",
        );
    }
}
