...
const ownerWatchdog = setInterval(
    () => {
        if (Date.now() - lastOwnerHeartbeat > ownerHeartbeatTimeoutMs)
            void shutdownAndExit(0);
    },
    ...
    if (message.type === "init") {
        if (runtime !== undefined) {
            send({
                id: message.id,
                ok: false,
                error: "OpenCode provider child is already initialized.",
                type: "ready",
            });
            return;
        }
...
class OpenCodeRuntime {
    ...
    readonly #toolCalls = new Map<
        string,
        { reject(error: Error): void; resolve(result: string): void }
    >();
    ...
    static async start(
        input: Extract<OpenCodeParentMessage, { type: "init" }>,
    ...
    prompt(message: string): void {
        if (this.#stopped)
            throw new Error("OpenCode Agent is already stopped.");
        if (this.#activeTurn !== undefined)
            throw new Error("OpenCode Agent already has an active turn.");
        const turn = this.#connection
            .prompt({
    ...
    async stop(): Promise<void> {
        ...
        for (const pending of this.#toolCalls.values())
            pending.reject(new Error("OpenCode Agent stopped."));
        this.#toolCalls.clear();
        await this.#mcp.close();
        this.#process.kill("SIGTERM");
        await waitForExit(this.#process, 2_000).catch(() =>
    ...
    acceptToolResult(
        message: Extract<OpenCodeParentMessage, { type: "tool.result" }>,
    ): void {
        ...
        if (message.ok) pending.resolve(message.result ?? "");
            ...
            pending.reject(
                new Error(message.error ?? "DevShell tool call failed."),
            );
    }
}
...
class McpToolBridge {
    ...
    static async start(
            ...
            mcp.registerTool(
                ...
                async (input) => {
                    if (
                        ...
                        bridge.current.#runtime === undefined
                    ) {
                        throw new Error(
                            "OpenCode DevShell tool bridge is not bound.",
                        );
                    }
                    const result = await bridge.current.#runtime.callTool(
                        tool.name,
                        input,
                    );
        ...
        if (address === null || typeof address === "string") {
            await mcp.close();
            throw new Error(
                "OpenCode MCP bridge did not obtain a TCP address.",
            );
        }
        bridge.current = new McpToolBridge(
            http,
            mcp,
    ...
    bind(runtime: OpenCodeRuntime): void {
        if (this.#runtime !== undefined)
            throw new Error("OpenCode MCP bridge is already bound.");
...
function requireRuntime(): OpenCodeRuntime {
    if (runtime === undefined)
        throw new Error("OpenCode provider child is not initialized.");
    return runtime;
}
...
async function waitForExit(
    ...
    await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
            ...
            reject(
                new Error(
                    "OpenCode ACP process did not exit after termination.",
                ),
            );
        }, timeoutMs);
        const cleanup = () => {
