...
export class OutputPreviewComponent implements Component {
    ...
    render(width: number): string[] {
        ...
        const visible = this.#expanded
            ? this.#lines
            : this.#lines.slice(-this.#tailLines);
        ...
        output.push(
            ...this.#footer.map((line) => truncateToWidth(line, width, "...")),
        );
        return output;
    }
}
...
export function clearComponent(component: Component | undefined): Component {
    const container =
        component instanceof Container ? component : new Container();
    container.clear();
    return container;
}
...
export function renderError(
    ...
    const fallback =
        lines.length === 0 ? renderStructured(result.details) : lines;
...
export function renderStructured(value: JsonValue | undefined): string[] {
    ...
    for (const [key, entry] of Object.entries(value)) {
        ...
        if (typeof entry !== "object")
            lines.push(`${key}: ${oneLine(String(entry), 240)}`);
        else {
            lines.push(`${key}:`);
            lines.push(
                ...renderStructured(entry as JsonValue).map(
                    (line) => `  ${line}`,
                ),
            );
        }
...
export function tailWithHint(lines: string[], limit: number): string[] {
    ...
    return [
        `... (${lines.length - limit} earlier lines, Ctrl+O to expand)`,
        ...lines.slice(-limit),
    ];
}
...
export function clipHead(lines: string[], limit: number): string[] {
    return lines.length <= limit
        ...
        : [
              ...lines.slice(0, limit),
              `... (${lines.length - limit} more lines, Ctrl+O to expand)`,
          ];
}
