...
export function formatShellCall(
    ...
    const suffix = [
        ...
        numberField(record, "timeoutMs") === undefined
            ? undefined
            : `timeout ${formatDuration(numberField(record, "timeoutMs")!)}`,
    ]
...
export function renderBashResult(
    ...
    const lines = [
        ...splitOutput(stringField(record, "stdout") ?? ""),
        ...splitOutput(stringField(record, "stderr") ?? ""),
    ];
...
function bashFooter(
    ...
    if (
        ...
        stringField(record, "termination") === "timeout"
    )
        warnings.push("timed out");
    if (warnings.length > 0)
        lines.push(style(theme, "warning", `[${warnings.join(" · ")}]`));
    ...
    const summary = [
        ...
        duration === undefined
            ? undefined
            : `${partial ? "Elapsed" : "Took"} ${formatDuration(duration)}`,
    ]
        .filter((value): value is string => value !== undefined)
        .join(" · ");
