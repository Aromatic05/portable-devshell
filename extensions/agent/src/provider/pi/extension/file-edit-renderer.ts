...
export function renderFileEditResultComponent(
    ...
    const operations = parseEditChangeSet(
        stringField(asRecord(context.args) ?? {}, "changes"),
    );
    ...
    for (const [index, operation] of operations.entries()) {
        ...
        const diff =
            operationResult === undefined
                ? undefined
                : stringField(operationResult, "diff");
        if (diff === undefined) continue;
        if (rendered) component.addChild(new Spacer(1));
        component.addChild(
            new Text(renderWorkerUnifiedDiff(diff, theme), 0, 0),
        );
        rendered = true;
    }
    return component;
...
export function formatFileEditStaticCall(
    changes: string | undefined,
    theme?: PiThemeLike,
): string {
    const operations = parseEditChangeSet(changes);
    if (operations.length === 0)
        return style(theme, "toolTitle", "file_edit", true);
    return operations
        .map((operation) => staticHeader(operation, theme))
        .join("\n");
}

export function renderFileEditFallback(
    ...
    for (const operation of operationRecords(value)) {
        ...
        if (diff !== undefined)
            lines.push(renderWorkerUnifiedDiff(diff, theme));
        const error = asRecord(operation.error);
        const message =
            error === undefined ? undefined : stringField(error, "message");
        if (message !== undefined) lines.push(message);
    }
    return lines;
}

export function parseEditChangeSet(
    changes: string | undefined,
): ParsedEditOperation[] {
    ...
    while (index < lines.length) {
        const match = lines[index]!.match(
            /^\*\*\* (Write|Patch|Rewrite|Delete|Move) File: (.+)$/u,
        );
        if (match === null) {
            index += 1;
            continue;
        ...
        while (index < lines.length) {
            ...
            if (
                /^\*\*\* (?:Write|Patch|Rewrite|Delete|Move) File: /u.test(
                    line,
                ) ||
                line === "*** End Edit"
            )
...
export function renderWorkerUnifiedDiff(
    diff: string,
    theme: PiThemeLike,
): string {
    const entries = parseUnifiedDiff(diff);
    const rows = entries.filter(
        (entry): entry is Extract<DiffEntry, { kind: "row" }> =>
            entry.kind === "row",
    );
    ...
    while (index < entries.length) {
        ...
        if (entry.kind === "ellipsis") {
            output.push(
                theme.fg("toolDiffContext", ` ${"".padStart(width, " ")} ...`),
            );
            index += 1;
            continue;
        }
        if (entry.prefix === "-") {
            ...
            if (removed.length === 1 && added.length === 1) {
                const intraline = renderIntraLineDiff(
                    removed[0]!.content,
                    added[0]!.content,
                    theme,
                );
                output.push(
                    theme.fg(
                        "toolDiffRemoved",
                        `-${String(removed[0]!.line).padStart(width, " ")} ${intraline.removed}`,
                    ),
                );
                output.push(
                    theme.fg(
                        "toolDiffAdded",
                        `+${String(added[0]!.line).padStart(width, " ")} ${intraline.added}`,
                    ),
                );
            } else {
                for (const row of removed)
                    output.push(renderDiffRow(row, width, theme));
...
function displayIdentity(operation: ParsedEditOperation): {
    label: string;
    path: string;
} {
    const label =
        operation.kind === "write" || operation.kind === "rewrite"
            ? "write"
            : operation.kind === "patch"
              ? "edit"
              : operation.kind;
    const path =
        operation.kind === "move" && operation.source !== undefined
...
function renderWritePreview(
    ...
    const rendered = visible.map((line) =>
        language === undefined ? theme.fg("toolOutput", line) : line,
    );
    if (remaining > 0) {
        rendered.push(
            theme.fg(
...
function renderDiffRow(
    row: Extract<DiffEntry, { kind: "row" }>,
    width: number,
    theme: PiThemeLike,
): string {
    const role =
    ...
    return theme.fg(
        role,
        `${row.prefix}${String(row.line).padStart(width, " ")} ${row.content}`,
    );
}
...
function detailsHaveFailure(value: JsonValue | undefined): boolean {
    return operationRecords(value).some(
...
function joinCall(
    ...
    return [
        ...
        ...values
            ...
            .map((value) => style(theme, "toolOutput", value)),
    ].join(" ");
}
