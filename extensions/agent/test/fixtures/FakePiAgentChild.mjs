...
async function handle(message) {
    ...
    if (message.type === "init") {
        process.send?.({
            ok: true,
            type: "ready",
            webUpstream: "http://127.0.0.1:43199/",
        });
        return;
    }
    ...
    if (message.type === "agent.command") {
        if (!agents.has(message.agentId))
            throw new Error(`unknown ${message.agentId}`);
        if (message.command === "prompt" && message.message === "__crash__") {
            process.exit(23);
        }
        if (
            message.command === "prompt" &&
            message.message === "__disconnect__"
        ) {
        ...
        if (
            message.command === "prompt" &&
            message.message === "__tool-cancel__"
        ) {
