...
test("Agent model projection filters capabilities and strips non-model input fields", () => {
    ...
    const projected = projectAgentModelTools([
        ...
        {
            description: "Host-internal capability",
    ...
    assert.deepEqual(
        ...
        ["bash_run", "file_edit", "tmux_read", "tmux_input"],
    );
    assert.match(projected[0]?.description ?? "", /tmux_run/u);
    assert.notEqual(projected[0]?.description, "Run bash");
    assert.deepEqual(
        (projected[0]?.inputSchema as { properties?: Record<string, unknown> })
            .properties,
        { command: { type: "string" } },
    );
    assert.deepEqual(
        (projected[0]?.inputSchema as { required?: string[] }).required,
        ["command"],
    );
    assert.equal(
        (projected[1]?.inputSchema as { properties?: Record<string, unknown> })
    ...
    assert.deepEqual(
        (projected[3]?.inputSchema as { properties?: Record<string, unknown> })
            .properties,
        {
            input: { type: "string" },
            pane: { type: "string" },
    ...
    assert.notEqual(
        bashSchema.properties.purpose,
        undefined,
        "projection must not mutate the canonical Worker schema",
    );
});
...
test("Agent model projection owns usage descriptions without mutating Worker schema descriptions", () => {
    const inputSchema = {
        properties: {
            ...
            timeout: {
                description: "Worker neutral timeout",
                type: ["number", "null"],
            },
            wait: {
                description: "Worker neutral wait",
                type: ["string", "null"],
            },
        },
        required: ["command"],
    ...
    assert.equal(
        inputSchema.properties.wait.description,
        "Worker neutral wait",
    );
    assert.equal(
        inputSchema.properties.timeout.description,
        "Worker neutral timeout",
    );
    ...
    assert.match(
        read?.description ?? "",
        /consumes all unread transcript data/u,
    );
...
test("Agent model result projection preserves file source and continuation semantics", () => {
    ...
    assert.match(
        read,
        /file=\.\/src\/demo\.ts view=content language=typescript/u,
    );
    assert.match(read, /semantic preview clipped/u);
    assert.match(
        read,
        /nextSelector=4; continue with file_read on \.\/src\/demo\.ts/u,
    );
    assert.match(read, /file=\.\/README\.md view=metadata/u);
    assert.match(
        read,
        /metadata=\{"exists":true,"mode":420,"sizeBytes":42,"type":"file"\}/u,
    );
    ...
    assert.match(
        glob,
        /nextCursor=glob-next; continue with file_glob cursor only/u,
    );
    ...
    assert.match(
        grep,
        /nextLine=12; continue file_grep against exact path \.\/src\/index\.ts/u,
    );
    assert.match(
        grep,
        /nextCursor=grep-next; continue with file_grep cursor only/u,
    );
});
...
test("Agent model result projection keeps tmux identities and transcript recovery", () => {
    ...
    assert.match(
        run,
        /task=task-a status=running pane=pane-a paneName=task-a detached=true/u,
    );
    ...
    assert.match(
        inspect,
        /pane=pane-a name=main status=running cwd=\/repo size=120x40/u,
    );
    assert.match(
        inspect,
        /rerun tmux_inspect pane=pane-a with narrower start\/end/u,
    );

    assert.match(
        projectAgentModelToolResult("tmux_manage", {
            panes: [
                {
                    id: "pane-a",
                    name: "main",
    ...
    assert.equal(
        projectAgentModelToolResult("tmux_manage", {
            pane: { id: "pane-b", name: "shell" },
        }),
        "pane=pane-b name=shell",
    );
    assert.equal(
        projectAgentModelToolResult("tmux_manage", { closedTaskId: "task-a" }),
        "closedTask=task-a",
    );
});
...
test("Agent model file_edit projection preserves failures without echoing full diffs", () => {
    ...
    assert.match(
        result,
        /error=file\.patchConflict: context changed: retryable/u,
    );
