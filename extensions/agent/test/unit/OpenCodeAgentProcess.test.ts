...
test("OpenCode process uses the provider-neutral Agent tool input and result projection", async () => {
    ...
    try {
        ...
        const log = (
            await readFile(
                join(runtimeDirectory, "fake-opencode-child.log"),
                "utf8",
            )
        )
            .trim()
            ...
            return Object.freeze({
                ...
                async send(message: ExtensionJsonValue) {
                    if (!child.connected || child.send === undefined)
                        throw new Error("Test child IPC is unavailable.");
