...
test("Agent provider runtime separates versioned provider code from stable installation and state", () => {
    ...
    assert.equal(
        ...
        "/extension-state/agent/providers/pi",
    );
    assert.equal(
        paths.prefixDirectory,
        "/extension-state/agent/providers/pi/prefix/1.2.3",
    );
    assert.equal(
        paths.installationDirectory,
        "/extension-state/agent/providers/pi/install",
    );
    assert.equal(
        paths.stateDirectory,
        "/extension-state/agent/providers/pi/state",
    );
    assert.equal(
        paths.cacheDirectory,
        "/extension-state/agent/providers/pi/cache",
    );
