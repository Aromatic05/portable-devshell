...
test("Pi devshell adapter requires an injected tool session instead of opening Control itself", async () => {
    ...
    assert.deepEqual(await bridge.loadResources(), {
        contextFiles: [],
        prompts: [],
        skills: [],
    });
...
test("Pi devshell tool hard-limits model content for long single-line progress and final results", async () => {
    ...
    assert.match(
        progressText,
        /semantic preview clipped: \d+ characters total/u,
    );
    ...
    assert.match(
        finalText,
        new RegExp(recoveryPath.replaceAll("/", "\\/"), "u"),
    );
    ...
    assert.equal(
        (result.details as { stdoutArtifact?: { handle?: string } })
            .stdoutArtifact?.handle,
        "private-handle",
    );
});
...
test("Pi devshell workspace resources load native project skills and prompt templates from the remote target", async () => {
    ...
    const resources = await loadDevshellPiWorkspaceResources(
        ...
        async (toolName, input): Promise<JsonValue> => {
            ...
            const requests = (
                input as { files: Array<{ path: string; view: string }> }
            ).files;
    ...
    assert.equal(
        resources.skills[0]?.content.includes(
            "Check tests before implementation.",
        ),
        true,
    );
...
test("Pi devshell workspace resources probe optional remote directories before globbing", async () => {
    ...
    for (const scenario of cases) {
        ...
        const resources = await loadDevshellPiWorkspaceResources(
            ...
            async (toolName, input, operationId): Promise<JsonValue> => {
                ...
                if (operationId === "pi-resources-glob") {
                    resourceFindPaths = (input as { patterns: unknown })
                        .patterns;
                    return {
                        entries: [...scenario.files.keys()].map((path) => ({
                            path,
                            type: "file",
                        })),
                    };
                ...
                const path = (input as { files: Array<{ path: string }> })
                    .files[0]!.path;
        ...
        assert.deepEqual(
            resourceFindPaths,
            scenario.expectedPaths.length === 0
                ? undefined
                : scenario.expectedPaths,
        );
        assert.deepEqual(
            resources.prompts.map((prompt) => prompt.name),
            scenario.promptNames,
        );
        assert.deepEqual(
            resources.skills.map((skill) => skill.resource.name),
            scenario.skillNames,
        );
    }
});
...
test("Pi devshell renderer formats common calls without JSON fallback", () => {
    ...
    assert.equal(
        formatPiToolCall("bash_run", {
            command: "pnpm test",
            cwd: "./extensions/agent",
        }),
...
test("Pi devshell renderer explicitly covers the current Agent model tool surface", () => {
    ...
    for (const toolName of expected)
        assert.equal(hasExplicitPiToolRenderer(toolName), true, toolName);
...
test("Pi devshell read renderer follows native collapsed and expanded semantics", () => {
    const args = {
        files: [{ path: "./src/demo.ts", selector: "10-12", view: "content" }],
    };
    const callContext = context(args);
    const call = renderPiToolCall(
        "file_read",
        args,
        identityTheme,
        callContext,
    ...
    const result = {
        content: [
            { type: "text", text: "10:const value = 1;\n11:return value;" },
        ],
        details: {
            files: [
                {
...
test("Pi devshell glob and metadata read results use native compact rows rather than structured keys", () => {
    assert.equal(
        ...
        [
            "src/",
            "src/index.ts",
            "[More results available: continue with next cursor]",
        ].join("\n"),
    );
    const info = formatPiToolResult(
        "file_read",
        {
            content: [],
            details: {
...
test("Pi devshell bash renderer shows a width-aware tail and completion status", () => {
    ...
    const result = renderPiToolResult(
        "bash_run",
        {
            content: [],
            details: {
    ...
    const partial = renderPiToolResult(
        "bash_run",
        {
            content: [],
            details: {
...
test("Pi devshell tmux renderer consumes Worker output arrays and keeps task state compact", () => {
    ...
    assert.equal(
        rendered,
        ["build", "test", "task-abc · running · detached"].join("\n"),
    );
...
test("Pi devshell renderer turns file grep results into readable sections", () => {
    ...
    assert.equal(
        rendered,
        ["src/a.ts", "  1:alpha", "  2:beta", "src/b.ts", "  7:gamma"].join(
            "\n",
        ),
    );
...
test("Pi devshell file edit always requests diff details without exposing them to the model", () => {
    ...
    assert.deepEqual(prepareAgentModelToolInput("file_edit", { changes }), {
        changes,
        resultDetail: "diff",
    });
    assert.deepEqual(
        prepareAgentModelToolInput("file_edit", {
            changes,
...
test("Pi devshell Write File renders like native Pi write and has no success result block", () => {
    ...
    const result = renderPiToolResult(
        ...
        {
            content: [
                { type: "text", text: "write ./tool-demo3.txt applied +3" },
            ],
            details: {
                operations: [
                    {
...
test("Pi devshell Patch File renders a separate stable native-style numbered diff result", () => {
    ...
    const resultSlot = renderPiToolResult(
        "file_edit",
        {
    ...
    const rendered = [
        ...visibleSelfLines(call),
        ...visibleSelfLines(resultSlot),
    ].join("\n");
    for (const marker of ["***", "--- original", "+++ modified", "@@"])
...
test("Pi devshell read renderer preserves per-file partial errors", () => {
    ...
    const collapsed = formatPiToolResult(
        "file_read",
        { content: [], details },
        false,
    );
    assert.match(
        collapsed,
        /^\.\/missing\.txt · file\.notFound: path does not exist$/mu,
    );
    assert.doesNotMatch(collapsed, /1:ok/u);

    const expanded = formatPiToolResult(
        "file_read",
        { content: [], details },
        true,
    );
