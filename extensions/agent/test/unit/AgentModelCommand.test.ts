...
function fixtures(events: string[]) {
    ...
    const runtime: AgentModelRuntimePort = {
        async abort(value) {
            events.push(`abort:${(value as { agentId: string }).agentId}`);
        },
        async followUp(value) {
            events.push(`follow:${(value as { agentId: string }).agentId}`);
        },
        get(agentId) {
        ...
        async prompt(value) {
            events.push(`prompt:${(value as { agentId: string }).agentId}`);
        },
        async reload(value) {
            events.push(`reload:${(value as { agentId: string }).agentId}`);
        },
        async start(value) {
            ...
            const created = record(
                "new",
                target.slice(0, delimiter),
                target.slice(delimiter + 1),
            );
            records.set(created.agentId, created);
            return created;
        },
        async steer(value) {
            events.push(`steer:${(value as { agentId: string }).agentId}`);
        },
        async stop(value) {
        ...
        async waitForIdle(value) {
            events.push(`wait:${(value as { agentId: string }).agentId}`);
        },
    };
...
test("Agent model start is pinned to the authoritative instance/workspace", async () => {
    ...
    const result = await executeAgentModelCommand(
        runtime,
        providers,
        ["start", "--provider", "pi"],
        invocation(),
    ...
    assert.equal(
        events[0],
        'start:{"provider":"pi","target":"worker-a:/repo"}',
    );
});
...
test("Agent model lifecycle cannot control an Agent outside the current Context", async () => {
    const events: string[] = [];
    const { providers, runtime } = fixtures(events);
    const listed = await executeAgentModelCommand(
        runtime,
        providers,
        ["list"],
        invocation(),
