...
export async function listSkills(
    ...
    for (const source of sources) {
        for (const name of await skillDirectoryNames(
            source.path,
            warnings,
            source.source,
        )) {
            if (accepted.has(name)) {
                warnings.push(
                    `${source.source}: skipped duplicate Skill '${name}'; a higher-priority source already provides it`,
                );
                continue;
            }
            ...
            const preview = await readTextWithinRoot(
                root,
                "SKILL.md",
                MAX_LIST_PREVIEW_BYTES,
                true,
            ).catch((error) => {
                warnings.push(
                    `${source.source}: skipping Skill '${name}': ${errorMessage(error)}`,
                );
                return undefined;
            });
            if (preview === undefined) continue;
            skills.push({
                description: skillDescription(preview.content),
                name,
...
export async function searchSkills(
    ...
    if (normalized.length === 0)
        throw skillUsageError("skill search requires a non-empty query");
...
export async function loadSkill(
    ...
    for (const source of skillSources(options)) {
        ...
        try {
            const entry = await readTextWithinRoot(
                root,
                "SKILL.md",
                MAX_ENTRY_BYTES,
                false,
    ...
    if (errors.length > 0)
        throw new Error(`Could not load Skill '${name}': ${errors.join("; ")}`);
    throw new Error(
        `Unknown Skill '${name}'. Run devshell skill list to discover installed Skills.`,
    );
}
...
export async function readSkillFile(
    ...
    if (relativePath === "SKILL.md")
        throw new Error("Use skill load to read SKILL.md");
    const selected = await selectSkillSource(name, options);
    const file = await readTextWithinRoot(
        selected.root,
        relativePath,
        MAX_ENTRY_BYTES,
        false,
...
async function skillDirectoryNames(
    path: string,
    warnings: string[],
    source: SkillSourceName,
): Promise<string[]> {
    ...
    try {
        ...
        warnings.push(
            `${source}: could not scan ${path}: ${errorMessage(error)}`,
        );
        return [];
    }
    return entries
...
async function relatedFiles(root: string): Promise<string[]> {
    ...
    async function walk(directory: string): Promise<void> {
        if (
            files.length >= MAX_RELATED_FILES ||
            scanned >= MAX_RELATED_SCAN_ENTRIES
        )
            return;
...
async function readText(
    ...
    try {
        ...
        if (!info.isFile())
            throw new Error("Skill entry must be a regular file");
        if (!allowTruncate && info.size > maxBytes) {
            throw new Error(
                `Skill file is ${info.size} bytes; maximum is ${maxBytes}`,
            );
        }
...
async function readTextWithinRoot(
    ...
    if (
        escaped === ".." ||
        escaped.startsWith(parentPrefix) ||
        isAbsolute(escaped)
    ) {
...
function skillDescription(markdown: string): string {
    ...
    if (lines[index]?.trim() === "---") {
        for (
            ...
            const match = /^description\s*:\s*(.+)$/iu.exec(
                lines[index]!.trim(),
            );
            if (match !== null)
                return normalizeDescription(stripQuotes(match[1]!));
        }
        index += 1;
    }
...
function normalizeDescription(value: string): string {
    ...
    return normalized.length <= 500
        ? normalized
        : `${normalized.slice(0, 499).trimEnd()}…`;
}
...
function validateSkillName(name: string): void {
    if (
        ...
        throw skillUsageError(
            "Skill name must be one non-empty directory name",
        );
    }
}
