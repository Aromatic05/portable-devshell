...
export async function executeSkillModelCommand(
    ...
    if (workers === undefined)
        throw new Error(
            "Skill Extension model commands require the workers capability.",
        );
    if (argv.length === 0 || ["help", "--help", "-h"].includes(argv[0] ?? "")) {
        if (argv.length > 1)
            throw usageError("skill help does not accept extra arguments");
    ...
    try {
        switch (argv[0]) {
            ...
            case "search":
                ...
                return json(
                    await modelSearch(session, argv[1]!, invocation.signal),
                );
            case "load":
            case "inspect":
                expect(argv, 2, `skill ${argv[0]} <name>`);
                return json(
                    await modelLoad(session, argv[1]!, invocation.signal),
                );
            case "read":
...
async function modelList(
    ...
    return {
        skills: [
            ...project,
            ...local.skills.filter((skill) => !accepted.has(skill.name)),
...
async function modelSearch(
    ...
    if (normalized.length === 0)
        throw usageError("skill search requires a non-empty query");
...
async function modelRead(
    ...
    if (project !== undefined) {
        ...
        const content = await workerRead(
            session,
            `./.agents/skills/${name}/${relativePath}`,
            signal,
        );
        return {
    ...
    return sanitizeRead(
        await readSkillFile(name, relativePath, { project: false }),
    ) as unknown as ExtensionJsonValue;
}
...
async function projectSkills(
    ...
    const paths = entries
        .flatMap((entry) => {
            ...
            return typeof value.path === "string" &&
                /^\.\/\.agents\/skills\/[^/]+\/SKILL\.md$/u.test(value.path)
                ? [value.path]
                : [];
        })
        .slice(0, 256);
    ...
    return files.flatMap((entry) => {
        ...
        if (typeof value.path !== "string" || typeof value.content !== "string")
            return [];
        const match = /^\.\/\.agents\/skills\/([^/]+)\/SKILL\.md$/u.exec(
            value.path,
        );
...
async function workerRead(
    ...
    const result = asRecord(
        await session.callTool("file_read", { path }, { signal }),
    );
    if (typeof result.content !== "string")
        throw new Error(`Worker file_read returned no content for ${path}.`);
...
function sanitizeRead(
    value: ReadSkillFileResult,
): Omit<ReadSkillFileResult, "sourcePath"> {
    const { sourcePath: _sourcePath, ...safe } = value;
    return safe;
}
...
function skillDescription(markdown: string): string {
    const frontmatter = /^---\n[\s\S]*?^description\s*:\s*(.+)$[\s\S]*?^---$/imu
        .exec(markdown)?.[1]
        ?.trim();
    if (frontmatter !== undefined && frontmatter.length > 0)
        return frontmatter.replace(/^['"]|['"]$/gu, "");
...
function expect(
    ...
    if (argv.length !== length)
        throw usageError(`Usage: devshell ${usageText}`);
}
