...
async function getSkill(
    ...
    if (assets === undefined)
        throw new Error("Skill Extension requires the assets capability.");
...
function catalogOptions(
    ...
    if (requestedWorkspace === undefined) {
        if (caller === undefined) {
            throw usageError(
                "skill requires the local CLI working directory or --workspace <directory>",
            );
        }
    ...
    if (caller === undefined) {
        throw usageError(
            "relative --workspace requires the local CLI working directory",
        );
    }
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (value === "--workspace") {
            if (workspace !== undefined)
                throw usageError("skill --workspace may be supplied only once");
        ...
        if (value.startsWith("-"))
            throw usageError(`Unknown skill option: ${value}`);
...
function expectPositionals(
    ...
    if (values.length !== expected)
        throw usageError(`Usage: devshell ${usage}`);
}
...
function requireLocalOwner(
    ...
    if (!invocation.localOwner) {
        throw new Error(
            "Skill commands are restricted to the local owner CLI.",
        );
    }
}
