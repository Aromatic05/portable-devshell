...
import type {
    McpClientFactory,
    McpClientPort,
} from "../../src/builtin/McpClientRuntime.ts";
import {
    executeMcpCommand,
    executeMcpModelCommand,
    type McpCommandRuntime,
} from "../../src/builtin/McpCommand.ts";
...
test("MCP command owns profile CRUD and restricts mutations to the local owner", async (t) => {
    ...
    assert.deepEqual(
        await executeMcpCommand(
            runtime,
            ["add", "demo", "https://example.com/mcp"],
            invocation(),
        ),
        {
            kind: "json",
    ...
    assert.deepEqual(
        await executeMcpCommand(runtime, ["list"], invocation(false)),
        {
            kind: "json",
            value: [{ name: "demo", url: "https://example.com/mcp" }],
        },
    ...
    assert.deepEqual(
        await executeMcpCommand(runtime, ["get", "demo"], invocation(false)),
        {
            kind: "json",
            value: { name: "demo", url: "https://example.com/mcp" },
        },
    ...
    assert.deepEqual(
        await executeMcpCommand(runtime, ["remove", "demo"], invocation()),
        {
            kind: "json",
            value: { name: "demo", url: "https://example.com/mcp" },
        },
        ...
        async connect(profile, signal): Promise<McpClientPort> {
            ...
            return {
                async callTool(name, input, requestSignal) {
                    events.push(
                        `call:${name}:${JSON.stringify(input)}:${requestSignal === connectSignal ? "same-signal" : "other-signal"}`,
                    );
                    return {
                        content: [
                            { text: String(input.text ?? ""), type: "text" },
                        ],
                    };
                },
                async close() {
                ...
                async listTools(requestSignal) {
                    events.push(
                        `tools:${requestSignal === connectSignal ? "same-signal" : "other-signal"}`,
                    );
