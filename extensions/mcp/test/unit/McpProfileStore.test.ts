...
import {
    McpProfileStore,
    validateMcpProfile,
} from "../../src/builtin/McpProfileStore.ts";
...
test("MCP profile store persists validated profiles atomically and sorts them", async (t) => {
    ...
    await store.add(
        validateMcpProfile("zeta", "https://example.com/mcp#ignored"),
    );
    ...
    assert.deepEqual(await store.get("alpha"), {
        name: "alpha",
        url: "http://127.0.0.1:3000/mcp",
    });
    await assert.rejects(
        store.add(validateMcpProfile("alpha", "https://other.example/mcp")),
        /already exists/u,
    );
    assert.deepEqual(await store.remove("zeta"), {
        name: "zeta",
    ...
    const persisted = JSON.parse(
        await readFile(join(root, "profiles.json"), "utf8"),
    );
    ...
    if (process.platform !== "win32") {
        assert.equal(
            (await stat(join(root, "profiles.json"))).mode & 0o777,
            0o600,
        );
    }
});
...
test("MCP profiles reject unsafe identities, schemes, and embedded credentials", () => {
    assert.throws(
        () => validateMcpProfile("../escape", "https://example.com/mcp"),
        /profile name/u,
    );
    assert.throws(
        () => validateMcpProfile("demo", "file:///tmp/server"),
        /http or https/u,
    );
    assert.throws(
        () => validateMcpProfile("demo", "https://user:pass@example.com/mcp"),
