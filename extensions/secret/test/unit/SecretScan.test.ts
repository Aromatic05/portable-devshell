...
test("secret scan reports locations without returning secret values", async () => {
    ...
    try {
        ...
        assert.deepEqual(result.findings, [
            { line: 1, path: "visible.txt", type: "github_token" },
        ]);
...
test("secret scan respects ignore files and skips obvious placeholders", async () => {
    ...
    try {
        ...
        await writeFile(
            join(root, "placeholder.env"),
            "SECRET = '${EXAMPLE:-dev-change-me}'\n",
            "utf8",
        );
        await writeFile(
            join(root, "visible.env"),
            "API_KEY = 'realistic-live-value-123'\n",
            "utf8",
        );
...
test("secret scan respects ignored directories", async () => {
    const root = await mkdtemp(
        join(tmpdir(), "devshell-secret-ignore-directory-"),
    );
    try {
        await mkdir(join(root, "ignored"));
        await writeFile(join(root, ".gitignore"), "ignored/\n", "utf8");
        await writeFile(
            join(root, "ignored/secret.env"),
...
test("secret scan applies glob and result limits", async () => {
    ...
    try {
        await writeFile(
            join(root, "a.env"),
            "PASSWORD = 'real-secret-value-a'\n",
            "utf8",
        );
        await writeFile(
            join(root, "b.env"),
            "PASSWORD = 'real-secret-value-b'\n",
            "utf8",
        );
        await writeFile(
            join(root, "c.txt"),
            "PASSWORD = 'real-secret-value-c'\n",
            "utf8",
        );
...
test("secret scan honors nested ignore scopes and negation", async () => {
    const root = await mkdtemp(
        join(tmpdir(), "devshell-secret-nested-ignore-"),
    );
    try {
        ...
        await writeFile(
            join(root, "drop.env"),
            "PASSWORD = 'drop-secret-value'\n",
            "utf8",
        );
        await writeFile(
            join(root, "keep.env"),
            "PASSWORD = 'keep-secret-value'\n",
            "utf8",
        );
        await writeFile(join(root, "nested", ".ignore"), "local.txt\n", "utf8");
        await writeFile(
            join(root, "nested", "local.txt"),
            "PASSWORD = 'nested-secret-value'\n",
            "utf8",
        );
        await writeFile(
            join(root, "nested", "visible.txt"),
            "PASSWORD = 'visible-secret-value'\n",
            "utf8",
        );
        const result = await scanSecrets({ cwd: root, limit: 20 });
