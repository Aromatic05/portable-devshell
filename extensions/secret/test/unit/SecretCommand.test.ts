...
import { executeSecretCommand } from "../../src/builtin/command/SecretCommand.ts";
...
test("Secret command rejects non-owner invocation and missing cwd for relative paths", async () => {
    await assert.rejects(
        executeSecretCommand(["scan", "/tmp"], invocation(undefined, false)),
        /local owner/u,
    );
    await assert.rejects(
        executeSecretCommand(["scan", "."], invocation()),
        /working directory/u,
    );
});
...
test("Secret command honors cancellation without faulting the Extension", async () => {
    ...
    try {
        ...
        await assert.rejects(
            executeSecretCommand(["scan", root], {
                localOwner: true,
