...
export async function executeSecretModelCommand(
    ...
    if (argv.length === 0 || ["help", "--help", "-h"].includes(argv[0] ?? "")) {
        if (argv.length > 1)
            throw usageError("secret help does not accept extra arguments");
        return { kind: "text", text: SECRET_MODEL_USAGE };
    }
    if (argv[0] !== "scan")
        throw usageError(`Unknown secret command: ${argv[0]}`);
    const parsed = parseArgs(argv.slice(1));
    const workers = extension.capabilities.workers;
    if (workers === undefined)
        throw new Error(
            "Secret Extension model commands require the workers capability.",
        );
...
async function scanRemote(
    ...
    do {
        ...
        for (const entry of Array.isArray(result.entries)
            ? result.entries
            : []) {
            ...
            if (
                ...
                !matchesGlob(displayPath(value.path, root), options.glob)
            )
                continue;
        ...
        nextCursor =
            paths.length >= MAX_FILES || typeof result.nextCursor !== "string"
                ? undefined
                : result.nextCursor;
    ...
    for (
        let offset = 0;
        offset < paths.length && findings.length < options.limit;
        offset += READ_BATCH
    ) {
        ...
        for (const entry of Array.isArray(result.files) ? result.files : []) {
            ...
            findings.push(
                ...scanSecretText(
                    displayPath(value.path, root),
                    stripLineNumbers(value.content),
                    remaining,
                ),
            );
    ...
    return {
        findings,
        truncated:
            paths.length >= MAX_FILES || findings.length >= options.limit,
        truncatedFiles,
    };
}
...
function parseArgs(args: readonly string[]): {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (argument === "--limit") {
            ...
            if (!/^\d+$/u.test(raw))
                throw usageError(
                    "secret scan --limit requires a positive integer",
                );
        ...
        if (argument.startsWith("-"))
            throw usageError(`Unknown secret scan option: ${argument}`);
        if (pathSeen)
            throw usageError("secret scan accepts at most one directory");
        directory = argument;
        pathSeen = true;
    }
...
function normalizeDirectory(value: string): string {
    ...
    if (value.startsWith("/") || value.includes("\\")) {
        throw usageError(
            "model secret scan directory must be Workspace-relative",
        );
    }
    ...
    if (
        ...
        throw usageError(
            "model secret scan directory must stay inside the Workspace",
        );
    }
    return `./${parts.join("/")}`;
}
...
function requireOption(
    ...
    if (value === undefined || value.length === 0)
        throw usageError(`secret scan ${option} requires a value`);
    return value;
}
