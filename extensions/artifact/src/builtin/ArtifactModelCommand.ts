...
export async function executeArtifactModelCommand(
    ...
    switch (command) {
        ...
        case "shares":
            ...
            return json(
                (await artifacts.listShares())
                    .filter((record) =>
                        shareBelongsToContext(record, invocation),
                    )
        ...
        case "revoke": {
            ...
            const shareRecord = (await artifacts.listShares()).find(
                (record) => record.shareId === shareId,
            );
            if (
                shareRecord === undefined ||
                !shareBelongsToContext(shareRecord, invocation)
            ) {
        ...
        case "transfers":
            ...
            return json(
                (await artifacts.listTransfers()).filter((record) =>
                    transferBelongsToContext(record, invocation),
                ),
            );
        case "help":
        case "--help":
        ...
        default:
            throw usage(
                `Unknown artifact model command: ${command}\n\n${ARTIFACT_MODEL_USAGE}`,
            );
    }
}
...
async function share(
    ...
    const parsed = parseOptions(
        args,
        new Set(["--expires-in", "--max-downloads"]),
    );
    if (parsed.positionals.length !== 1) {
        throw usage(
            "artifact share requires <artifact:<handle>|path:<path>> [--expires-in <seconds>] [--max-downloads <count>]",
        );
    }
    const sourceText = parsed.positionals[0]!;
    const source =
        sourceText.startsWith("artifact:") &&
        sourceText.length > "artifact:".length
            ? {
                  handle: sourceText.slice("artifact:".length),
                  instance: invocation.instance,
              }
            : sourceText.startsWith("path:") &&
                sourceText.length > "path:".length
              ? {
                    instance: invocation.instance,
                    path: sourceText.slice("path:".length),
...
async function transfer(
    ...
    if (operation !== "status" && operation !== "cancel") {
        throw usage(
            "Model Artifact commands do not create cross-instance transfers. Use artifact transfer status|cancel <transferId>.",
        );
    }
    ...
    if (!transferBelongsToContext(record, invocation))
        throw unavailable("transfer", transferId);
    return operation === "status"
        ? record
        : await artifacts.cancelTransfer(transferId);
}
...
function shareBelongsToContext(
    ...
    return (
        record.source.workspace === undefined ||
        record.source.workspace === invocation.workspace
    );
}
...
function transferBelongsToContext(
    ...
    if (
        record.source.instance !== invocation.instance ||
        record.target.instance !== invocation.instance
    )
        return false;
    if (
        record.source.workspace !== undefined &&
...
function unavailable(kind: string, id: string): Error {
    return new Error(
        `Artifact ${kind} ${id} is unavailable in the current model Context.`,
    );
}
