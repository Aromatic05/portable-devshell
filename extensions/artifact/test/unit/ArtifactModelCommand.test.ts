...
function capability(calls: unknown[]): ExtensionArtifactCapability {
    return {
        ...
        async getTransfer(transferId) {
            return transferId === "cross"
                ? crossTransfer(transferId)
                : localTransfer(transferId);
        },
        async listShares() {
            return [
        ...
        async listTransfers() {
            return [localTransfer("local"), crossTransfer("cross")];
        },
        ...
        async startTransfer() {
            throw new Error("model must not start transfers");
        },
...
test("Artifact model share pins authority, instance, and path Workspace to the current Context", async () => {
    const calls: unknown[] = [];
    await executeArtifactModelCommand(
        capability(calls),
        ["share", "path:./out.txt"],
        invocation,
    );
    ...
    await assert.rejects(
        executeArtifactModelCommand(
            capability(calls),
            ["share", "path:./out.txt", "--authority", "instance-b"],
            invocation,
        ),
        /Unknown option/u,
    );
...
test("Artifact model list/revoke hides records outside the current Context", async () => {
    const calls: unknown[] = [];
    const listed = await executeArtifactModelCommand(
        capability(calls),
        ["shares"],
        invocation,
    );
...
test("Artifact model transfer status/cancel only operate on current-Context records and cannot create transfers", async () => {
    ...
    await assert.rejects(
        executeArtifactModelCommand(
            capability(calls),
            ["transfer", "instance-a", "artifact:h", "instance-b", "./x"],
            invocation,
        ),
