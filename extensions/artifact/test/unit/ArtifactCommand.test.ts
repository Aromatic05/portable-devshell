...
function fakeCapability(calls: unknown[]): ExtensionArtifactCapability {
    return {
        async cancelTransfer(transferId) {
            ...
            return { operation: "cancel", transfer: record(transferId) };
        },
        ...
        async startTransfer(input) {
            ...
            return { operation: "start", transfer: record("t1") };
        },
...
test("Artifact command maps explicit CLI source/authority into the public capability", async () => {
    const calls: unknown[] = [];
    const result = await executeArtifactCommand(
        fakeCapability(calls),
        [
            "share",
            "one",
            "path:./x",
