...
test("structured advice uses stable codes for deduplication without exposing metadata", () => {
    assert.deepEqual(
        composeComments([
            {
                code: "memory.highWatermark",
                text: "Project memory is nearing its limit.",
            },
            {
                code: "memory.highWatermark",
                text: "Updated wording is still the same warning.",
            },
            { text: "A user-provided instruction." },
            { text: "A user-provided instruction." },
        ]),
        [
            "Project memory is nearing its limit.",
            "A user-provided instruction.",
        ],
    );
});

test("clean success produces only user comments in order", () => {
    assert.deepEqual(mergeComments(["first", "second"], []), [
        "first",
        "second",
    ]);
});
...
test("user comments precede hints and keep their order", () => {
    const hints = [
        errorHint("bash.nonZeroExit", "x"),
        diagnosticHint("tmux.taskRunning", "y"),
    ];
    assert.deepEqual(mergeComments(["alpha", "beta"], hints), [
        "alpha",
        "beta",
        "[bash.nonZeroExit] x",
        "[tmux.taskRunning] y",
...
test("empty strings are removed while non-empty whitespace comments are preserved verbatim", () => {
    assert.deepEqual(mergeComments(["keep", "", "   "], []), ["keep", "   "]);
});

test("exact duplicate user comments are collapsed", () => {
    assert.deepEqual(mergeComments(["same", "same", "other"], []), [
        "same",
        "other",
    ]);
});
...
test("hints with the same stable code are deduplicated", () => {
    ...
    assert.deepEqual(mergeComments([], hints), ["[bash.nonZeroExit] first"]);
});
