...
const warningTexts: Record<string, string> = {
    ...
    "tmux.taskAdopted": "Continue using the existing task id.",
    "tmux.outputSkipped": "Earlier unread output was skipped.",
    "tmux.outputDropped": "Old output was dropped.",
    "tmux.windowResync": "Pane history was resynchronized.",
    "tmux.foreignPanes": "Unmanaged panes are omitted.",
    "tmux.paneCollected": "Use a current pane.",
    "tmux.capacityFull": "Reuse or close a pane.",
    "tmux.observationReset": "Output history may be incomplete.",
};
...
function taskStatusHints(
    task: Record<string, JsonValue> | undefined,
): ToolDiagnosticHint[] {
    if (task === undefined) return [];
    const status = asString(task.status);
    if (status === undefined || status === "running" || status === "0")
        return [];
    if (status === "unknown") {
        return [
            errorHint(
    ...
    if (
        asNumber(status as unknown as JsonValue) !== undefined ||
        /^-?\d+$/.test(status)
    ) {
        return [
            errorHint(
...
export function tmuxTaskResultHints(
    toolName: string,
    result: JsonValue,
): ToolDiagnosticHint[] {
    ...
    if (toolName === "tmux_run" && asBoolean(record.detached) === true) {
        hints.push(
            diagnosticHint(
                "tmux.runDetached",
                "The task has been detached and is still running. Continue with the returned task id; use tmux_read when you need the result unless the client explicitly provides automatic recovery.",
            ),
        );
    }

    if (toolName === "tmux_read" && asBoolean(record.detached) === true) {
        hints.push(
            diagnosticHint(
                "tmux.readDetached",
                "The read wait has been detached while the task keeps running. Reuse tmux_read when you need more output unless the client explicitly provides automatic recovery.",
            ),
        );
    }
    ...
    if (
        ...
        hints.push(
            diagnosticHint("tmux.taskRunning", "Continue with tmux_read."),
        );
    }
...
export function tmuxInspectResultHints(
    result: JsonValue,
): ToolDiagnosticHint[] {
    const record = asRecord(result);
    if (record === undefined) return [];
    const hints: ToolDiagnosticHint[] = [
        ...warningHints(asArray(record.warnings)),
    ];
    if (asBoolean(record.observationReset) === true) {
        hints.push(
            diagnosticHint(
...
export function tmuxListResultHints(result: JsonValue): ToolDiagnosticHint[] {
    ...
    if (used !== undefined && max !== undefined && used >= max && max > 0) {
        hints.push(
            diagnosticHint("tmux.capacityFull", "Reuse or close a pane."),
        );
    }
    if (asBoolean(record.observationReset) === true) {
        hints.push(
...
export function tmuxErrorHints(
    toolName: string,
    ...
    switch (body.code) {
        ...
        case "tmux.capacityReached":
            return [
                errorHint("tmux.capacityReached", "Reuse or close a pane."),
            ];
        case "tmux.paneBusy":
            return [
                errorHint(
        ...
        case "tmux.lastPane":
            return [errorHint("tmux.lastPane", "Keep the final managed pane.")];
        ...
        case "tmux.invalidFormat":
            return [errorHint(body.code, "Fix the input encoding.")];
        case "tmux.invalidPaneName":
            return [
                errorHint("tmux.invalidPaneName", "Use a valid pane name."),
            ];
        ...
        case "tmux.unavailable":
            return [
                errorHint("tmux.unavailable", "Check the tmux installation."),
            ];
        case "tmux.commandFailed":
            return [
                errorHint("tmux.commandFailed", "Inspect tmux server state."),
            ];
        ...
        case "tmux.storageFailed":
            return [errorHint("tmux.storageFailed", "Inspect worker storage.")];
        case "tmux.internalError":
        case "tmux.gcFailed":
            return [errorHint(body.code, "Inspect worker tmux state.")];
        case "tool.cancelled":
            return [
                errorHint(
                    "tool.cancelled",
