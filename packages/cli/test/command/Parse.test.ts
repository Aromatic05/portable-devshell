...
test("CliParser parses Task 11 command fixture", async () => {
    const fixturePath = fileURLToPath(
        new URL("../cli-argv.json", import.meta.url),
    );
...
test("CliParser rejects invalid command shapes", () => {
    ...
    assert.throws(() =>
        parser.parse(["instance", "call", "demo-local", "bash_run", "{bad"]),
    );
    assert.throws(() => parser.parse(["instance", "create", "demo-local"]));
    assert.throws(() =>
        parser.parse(["instance", "logs", "demo-local", "--bad"]),
    );
...
test("CliParser treats unknown top-level namespaces as cli.commands local ids", () => {
    ...
    assert.deepEqual(parser.parse(["extension", "list"]), {
        json: false,
        kind: "extension.list",
    });
    assert.deepEqual(parser.parse(["extension", "list", "--json"]), {
        json: true,
        kind: "extension.list",
    });
    assert.deepEqual(
        parser.parse(["extension", "install", "./example.dsext"]),
        {
    ...
    assert.deepEqual(
        parser.parse(["extension", "remove", "agent", "--purge"]),
        {
            extensionId: "agent",
            kind: "extension.remove",
            purge: true,
    ...
    assert.throws(
        () => parser.parse(["extension", "remove", "agent", "--unknown"]),
        /Unknown extension remove option/u,
    );
});
...
test("CliParser parses protected debug patch lifecycle commands", () => {
    ...
    assert.deepEqual(parser.parse(["debug", "targets"]), {
        kind: "debug.targets",
    });
    ...
    assert.throws(() =>
        parser.parse(["debug", "load", "worker:demo-local", "./probe.js"]),
    );
...
test("CliParser routes artifact through the native Extension command pipeline", () => {
    const parser = new CliParser();
    assert.deepEqual(
        parser.parse(["artifact", "transfer", "status", "transfer-1"]),
        {
