...
async function runRealWorkerSmoke(): Promise<void> {
    ...
    await writeFile(
        join(workspacePath, "secret.env"),
        "PASSWORD = 'real-secret-value-123'\n",
        "utf8",
    );
    ...
    const restoreWindowsIdentity =
        installUniqueWindowsTestIdentity("cli-real-worker");
    ...
    try {
        await mkdir(join(homeDirectory, ".devshell", "control"), {
            recursive: true,
        });
        await mkdir(join(homeDirectory, ".devshell", "control", "instances"), {
            recursive: true,
        });
        await writeFile(
        ...
        if (secretExitCode !== 0) {
            assert.fail(
                `secret scan failed with exit ${secretExitCode}: ${stderr.flush()}`,
            );
        }
        const secretScan = JSON.parse(stdout.flush()) as {
            findings: Array<{ line: number; path: string; type: string }>;
        };
        assert.deepEqual(secretScan.findings, [
        ...
        assert.deepEqual(skillGet.target, {
            collection: "managed",
        ...
        assert.match(
            stdout.flush(),
            /Read and review the requested changes\./u,
        );
        ...
        assert.equal(
            await runCli([
                ...
                workspacePath,
                "bash_run",
                '{"command":"echo portable-devshell","timeoutMs":30000}',
            ]),
            0,
        );
...
async function runStartupFailureDiagnostics(): Promise<void> {
    const homeDirectory = await createTestTempDirectory(
        "cli-startup-failure-home",
    );
    const xdgRuntimeDir = await createTestTempDirectory(
        "cli-startup-failure-runtime",
    );
    const stdout = createBuffer();
    const stderr = createBuffer();
    const runCli = async (args: string[]) =>
    ...
    try {
        ...
        await writeFile(
            join(logDirectory, "control.log"),
            "HISTORICAL_STARTUP_MARKER\n",
            "utf8",
        );
        await writeFile(
            join(controlDirectory, "config.toml"),
...
async function runControlStartupWithLockedAudit(): Promise<void> {
    const homeDirectory = await createTestTempDirectory(
        "cli-locked-audit-home",
    );
    const xdgRuntimeDir = await createTestTempDirectory(
        "cli-locked-audit-runtime",
    );
    const stdout = createBuffer();
    const stderr = createBuffer();
    const workerEnvName = workerPathEnvironmentName();
...
async function runInteractiveCreateFlow(t: {
    after(callback: () => Promise<void> | void): void;
}): Promise<void> {
...
test("CliInstance integration", async (t) => {
    await t.test(
        "CliMain covers Task 11 instance commands through control rpc",
        async (subtest) => {
            await runInstanceCommandsThroughControlRpc(subtest);
        },
    );
    await t.test(
        "CliMain reports only the current daemon startup failure",
        async () => {
            await runStartupFailureDiagnostics();
        },
    );
    await t.test(
        "CliMain starts Control without touching a locked instance audit database",
    ...
    await t.test(
        "CliMain runs Task 12 real worker smoke through control lifecycle",
        realWorkerTestOptions(workerBinaryPath),
        async () => {
    ...
    await t.test(
        "CliMain creates an instance interactively and uses it through the real control lifecycle",
        realWorkerTestOptions(workerBinaryPath),
        async (subtest) => {
...
async function handleCliCommandStream(
    codec: Codec,
    event: Event,
): Promise<void> {
    ...
    if (operation === "logs" && argv[2] === "-f") {
        await sendCliStreamEvent(codec, event, streamId, "cli.stdout", {
            chunk: "[1] stdout before\n",
        });
        await sendCliStreamEvent(codec, event, streamId, "cli.stdout", {
            chunk: "[2] stdout after\n",
        });
...
async function completeCliCommand(
    codec: Codec,
    event: Event,
    streamId: string,
    text: string,
...
function renderHarnessSnapshot(
    value: ReturnType<typeof stoppedSnapshot>,
): string {
    return [
        `instance: ${value.name}`,
        `status: ${value.status}`,
...
async function readControlPid(
    ...
    try {
        const source = (
            await readFile(
                join(homeDirectory, ".devshell", "control", "control.pid"),
                "utf8",
            )
        ).trim();
...
async function waitForControlShutdown(
    ...
    throw new Error(
        `timed out waiting for control shutdown at ${socketPath}\n${origin}`,
    );
}

async function ensureProcessExit(
    ...
    throw new Error(
        `timed out waiting for process ${pid} to exit after SIGKILL\n${origin}`,
    );
}

function captureStack(label: string): string {
    return (
        new Error(`Timeout origin: ${label}`).stack ??
        `Timeout origin: ${label}`
    );
}
