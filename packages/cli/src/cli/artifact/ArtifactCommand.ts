...
export async function executeArtifactCommand(
    ...
    switch (command) {
        ...
        case "shares":
            ...
            writeJson(stdout, await client.listArtifactShares());
            return;
        case "revoke":
            if (rest.length !== 1) {
                throw usage("artifact revoke requires <shareId>");
            }
            writeJson(stdout, await client.revokeArtifactShare(required(rest[0], "shareId")));
            return;
        case "transfer":
        ...
        case "transfers":
            ...
            writeJson(stdout, await client.listArtifactTransfers());
            return;
        case "help":
        case "--help":
        case "-h":
...
async function share(client: ArtifactCliClient, args: readonly string[]): Promise<ArtifactShareResult> {
    ...
    const expiresInSeconds = parsed.options.has("--expires-in")
        ...
        : undefined;
    return await client.createArtifactShare(authority, {
        ...source,
...
async function transfer(
    ...
    if (operation === "status") {
        ...
        return await client.getArtifactTransfer(required(args[1], "transferId"));
    }
    if (operation === "cancel") {
        if (args.length !== 2) {
            throw usage("artifact transfer cancel requires <transferId>");
        }
        return await client.cancelArtifactTransfer(required(args[1], "transferId"));
    }
    ...
    return await client.startArtifactTransfer(authority, {
        ...source,
        instance,
        operation: "start",
