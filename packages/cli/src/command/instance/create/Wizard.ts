...
export class CliWizardInstanceCreate {
    ...
    async #collectDraft(
        ...
        const enabled = await this.#confirm(
            lines,
            "enabled",
        ...
        const providerFields = await this.#providerFields(
            lines,
            schema,
            name,
            provider,
        ...
        const mcpEnabled = await this.#confirm(
            lines,
            "MCP enabled",
            schema.defaultMcpEnabled,
        );
        const mcpAuth = mcpEnabled
            ? await this.#mcpAuth(lines)
            : { auth: "none" as const };
        ...
        return {
            ...(providerFields.container === undefined
                ? {}
                : { container: providerFields.container }),
            ...(providerFields.dockerBinary === undefined
                ? {}
                : { dockerBinary: providerFields.dockerBinary }),
            ...(providerFields.podmanBinary === undefined
                ? {}
                : { podmanBinary: providerFields.podmanBinary }),
            ...(providerFields.ssh === undefined
                ? {}
                : { ssh: providerFields.ssh }),
            ...(approvalPolicy.mode === "disabled" &&
            approvalPolicy.rules === undefined
                ? {}
                : { approvalPolicy }),
            ...(env === undefined ? {} : { env }),
            ...(logs === undefined ? {} : { logs }),
    ...
    async #provider(
        lines: AsyncIterator<string>,
        schema: InstanceCreateSchema,
    ): Promise<InstanceCreateDraft["provider"]> {
        while (true) {
            const value = (
                await this.#optional(
                    lines,
                    `provider (${schema.providers.join(" | ")})`,
                    schema.defaultProvider,
                )
            ).trim();

            if (
                schema.providers.includes(
                    value as InstanceCreateDraft["provider"],
                )
            ) {
            ...
            this.#output.write(
                `provider must be one of ${schema.providers.join(", ")}.\n`,
            );
        }
    }
    ...
    async #mcpAuth(
        lines: AsyncIterator<string>,
    ): Promise<ConfigInstanceMcpDraft> {
        ...
        switch (auth) {
            ...
            case "oauth2": {
                const resourceName = await this.#required(
                    lines,
                    "OAuth resource name",
                );
                const requiredScopes = await this.#stringList(
                    lines,
                    "OAuth required scopes",
                    ["mcp"],
                );
                const documentationUrl = await this.#blankAsUndefined(
                    lines,
    ...
    async #logs(
        lines: AsyncIterator<string>,
    ): Promise<ControlInstanceLogsConfig | undefined> {
        if (!(await this.#confirm(lines, "configure log limits", false))) {
            return undefined;
        }
        const retentionDays = await this.#positiveIntegerOptional(
            lines,
            "log retention days",
        );
        const maxBytes = await this.#positiveIntegerOptional(
            lines,
            "log max bytes",
        );
        const eventBufferSize = await this.#positiveIntegerOptional(
            lines,
            "log event buffer size",
        );
        const result = {
    ...
    async #tools(
        lines: AsyncIterator<string>,
    ): Promise<ControlInstanceToolsConfig | undefined> {
        if (!(await this.#confirm(lines, "configure tool scheduler", false))) {
            return undefined;
        }
        const maxRunning = await this.#positiveIntegerOptional(
            lines,
            "scheduler max running",
        );
        const maxRunningPerSession = await this.#positiveIntegerOptional(
            lines,
            "scheduler max running per session",
        );
        const queueDepth = await this.#positiveIntegerOptional(
            lines,
            "scheduler queue depth",
        );
        const queueDepthPerSession = await this.#positiveIntegerOptional(
            lines,
            "scheduler queue depth per session",
        );
        const queueTimeoutMs = await this.#positiveIntegerOptional(
            lines,
            "scheduler queue timeout ms",
        );
        const byTool = await this.#jsonOptional<
            NonNullable<
                NonNullable<ControlInstanceToolsConfig["scheduler"]>["byTool"]
            >
        >(
            lines,
        ...
        const scheduler = {
            ...
            ...(maxRunningPerSession === undefined
                ? {}
                : { maxRunningPerSession }),
            ...(queueDepth === undefined ? {} : { queueDepth }),
            ...(queueDepthPerSession === undefined
                ? {}
    ...
    async #securityMode(
        ...
        while (true) {
            const value = await this.#optional(
                lines,
                "security mode (disabled | workspace)",
                schema.defaultSecurityMode,
            );
            if (value === "disabled" || value === "workspace") {
                return value;
            }
            this.#output.write(
                "security mode must be disabled or workspace.\n",
            );
        }
    }
        ...
        switch (provider) {
            ...
            case "docker":
                return {
                    ...
                    dockerBinary: await this.#blankAsUndefined(
                        lines,
                        "docker binary",
                    ),
                };
            case "podman":
                return {
                    ...
                    podmanBinary: await this.#blankAsUndefined(
                        lines,
                        "podman binary",
                    ),
                };
        ...
        switch (mode) {
            case "preset": {
                const preset = await this.#preset(
                    lines,
                    schema.container.presets,
                );
                return {
                    ...(await this.#managedContainerFields(
                        lines,
                        defaultContainerName,
                    )),
                    image: await this.#optional(
                        lines,
                        "preset image",
                        preset.image,
                    ),
            ...
            case "dockerfile":
                return {
                    ...
                    build: {
                        ...
                        dockerfile: await this.#blankAsUndefined(
                            lines,
                            "dockerfile path",
                        ),
                        tag:
                            (await this.#blankAsUndefined(
                                lines,
                                "build tag",
                            )) ?? `devshell-${instanceName}:latest`,
                    },
                    mode,
                };
            case "compose":
                return {
                    compose: {
                        ...
                        projectName: await this.#blankAsUndefined(
                            lines,
            ...
            case "existingStoppedContainer":
                return {
                    adoptLifecycle: await this.#confirm(
                        lines,
                        "stop adopted container on instance stop",
                        false,
                    ),
                    containerName: await this.#required(
                        lines,
                        "existing stopped container name",
                    ),
                    mode,
                };
        }
    ...
    async #containerMode(
        ...
        const options = [
            ...
            {
                label: "Adopt existing stopped container",
                mode: "existingStoppedContainer",
            },
        ] as const satisfies ReadonlyArray<{
            label: string;
            mode: InstanceContainerConfig["mode"];
        }>;
        const defaultIndex = options.findIndex(
            (entry) => entry.mode === schema.container.defaultMode,
        );
        ...
        while (true) {
            ...
            const selected = Number.isInteger(parsed)
                ? options[parsed - 1]
                : undefined;
    ...
    async #preset(
        ...
        while (true) {
            ...
            this.#output.write(
                "preset must match one of the listed presets.\n",
            );
        }
    }
    ...
    async #managedContainerFields(
        ...
        const containerName = await this.#optional(
            lines,
            "container name",
            defaultContainerName,
        );
        const user = await this.#blankAsUndefined(lines, "container user");
        const network = await this.#blankAsUndefined(
            lines,
            "container network",
        );
    ...
    async #mounts(
        lines: AsyncIterator<string>,
    ): Promise<InstanceContainerMountConfig[] | undefined> {
    ...
    async #stringList(
        ...
        const raw = await this.#optional(
            lines,
            `${label} (comma or space separated)`,
            defaults.join(","),
        );
    ...
    async #confirm(
        ...
        while (true) {
            const answer = (await this.#ask(lines, `${label} ${suffix}: `))
                .trim()
                .toLowerCase();
    ...
    async #optional(
        ...
        const answer = await this.#ask(
            lines,
            `${label}${defaultValue.length > 0 ? ` [${defaultValue}]` : ""}: `,
        );
        const trimmed = answer.trim();
        return trimmed.length === 0 ? defaultValue : trimmed;
    }

    async #blankAsUndefined(
        lines: AsyncIterator<string>,
        label: string,
    ...
    #renderSummary(summary: InstanceCreateSummary): void {
        ...
        this.#output.write(
            `mcp context mode: ${summary.mcp.contextMode ?? "explicit"}\n`,
        );
        if (summary.mcp.auth.oauth2 !== undefined) {
            this.#output.write(
                `oauth resource: ${summary.mcp.auth.oauth2.resourceName}\n`,
            );
            this.#output.write(
                `oauth scopes: ${summary.mcp.auth.oauth2.requiredScopes.join(",")}\n`,
            );
            if (summary.mcp.auth.oauth2.documentationUrl !== undefined) {
                this.#output.write(
                    `oauth documentation: ${summary.mcp.auth.oauth2.documentationUrl}\n`,
                );
            }
        }
        this.#output.write(`mcp path: ${summary.mcp.path}\n`);
        this.#output.write(
            `model Extensions: ${summary.extensions.model.join(",")}\n`,
        );
        this.#output.write(`security mode: ${summary.security.mode}\n`);
        this.#output.write(
            `approval mode: ${summary.approvalPolicy?.mode ?? "disabled"}\n`,
        );
        if ((summary.approvalPolicy?.rules?.length ?? 0) > 0) {
            this.#output.write(
                `approval rules: ${summary.approvalPolicy!.rules!.length}\n`,
            );
        }
        if (summary.env !== undefined) {
            this.#output.write(
                `instance env keys: ${Object.keys(summary.env).sort().join(",")}\n`,
            );
        }
        ...
        if (summary.tools?.scheduler !== undefined) {
            this.#output.write(
                `tool scheduler: ${JSON.stringify(summary.tools.scheduler)}\n`,
            );
        }
    }
    ...
    #renderContainerSummary(container: InstanceContainerConfig): void {
        ...
        switch (container.mode) {
            ...
            case "dockerfile":
                ...
                this.#output.write(
                    `build context: ${container.build.context}\n`,
                );
                if (container.build.dockerfile !== undefined) {
                    this.#output.write(
                        `dockerfile path: ${container.build.dockerfile}\n`,
                    );
                }
            ...
            case "compose":
                ...
                this.#output.write(
                    `compose service: ${container.compose.service}\n`,
                );
                if (container.compose.projectName !== undefined) {
                    this.#output.write(
                        `compose project: ${container.compose.projectName}\n`,
                    );
                }
                return;
            case "existingImage":
            ...
            case "existingStoppedContainer":
                ...
                this.#output.write(
                    `adopt lifecycle: ${container.adoptLifecycle === true}\n`,
                );
                return;
        }
    }
    ...
    #renderManagedContainerExtras(
        ...
        if ((container.mounts?.length ?? 0) > 0) {
            this.#output.write(
                `container mounts: ${container.mounts?.map((mount) => `${mount.source}:${mount.target}:${mount.mode}`).join(", ")}\n`,
            );
        }
        ...
        if (
            ...
            this.#output.write(
                `container env keys: ${Object.keys(container.env).sort().join(", ")}\n`,
            );
        }
    }
