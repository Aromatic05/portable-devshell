...
export function renderInstanceList(
    instances: readonly InstanceListEntry[],
): string {
...
export function renderInstanceSnapshot(snapshot: InstanceSnapshot): string {
    ...
    if (
        snapshot.lastErrorCode !== undefined ||
    ...
    if (snapshot.reverse !== undefined) {
        ...
        if (snapshot.reverse.lastErrorCode !== undefined) {
            lines.push(
                `reverseLastErrorCode: ${snapshot.reverse.lastErrorCode}`,
            );
            lines.push(
                `reverseLastErrorMessage: ${snapshot.reverse.lastErrorMessage ?? "-"}`,
            );
        }
    }
    ...
    lines.push(
        ...(activeTodos.length === 0
            ? ["Todo: none"]
            : activeTodos.map(renderTodoSummary)),
    );
    return `${lines.join("\n")}\n`;
}

function renderTodoSummary(
    todo: import("@portable-devshell/shared").ActiveTodoSummary,
): string {
