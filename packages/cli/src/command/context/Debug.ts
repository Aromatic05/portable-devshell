...
export function parseDebugCommand(argv: readonly string[]): CliParsedCommand {
    switch (argv[0]) {
        ...
        case "--help":
        case "-h":
            return expectNoExtra(argv, { kind: "help", topic: "debug" });
        case "targets":
            return expectNoExtra(argv, { kind: "debug.targets" });
        case "list":
            return expectNoExtra(argv, { kind: "debug.list" });
        case "load":
            if (
            ...
            return {
                ctxId: required(argv[4], "debug ctxId is required"),
                file: required(argv[2], "debug patch file is required"),
                kind: "debug.load",
                target: required(argv[1], "debug target is required"),
                ...(argv[6] === undefined
                    ? {}
                    : {
                          toolName: required(
                              argv[6],
                              "debug toolName is required",
                          ),
                      }),
            };
        case "release":
        case "unload":
            if (argv.length !== 2)
                throw CliRenderError.usage(
                    `debug ${argv[0]} requires <patchId>`,
                );
            return {
                kind: argv[0] === "release" ? "debug.release" : "debug.unload",
                patchId: required(argv[1], "debug patchId is required"),
            };
        default:
            throw CliRenderError.usage(
...
export async function executeDebugCommand(
    ...
    switch (command.kind) {
        case "debug.targets":
            context.writeJson(await context.clients.debug.targets());
            return true;
        case "debug.list":
            context.writeJson(await context.clients.debug.list());
            return true;
        case "debug.load":
            context.writeJson(
                await context.clients.debug.load({
                    scope: {
                        ctxId: command.ctxId,
                        ...(command.toolName === undefined
                            ? {}
        ...
        case "debug.release":
            context.writeJson(
                await context.clients.debug.release(command.patchId),
            );
            return true;
        case "debug.unload":
            context.writeJson(
                await context.clients.debug.unload(command.patchId),
            );
            return true;
        default:
            return false;
