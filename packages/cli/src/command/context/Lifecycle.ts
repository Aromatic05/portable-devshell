...
export function parseContextCommand(argv: readonly string[]): CliParsedCommand {
    switch (argv[0]) {
        ...
        case "--help":
        case "-h":
            return expectNoExtra(argv, { kind: "help", topic: "context" });
        case "messages":
        case "send":
            return parseContextMessageCommand(argv);
        case "list":
            return expectNoExtra(argv, { kind: "context.list" });
        case "disable":
        case "renew":
            if (argv.length !== 2)
                throw CliRenderError.usage(
                    `context ${argv[0]} requires <ctxId>`,
                );
            return {
                ctxId: required(argv[1], "ctxId is required"),
                kind:
                    argv[0] === "disable" ? "context.disable" : "context.renew",
            };
        default:
            throw CliRenderError.usage(
                `${`Unknown context command: ${argv[0] ?? ""}`.trim()}\n\n${renderCliTopicUsage("context")}`,
            );
    }
}
...
export async function executeContextLifecycle(
    command: CliParsedCommand,
    context: CliDispatchContext,
): Promise<boolean> {
    switch (command.kind) {
        case "context.list":
            context.writeJson(await context.clients.context.list());
            return true;
        case "context.disable":
            context.writeJson(
                await context.clients.context.disable(command.ctxId),
            );
            return true;
        case "context.renew":
            context.writeJson(
                await context.clients.context.renew(command.ctxId),
            );
            return true;
        default:
            return false;
