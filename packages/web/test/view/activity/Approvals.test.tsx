...
import { Approvals } from "../../../src/view/page/activity/Approvals.js";
...
it("shows the workspace authority for a pending tool approval", () => {
    ...
    expect(
        screen.getByText(/alpha · \/projects\/alpha · needs review/u),
    ).toBeInTheDocument();
    expect(
        screen.getByText(/Workspace: \/projects\/alpha/u),
    ).toBeInTheDocument();
...
it("summarizes the OAuth client, scopes, resources, and redirects before approval", () => {
    ...
    expect(
        within(dialog).getByText(/devshell\.example\/alpha\/mcp/u),
    ).toBeInTheDocument();
    expect(
        within(dialog).getByText(/chatgpt\.com\/callback/u),
    ).toBeInTheDocument();
});
...
it("keeps a failed approval confirmation open and shows the error in the dialog", async () => {
    ...
    fireEvent.click(
        within(
            screen.getByRole("dialog", { name: "Confirm approve" }),
