...
import {
    ...
    sessionPath,
    webReturnTo,
} from "../../src/app/session/Session.js";
...
describe("BrowserWebSession", () => {
    it("uses same-origin cookies and only adds Authorization for token exchange", async () => {
        const request = vi
            ...
            .mockResolvedValueOnce(
                new Response(
                    JSON.stringify({ auth: "none", authenticated: false }),
                    { status: 200 },
                ),
            )
            .mockResolvedValueOnce(
    ...
    it("reads the configured auth mode from the unauthorized session body", async () => {
        const request = vi
            ...
            .mockResolvedValueOnce(
                new Response(
                    JSON.stringify({ auth: "oauth2", authenticated: false }),
                    { status: 200 },
                ),
            )
            .mockResolvedValueOnce(
                new Response(
                    JSON.stringify({ auth: "token", authenticated: false }),
                    { status: 200 },
                ),
            )
            .mockResolvedValueOnce(new Response(null, { status: 204 }));
    ...
    it("preserves a safe Agent return target through token and OAuth authentication", async () => {
        ...
        expect(navigate).toHaveBeenCalledWith(
            "/devshell/web/agent/?session=ag-1",
        );
    ...
    it("rejects external and out-of-scope return targets", () => {
        expect(
            webReturnTo({
                pathname: "/web/",
                search: "?returnTo=https%3A%2F%2Fevil.test%2F",
            } as Location),
        ).toBeUndefined();
        expect(
            webReturnTo({
                pathname: "/web/",
                search: "?returnTo=%2F%2Fevil.test%2F",
            } as Location),
        ).toBeUndefined();
        expect(
            webReturnTo({
                pathname: "/devshell/web/",
                search: "?returnTo=%2Fother%2F",
            } as Location),
        ).toBeUndefined();
    });
});
