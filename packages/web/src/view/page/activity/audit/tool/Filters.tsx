...
import { webRouteHref, type AuditScope } from "../../../../../app/Route.js";
import {
...
export type AuditContextStatusFilter =
    "active" | "expired" | "disabled" | "all";
    ...
    return (
        <div className="audit-query">
            <div className="audit-query-primary">
                ...
                <label className="audit-context-id">
                    Context ID
                    <input
                        onChange={(event) =>
                            onChange({ ...filters, ctxId: event.target.value })
                        }
                        placeholder="ctxId"
                        type="search"
                        value={filters.ctxId}
                ...
                {(
                    ...
                    <button
                        ...
                        onClick={() =>
                            onChange({
                                ...filters,
                                result: value as ToolCallResult,
                            })
                        }
                        type="button"
                    >
            ...
            {advancedCount === 0 ? null : (
                <div
                    aria-label="Active filters"
                    className="audit-filter-chips"
                    role="group"
                >
                    {contextStatus === "all" ? null : (
                        <button
                            onClick={() => onContextStatusChange("all")}
                            type="button"
                        >
                            {contextStatus === "active"
                                ? "Context: Active · last 30 min ×"
                                : `Context: ${contextStatus} ×`}
                        </button>
                    )}
                    {filters.ctxId.length === 0 ? null : (
                        <button
                            onClick={() => onChange({ ...filters, ctxId: "" })}
                            type="button"
                        >
                            Context ID: {filters.ctxId} ×
                        </button>
                    )}
                    {filters.workspace.length === 0 ? null : (
                        <button
                            onClick={() =>
                                onChange({ ...filters, workspace: "" })
                            }
                            type="button"
                        >
                    ...
                    {filters.tool === "all" ? null : (
                        <button
                            onClick={() =>
                                onChange({ ...filters, tool: "all" })
                            }
                            type="button"
                        >
                            Tool: {filters.tool} ×
                    ...
                    <label>
                        ...
                        <select
                            onChange={(event) =>
                                onContextStatusChange(
                                    event.target
                                        .value as AuditContextStatusFilter,
                                )
                            }
                            value={contextStatus}
                        >
...
function groupScopeOptions(scopes: readonly AuditScopeOption[]): {
    ...
    const grouped = new Map<
        string,
        { id: string; label: string; options: AuditScopeOption[] }
    >();
    ...
    for (const option of scopes) {
        ...
        const group = grouped.get(option.group.id) ?? {
            ...option.group,
            options: [],
        };
