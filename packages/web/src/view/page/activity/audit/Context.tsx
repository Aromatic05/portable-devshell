...
export function ContextBatchDisableDialog({
    ...
    useEffect(() => {
        setSelected((current) => {
            const next = new Set(
                [...current].filter((ctxId) => candidateIds.has(ctxId)),
            );
            if (
                next.size === current.size &&
                [...next].every((ctxId) => current.has(ctxId))
            ) {
                return current;
            }
            return next;
    ...
    async function disableSelected(): Promise<void> {
        ...
        if (succeeded) {
            ...
            setFailure(
                "Some selected Contexts could not be disabled. Successful disables were kept; failed Contexts remain available to retry.",
            );
        }
    }
    ...
    const allSelected =
        candidates.length > 0 &&
        candidates.every((context) => selected.has(context.ctxId));
        ...
        >
            ...
            >
                <div className="context-batch-heading">
                    <div>
                        <h2 id="context-batch-title">
                            Disable inactive Contexts
                        </h2>
                        <p className="hint">
                            Only explicitly selected Contexts are disabled.
                        </p>
                    </div>
                    <button
                        disabled={busy}
                ...
                <div className="context-batch-controls">
                    <label>
                        ...
                        <select
                            ...
                            onChange={(event) =>
                                setPreset(
                                    event.target.value as InactivityPreset,
                                )
                            }
                            value={preset}
                        >
                ...
                <p aria-live="polite" className="hint">
                    {candidates.length} Context
                    {candidates.length === 1 ? "" : "s"} unused for at least{" "}
                    {formatThreshold(thresholdMinutes)}.
                </p>
                ...
                {candidates.length === 0 ? (
                    <p className="empty">
                        No non-disabled Contexts match this inactivity
                        threshold.
                    </p>
                ) : (
                    <div className="context-batch-table-wrap">
                        <table className="context-batch-table">
                            <thead>
                                <tr>
                                    <th>Select</th>
                                    <th>Context</th>
                                    <th>Initial instance</th>
                                    <th>Initial workspace</th>
                                    <th>Status</th>
                                    <th>Last used</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={context.ctxId}>
                                        <td>
                                            <input
                                                ...
                                                disabled={busy || confirming}
                                                onChange={() =>
                                                    toggle(context.ctxId)
                                                }
                                                type="checkbox"
                                            />
                                        </td>
                                        <td>
                                            <code>{context.ctxId}</code>
                                        </td>
                                        <td>{context.instance}</td>
                                        <td>
                                            <code>{context.workspace}</code>
                                        </td>
                                        <td>
                                            <span
                                                className={`result ${context.status === "active" ? "success" : "pending"}`}
                                            >
                                                {context.status}
                                            </span>
                                        </td>
                ...
                {confirming ? (
                    <div
                        className="context-batch-confirmation"
                        aria-live="polite"
                    >
                        <p>
                            Disable {selectedCount} selected Context
                            {selectedCount === 1 ? "" : "s"}? Disabled Contexts
                            cannot be renewed.
                        </p>
...
export function ContextIntervention({
    ...
    const context = state.readModel.contexts.find(
        (record) => record.ctxId === ctxId,
    );
    const environment =
        context === undefined
            ? undefined
    ...
    return (
        <section
            className="card context-intervention"
            aria-labelledby="context-intervention-title"
        >
            <div className="context-intervention-heading">
                ...
                {context === undefined ||
                context.status === "disabled" ? null : (
                    <div className="actions">
                        <button
                            disabled={
                                !interactive ||
                                state.operations[renewOperation] !== undefined
                            }
                            onClick={() => {
                                setFailure(undefined);
                                void store
            ...
            {context === undefined ? (
                <p className="hint">Context registry record unavailable.</p>
            ) : (
                <p className="hint">
                    Workspace:{" "}
                    {environment?.workspace ??
                        context.workspace ??
                        "not attached"}{" "}
                    · Status: {context.status} · expires {context.expiresAt}
                </p>
            )}
            {failure === undefined ? null : (
                <p className="error" role="alert">
