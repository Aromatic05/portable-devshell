...
export function ConversationSidebar({
    ...
    onClose(): void;
    persistPreferences(next: ConversationPreferencesSnapshot, patch: ConversationPreferencesPatch): void;
    ...
    const allSessions = useMemo(
        () => applyConversationPreferences(allBaseSessions, conversationPreferences),
    ...
    function renderSession(session: WebMessageSession) {
        const active = route.view === "thread" &&
            route.instance === session.instance && route.ctxId === session.ctxId;
        const key = conversationKey(session);
        ...
        >
            <span
                ...
                onPointerDown={(event) => {
                    if (editing || !preferencesAvailable || event.button !== 0) return;
            ...
            {editing ? <div className="conversation-rename-editor">
                ...
                onClick={() => {
                    onClose();
                    navigate({ page: "messages", view: "thread", instance: session.instance, ctxId: session.ctxId });
                }}
                onDoubleClick={(event) => {
                    event.preventDefault();
    ...
    return <>
        <button
            ...
            type="button"
        />
        <div className={`messages-sidebar${open ? " open" : ""}`} ref={sidebarRef}>
            <div className="messages-sidebar-heading">
                <strong>Conversations</strong>
                <div aria-label="Conversation scope" className="messages-session-scope" role="group">
                    <button aria-pressed={sessionScope === "current"} onClick={() => setSessionScope("current")} type="button">Current</button>
                    <button aria-pressed={sessionScope === "history"} onClick={() => setSessionScope("history")} type="button">History</button>
                </div>
                <button aria-label="Close conversations" className="messages-sidebar-close" onClick={onClose} type="button">×</button>
            </div>
            {sessionScope === "current" && idleCurrentCount > 0 ? <button className="messages-archive-idle" onClick={archiveIdle} type="button">Archive idle</button> : null}
            {preferenceError === undefined ? null : <p className="messages-preference-error error" role="alert">
                Conversation preferences are not being persisted: {preferenceError}
            </p>}
            <label className="messages-search">
                <span className="sr-only">Search conversations</span>
                <input onChange={(event) => setQuery(event.target.value)} placeholder="Search conversations" ref={searchRef} type="search" value={query} />
            </label>
                ...
                {visibleSessions.length === 0
                    ...
                    : sessionScope === "current"
                        ...
                        : historyGroups.map((group) => {
                            ...
                            const expanded = query.trim().length > 0 || containsSelected || expandedHistoryGroups.has(group.key);
                            return <section aria-label={group.label} className="conversation-workspace-group" key={group.key} role="group">
                                <button
                                ...
                                {expanded ? <div className="conversation-workspace-sessions">{group.sessions.map(renderSession)}</div> : null}
                            </section>;
                        })}
            </nav>
        </div>
