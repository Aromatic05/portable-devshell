...
export function Todos({ disabled = false, state, store }: { disabled?: boolean; state: WebState; store: WebStore }) {
    ...
    return (
        <section>
            ...
            <h3>Tasks</h3>
            {todos.length === 0 ? <p className="empty">No active todos are available.</p> : <div className="todo-list">{todos.map((todo) => <TodoCard
                ...
                todo={todo}
            />)}</div>}
            {selected === undefined ? null : <ConfirmationDialog
                actionLabel="Delete"
                busy={state.operations[`todo-delete:${selected.instance}:${selected.taskId}`] !== undefined}
                ...
                onConfirm={() => {
                    setFailure(undefined);
                    void store.deleteTodo(selected.instance, selected.taskId).then((succeeded) => {
                        if (succeeded) setSelected(undefined);
                        else setFailure(store.state?.error ?? state.error ?? "Todo project could not be deleted.");
                    });
                }}
            />}
        </section>
...
function TodoCard({
    ...
    return <article className="card" key={`${todo.instance}-${todo.taskId}-${todo.revision}`}>
        <h3>{todo.title}</h3>
        <p>{todo.instance} · revision {todo.revision}</p>
        <strong>{todo.completed}/{todo.total} complete · {todo.status}</strong>
        {todo.currentItem === undefined ? null : <p><strong>Current</strong>{todo.currentItem}</p>}
        {todo.checkpoint === undefined ? null : <div className="todo-checkpoint">
            <p><strong>Checkpoint</strong>{todo.checkpoint.summary}</p>
            {todo.checkpoint.next === undefined ? null : <p><strong>Next</strong>{todo.checkpoint.next}</p>}
            {todo.checkpoint.blockers === undefined || todo.checkpoint.blockers.length === 0 ? null : <p><strong>Blockers</strong>{todo.checkpoint.blockers.join(" · ")}</p>}
        </div>}
        {items.length === 0 ? null : <details className="todo-details">
            <summary>Task steps</summary>
            <ol>{items.map((item) => <li key={item.id}><span className={`result ${item.status}`}>{item.status}</span> {item.content}{item.detail === undefined ? null : <small>{item.detail}</small>}</li>)}</ol>
        </details>}
        <p><button className="danger subtle" disabled={!interactive} onClick={onDelete}>Delete project</button></p>
    </article>;
}
