    ...
    return (
        <section>
            ...
            {entry !== undefined ? <article className="detail">
                ...
                <div className="actions"><button className={entry.snapshot.status === "stopped" ? "primary" : "danger"} disabled={state.connection !== "online" || state.operations[`start:${entry.name}`] !== undefined || state.operations[`stop:${entry.name}`] !== undefined} onClick={() => setConfirmation({ instance: entry.name, action: entry.snapshot.status === "stopped" ? "Start" : "Stop" })}>{entry.snapshot.status === "stopped" ? "Start" : "Stop"}</button></div>
                <h4>Recent logs</h4><pre>{(state.logs[entry.name] ?? []).map((log) => `${log.at} ${log.message}`).join("\n") || "No recent logs."}</pre>
            </article> : null}
            {confirmation !== undefined ? <ConfirmationDialog actionLabel={confirmation.action} busy={operation !== undefined && state.operations[operation] !== undefined} description={`${confirmation.action} ${confirmation.instance}?`} onCancel={() => setConfirmation(undefined)} onConfirm={() => { void (confirmation.action === "Start" ? store.start(confirmation.instance) : store.stop(confirmation.instance)).finally(() => setConfirmation(undefined)); }} /> : null}
        </section>
    );
}
