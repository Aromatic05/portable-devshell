...
export function ToolCalls({
    ...
    async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
        ...
        if (!online || !concreteContext || draft.trim().length === 0) return;
    ...
    return <section>
        ...
        <ToolCallFilters contexts={contexts} filters={filters} instances={instances} onChange={changeFilters} onClear={() => { setDraft(""); setFilters(emptyToolCallFilters); }} tools={tools} />
        <section className="card context-composer" aria-labelledby="context-message-title">
            ...
            {concreteContext ? <>
                ...
                <form onSubmit={(event) => void submit(event)}>
                    <label>Message<textarea disabled={!online} maxLength={20_000} onChange={(event) => setDraft(event.target.value)} placeholder="Send guidance to the Agent working in this Context" rows={4} value={draft} /></label>
                    <button className="primary" disabled={!online || draft.trim().length === 0 || (operation !== undefined && state.operations[operation] !== undefined)} type="submit">{operation !== undefined && state.operations[operation] !== undefined ? "Sending…" : "Send message"}</button>
                </form>
                {!online ? <p className="empty">Reconnect before sending a message.</p> : null}
