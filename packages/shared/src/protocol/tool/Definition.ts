...
export function toolNamespace(name: string): string | undefined {
    ...
    if (separator <= 0 || separator === name.length - 1 || name.indexOf("_", separator + 1) !== -1) {
        return undefined;
    }
    const namespace = name.slice(0, separator);
    const operation = name.slice(separator + 1);
    return /^[a-z0-9]+$/u.test(namespace) && /^[A-Za-z0-9]+$/u.test(operation) ? namespace : undefined;
}
...
function parseRequiredCapabilities(value: unknown): ToolCapability[] {
    if (!Array.isArray(value)) throw new Error("tool.requiredCapabilities must be an array");
...
export const toolSchema = {
    parse(value: unknown): ToolDefinition {
        ...
        if (namespace === undefined) throw new Error("tool.name must use namespace_operation form");
        if (typeof value.description !== "string") throw new Error("tool.description must be a string");
