...
function validateModelExtensions(
    instance: ControlInstanceConfig,
...
function validateAlerts(
    alerts: ControlInstanceAlertsConfig | undefined,
    base: readonly (string | number)[],
): void {
    if (alerts === undefined) return;
    if (
        alerts.intervalMs !== undefined &&
        (!Number.isSafeInteger(alerts.intervalMs) || alerts.intervalMs < 1_000)
    ) {
        throw configInputError(
            "semantic",
    ...
    nonNegativeInteger(alerts.maxUncommittedChanges, [
        ...base,
        "alerts",
        "maxUncommittedChanges",
    ]);
    positiveInteger(alerts.workerMemoryBytes, [
        ...base,
        "alerts",
        "workerMemoryBytes",
    ]);
    for (const [index, script] of (alerts.scripts ?? []).entries()) {
        if (
            script.id.trim().length === 0 ||
            script.command.length === 0 ||
            script.command.some((part) => part.trim().length === 0)
        ) {
            throw configInputError(
                "semantic",
        ...
        positiveInteger(script.timeoutMs, [
            ...base,
            "alerts",
            "scripts",
            index,
...
function validateGlobalMcp(config: ControlConfig): void {
    if (
        !Number.isSafeInteger(config.mcp.listenPort) ||
        config.mcp.listenPort < 0 ||
        config.mcp.listenPort > 65535
    ) {
        throw configInputError(
            "semantic",
            ["mcp", "listenPort"],
...
function validateWeb(config: ControlConfig): void {
    if (
        !Number.isSafeInteger(config.web.listenPort) ||
        config.web.listenPort < 0 ||
        config.web.listenPort > 65535
    ) {
        throw configInputError(
            "semantic",
            ["web", "listenPort"],
    ...
    if (auth.mode === "oauth2" && auth.oauth2.documentationUrl !== undefined) {
        parseUrl(auth.oauth2.documentationUrl, [
            "web",
            "auth",
            "oauth2",
            "documentationUrl",
...
function validateMcpAuth(
    ...
    if (
        instance.mcp.contextMode === "openai-session" &&
        instance.mcp.auth.mode === "token"
    ) {
        throw configInputError(
            "semantic",
    ...
    if (instance.mcp.auth.mode === "oauth2") {
        ...
        if (oauth2.documentationUrl !== undefined) {
            parseUrl(oauth2.documentationUrl, [
                ...base,
                "mcp",
                "auth",
                "oauth2",
    ...
    if (
        instance.mcp.auth.mode === "token" &&
        Buffer.byteLength(instance.mcp.auth.token, "utf8") < 32
    ) {
        throw configInputError(
            "semantic",
            [...base, "mcp", "auth", "token"],
...
function validateLogs(
    ...
    if (
        logs.maxBytes !== undefined &&
        (!Number.isSafeInteger(logs.maxBytes) ||
            logs.maxBytes < minimumAuditStorageBytes)
    ) {
        throw configInputError(
            "semantic",
            [...base, "logs", "maxBytes"],
...
function validateScheduler(
    scheduler: ControlToolSchedulerConfig | undefined,
    base: readonly (string | number)[],
): void {
    if (scheduler === undefined) return;
    positiveInteger(scheduler.maxRunning, [
        ...base,
        "tools",
        "scheduler",
        "maxRunning",
    ]);
    nonNegativeInteger(scheduler.queueDepth, [
        ...base,
        "tools",
        "scheduler",
        "queueDepth",
    ]);
    positiveInteger(scheduler.queueTimeoutMs, [
        ...base,
        "tools",
        "scheduler",
        "queueTimeoutMs",
    ]);
    positiveInteger(scheduler.maxRunningPerSession, [
        ...base,
        "tools",
        "scheduler",
        "maxRunningPerSession",
    ]);
    nonNegativeInteger(scheduler.queueDepthPerSession, [
        ...base,
        "tools",
        "scheduler",
    ...
    for (const [toolName, limits] of Object.entries(scheduler.byTool ?? {})) {
        positiveInteger(limits.maxRunning, [
            ...base,
            "tools",
            "scheduler",
            "byTool",
        ...
        nonNegativeInteger(limits.queueDepth, [
            ...base,
            "tools",
            "scheduler",
            "byTool",
...
function validateContainer(
    ...
    for (const [index, mount] of ("mounts" in container
        ? (container.mounts ?? [])
        : []
    ).entries()) {
        if (mount.source.length === 0) {
            throw configInputError(
                "semantic",
                [...base, "container", "mounts", index, "source"],
                "config.mount.source",
                "must not be empty",
            );
        }
        if (mount.target.length === 0) {
            throw configInputError(
                "semantic",
                [...base, "container", "mounts", index, "target"],
                "config.mount.target",
                "must not be empty",
            );
        }
