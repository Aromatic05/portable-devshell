...
export class PrefixRoute {
    ...
    readonly #eventListeners = new Set<
        (incoming: PrefixRouteIncoming) => void
    >();
    ...
    static snapshot(
        definitions: readonly PrefixRouteDestinationDefinition[],
    ): PrefixRouteSnapshot {
        ...
        for (const destinationDefinition of definitions) {
            if (destinations.has(destinationDefinition.destination)) {
                throw new Error(
                    `Duplicate route destination: ${destinationDefinition.destination}`,
                );
            }
            ...
            for (const moduleDefinition of destinationDefinition.modules) {
                ...
                if (modules.has(moduleDefinition.name)) {
                    throw new Error(
                        `Duplicate route module: ${destinationDefinition.destination}/${moduleDefinition.name}`,
                    );
                }
    ...
    async #accept(event: Event): Promise<void> {
        ...
        if (event.replyTo !== undefined) {
            throw protocolFailure(
                "Clients must not send reply events to the server route.",
            );
        }
    ...
    async #acceptStream(incoming: PrefixRouteIncoming): Promise<void> {
        ...
        if (incoming.module === "stream" && incoming.event.name === "cancel") {
            ...
            if (incoming.destination !== active.destination) {
                throw protocolFailure(
                    `Stream ${streamId} was addressed to the wrong destination.`,
                );
            }
        ...
        if (
            incoming.destination !== active.destination ||
            incoming.module !== active.module
        ) {
            throw protocolFailure(
                `Stream ${streamId} was addressed to the wrong route.`,
            );
        }
        ...
        const context: PrefixRouteContext = {
            ...
            openStream: async (initialPayload, options = {}) => {
                if (
                    ...
                    this.#serverStreams.has(incoming.event.id)
                ) {
                    throw new Error(
                        `Stream ${incoming.event.id} is already open.`,
                    );
                }
            ...
            const result = await handler(
                {
                    ...
                    name: incoming.event.name,
                    ...(incoming.event.payload === undefined
                        ? {}
                        : { payload: incoming.event.payload }),
                    ...(incoming.event.seq === undefined
                        ? {}
                        : { seq: incoming.event.seq }),
                },
                context,
            );
    ...
    #cancelServerRequest(incoming: PrefixRouteIncoming): void {
        ...
        if (active.destination !== incoming.destination) {
            throw protocolFailure(
                `Request ${payload.requestId} was addressed to the wrong destination.`,
            );
        }
    ...
    #createServerStream(active: ActiveServerStream): PrefixRouteStream {
        ...
        return {
            id: active.id,
            cancel: async (error) =>
                await finish("cancelled", undefined, error),
    ...
    #nextId(): string {
        return `${this.#eventIdPrefix}-${createTransportId()}`;
    }
}
