...
test("Workspace switch defaults enabled without restoring persistent workspace path authority", () => {
    ...
    assert.equal(
        normalizeConfigDraft(parsed).instances[0]?.workspace.enabled,
        false,
    );
    const patched = applyConfigInstancePatch(instance, {
        workspace: { enabled: false },
    });
    assert.equal(
        normalizeConfigInstanceDraft(patched).workspace.enabled,
        false,
    );
    assertConfigIssue(
        () =>
