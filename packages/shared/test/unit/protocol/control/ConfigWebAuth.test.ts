...
test("web token mode normalizes into a structured auth config", () => {
    const global = normalizeConfigGlobalDraft(
        parseConfigGlobalDraft({
            web: { auth: "token", token: ` ${strongToken} ` },
        }),
    );
...
test("web oauth2 block rejects unknown residual fields", () => {
    assert.throws(() =>
        parseConfigGlobalDraft({
            web: {
                auth: "oauth2",
                oauth2: { clientSecret: "x", resourceName: "web" },
            },
        }),
    );
});
...
test("config view exposes a flat web auth draft that masks the token", () => {
    ...
    const reparsed = normalizeConfigGlobalDraft(
        parseConfigGlobalDraft({ web: view.web }),
    );
    assert.deepEqual(reparsed.web.auth, {
        mode: "token",
        token: MASKED_CONFIG_TOKEN,
    });
});
...
test("web auth patch preserves the current token when the masked placeholder is submitted", () => {
    ...
    const unchanged = applyConfigWebPatch(config.web, {
        auth: "token",
        token: MASKED_CONFIG_TOKEN,
    });
    const normalized = normalizeConfigGlobalDraft({ web: unchanged });
    assert.deepEqual(normalized.web.auth, {
        mode: "token",
        token: strongToken,
    });

    const rotated = applyConfigWebPatch(config.web, {
        auth: "token",
        token: "b".repeat(48),
    });
