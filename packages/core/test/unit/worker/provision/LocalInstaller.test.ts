...
test(
    "WorkerInstallerLocal returns an executable pinned to the installed asset",
    {
        skip:
            process.platform === "win32"
                ? "requires Unix executable semantics"
                : false,
    },
    async (t) => {
        ...
        const executable = await installer.ensure(
            ...
            createAsset(binaryPath, sha256, target),
            target,
        );
...
test(
    "WorkerInstallerLocal pins each concurrent Unix install to its requested asset",
    {
        skip:
            process.platform === "win32"
                ? "Unix aliases are not used on Windows"
                : false,
    },
    async (t) => {
        const devshellHomeDirectory = await createTestTempDirectory(
            "home-concurrent-assets",
        );
        const workerDirectory = await createTestTempDirectory(
            "worker-concurrent-assets",
        );
        t.after(async () => {
            await rm(devshellHomeDirectory, { recursive: true, force: true });
            await rm(workerDirectory, { recursive: true, force: true });
        ...
        assert.equal(
            await readFile(firstExecutable, "utf8"),
            first.toString("utf8"),
        );
        assert.equal(
            await readFile(secondExecutable, "utf8"),
...
test(
    "WorkerInstallerLocal prunes stale worker generations only after successful activation",
    {
        skip:
            process.platform === "win32"
                ? "requires Unix symlink semantics"
                : false,
    },
    async (t) => {
        const devshellHomeDirectory =
            await createTestTempDirectory("home-generation-gc");
        const workerDirectory = await createTestTempDirectory(
            "worker-generation-gc",
        );
        t.after(async () => {
            await rm(devshellHomeDirectory, { recursive: true, force: true });
            await rm(workerDirectory, { recursive: true, force: true });
        ...
        const nextSha256 = createHash("sha256")
            .update(nextContents)
            .digest("hex");
        await writeFile(nextPath, nextContents, { mode: 0o755 });
        await installer.ensure(
            devshellHomeDirectory,
            createAsset(nextPath, nextSha256, target),
            target,
        );
        await assert.doesNotReject(access(currentGeneration));
    },
...
test(
    "WorkerInstallerLocal repairs corrupted content and preserves the old alias when activation fails",
    {
        skip:
            process.platform === "win32"
                ? "requires Unix symlink semantics"
    ...
    async (t) => {
        const devshellHomeDirectory =
            await createTestTempDirectory("home-repair");
...
test("WorkerInstallerLocal rejects a bundle whose declared checksum does not match its bytes", async (t) => {
    const devshellHomeDirectory =
        await createTestTempDirectory("home-mismatch");
    ...
    await assert.rejects(
        ...
        (error: unknown) =>
            (error as { code?: string }).code === "core.workerProvisionFailed",
    );
});
...
test("WorkerInstallerLocal preserves the active Windows alias when replacement activation fails", async (t) => {
    const devshellHomeDirectory = await createTestTempDirectory(
        "home-windows-activation",
    );
    const workerDirectory = await createTestTempDirectory(
        "worker-windows-activation",
    );
    t.after(async () => {
        await rm(devshellHomeDirectory, { recursive: true, force: true });
        await rm(workerDirectory, { recursive: true, force: true });
    ...
    await installer.ensure(
        devshellHomeDirectory,
        createAsset(binaryPath, firstSha, target),
        target,
    );
    const alias = join(
        devshellHomeDirectory,
        ...
        fileSystem: {
            rename: async (source, destination) => {
                if (
                    ...
                    throw new Error(
                        "injected Windows alias activation failure",
                    );
                }
                await rename(source, destination);
            },
    ...
    await assert.rejects(
        failing.ensure(
            devshellHomeDirectory,
            createAsset(binaryPath, secondSha, target),
            target,
        ),
...
test("WorkerInstallerLocal rejects asset target mismatch", async (t) => {
    ...
    await assert.rejects(
        installer.ensure(
            devshellHomeDirectory,
            createAsset(binaryPath, sha256, assetTarget),
            requestedTarget,
        ),
        (error: unknown) => {
