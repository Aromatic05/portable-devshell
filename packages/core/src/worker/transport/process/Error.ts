...
import type { ProviderCommandContext } from "../command/Transport.js";
...
function buildCommandDiagnostic(
    ...
    return {
        ...(readCauseCode(cause) === undefined
            ? {}
            : { causeCode: readCauseCode(cause) }),
        ...(readCauseMessage(cause) === undefined
            ? {}
            : { causeMessage: readCauseMessage(cause) }),
