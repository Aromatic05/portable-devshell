...
import { WorkerAssetResolver } from "../../provision/AssetResolver.js";
...
} from "../../provision/install/LocalInstaller.js";
import { resolveWorkerDevshellHomeDirectory } from "../../provision/HomeDirectory.js";
import { workerInstalledAliasFileName } from "../../provision/target/Target.js";
import { probeLocalWorkerTarget } from "../../provision/target/Probe.js";
...
export class WorkerTransportDriverLocal implements WorkerTransport {
    ...
    async installWorker(): Promise<void> {
        ...
        const context = this.#createCommandContext("installWorker", [
    ...
    async runWorkerCommand(
        ...
        if (
            command === "retire" &&
            this.#workerBinary.executable === "devshell-worker"
        ) {
            const installed = await this.#provisionExecutable(options.env);
            return await this.#runResolvedCommand(
        ...
        if (
            ...
            const installed = await this.#provisionExecutable(options.env);
            const statusResult = await this.#runResolvedCommand(
                installed.executablePath,
                "status",
                {
            ...
            const daemon = readDaemonIdentity(
                statusResult.exitCode,
                statusResult.stdout,
            );
            if (
                !daemon.known ||
                (daemon.running && daemon.workerSha256 !== installed.sha256)
            ) {
                const stopResult = await this.#runResolvedCommand(
                    installed.executablePath,
                    "stop",
                    {
                        instanceName: options.instanceName,
                ...
                if (stopResult.exitCode !== 0) {
                    throw this.#process.createError(
                        ...
                        new Error(
                            stopResult.stderr ||
                                stopResult.stdout ||
                                "existing worker could not be stopped",
                        ),
                        {
                            errorCode: errorCodes.coreWorkerProvisionFailed,
                            result: stopResult,
    ...
    async #provisionExecutable(
        env?: NodeJS.ProcessEnv,
    ): Promise<WorkerInstallerLocalResult> {
        if (this.#workerBinary.executable !== "devshell-worker") {
            return {
                executablePath: this.#workerBinary.executable,
                sha256: "",
            };
        }
        ...
        const target = probeLocalWorkerTarget("local", "resolveExecutable");
        const asset = await this.#resolver
            ...
            .catch((error) => {
                ...
                throw this.#process.createError(
                    this.#createCommandContext("resolveExecutable", [
                        "devshell-worker",
                    ]),
                    error,
                );
        ...
        return await this.#installer
            .ensureInstalled(devshellHomeDirectory, asset, target)
            .catch((error) => {
                throw this.#process.createError(
                    this.#createCommandContext("resolveExecutable", [
                        "devshell-worker",
...
function readDaemonIdentity(
    ...
    try {
        ...
        return {
            ...
            workerSha256:
                typeof parsed.workerSha256 === "string"
                    ? parsed.workerSha256
                    : undefined,
        };
    } catch {
