...
export function readErrorMessage(error: unknown): string {
    if (typeof error === "object" && error !== null && "details" in error) {
        ...
        if (
            ...
            const causeMessage = (details as Record<string, unknown>)
                .causeMessage;
...
export function withInstanceDetails(
    details: CommandDiagnostic | undefined,
    instance: string,
): CommandDiagnostic {
    return {
        ...(details ?? {}),
...
export function wrapWorkerCommandError(
    error: unknown,
    code: string,
    message: string,
    instance: string,
    ...
    return createError({
        ...
        details: toJsonDetails(
            withInstanceDetails(
                readCommandDiagnostic((error as { details?: unknown }).details),
                instance,
            ),
        ),
    });
...
export function readCommandDiagnostic(
    value: unknown,
    ...
    return {
        ...
        ...(Array.isArray(candidate.command)
            ? {
                  command: candidate.command.filter(
                      (entry): entry is string => typeof entry === "string",
                  ),
              }
            : {}),
        ...(typeof candidate.commandDisplay === "string"
        ...
        ...(typeof candidate.stderrTail === "string"
            ...
            : {}),
        ...(typeof candidate.stdoutTail === "string"
            ? { stdoutTail: candidate.stdoutTail }
            : {}),
    };
}
