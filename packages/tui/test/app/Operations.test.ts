...
import {
    type TuiRuntimeOperationClients,
    type TuiRuntimeOperationSession,
} from "../../src/app/Operations.ts";

{
    function createHarness(
        ...
        const initialHomeDirectory =
            options.initialHomeDirectory === undefined
                ? "/home/alpha"
        ...
        const session: TuiRuntimeOperationSession = {
            commands: {
                ...
                async decideToolApproval(
                    ...
                    calls.push(
                        `approval.${decision}:${instance}:${approvalId}`,
                    );
                },
            ...
            async reconnect() {
                ...
                if (
                    reconnectAttempts <= (options.restartReconnectFailures ?? 0)
                ) {
            ...
            async refreshInstances() {
                ...
                if (options.startedHomeDirectory !== undefined) {
                    store.patchControlReadModel({
                        instances: store.getState().instances.map((instance) =>
                            instance.name === "alpha"
                                ? {
                                      ...instance,
                                      homeDirectory:
                                          options.startedHomeDirectory,
                                  }
                                : instance,
                        ),
                    });
    ...
    test("starting a previously stopped instance refreshes worker home before direct tool calls", async () => {
        ...
        assert.equal(
            harness.calls.includes(
                'tool.call:alpha:/home/started-alpha:bash_run:{"command":"pwd"}',
            ),
            true,
        );
    });
    ...
    test("reverse instance creation reports committed creation when device code generation fails", async () => {
        ...
        assert.match(
            status ?? "",
            /devshell instance device-code reverse-one/u,
        );
        assert.equal(
            harness.store.getState().panelErrors[
                "instances:reverse-one:enrollment"
            ]?.message,
    ...
    test("runtime operations expose control callbacks and route page refreshes", async () => {
        ...
        await harness.operations.restartControl();
        await harness.operations.decideApproval(
            "alpha",
            "approval-1",
            "approve",
        );
        ...
        assert.equal(
            harness.calls.includes("instance.create:remote-one"),
            true,
        );
        ...
        assert.equal(
            harness.calls.includes(
                'tool.call:alpha:/home/alpha:bash_run:{"command":"pwd"}',
            ),
            true,
        );
        assert.deepEqual(harness.refreshed, [
