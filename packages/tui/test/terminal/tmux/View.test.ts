...

{
    ...
    function visibleText(view: { visibleLines: TuiTerminalLine[] }): string[] {
        return view.visibleLines.map((line) =>
    ...
    test("Attach forwards ordinary input to tmux_input using caret notation", () => {
        assert.deepEqual(routeTmuxAttachInput("hello"), {
            input: "hello",
            kind: "send",
        });
        assert.deepEqual(routeTmuxAttachInput("\r"), {
            input: "^M",
            kind: "send",
        });
    ...
    test("Attach escapes literal carets and preserves escape sequences for tmux_input", () => {
        ...
        assert.deepEqual(routeTmuxAttachInput("\u001b[A"), {
            input: "^[[A",
            kind: "send",
        });
    });
    ...
    test("list browsing maps arrows and vi keys to selection, Enter to activate, Esc to close", () => {
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b[B", "list"), {
            direction: "next",
            kind: "select",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("j", "list"), {
            direction: "next",
            kind: "select",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b[A", "list"), {
            direction: "previous",
            kind: "select",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("k", "list"), {
            direction: "previous",
            kind: "select",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\r", "list"), {
            kind: "activate",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b", "list"), {
            kind: "close",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("x", "list"), {
            kind: "noop",
        });
    });

    test("view browsing scrolls vertically, switches panes horizontally, Enter attaches, and Esc closes", () => {
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b[B", "view"), {
            delta: 1,
            kind: "scroll",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("j", "view"), {
            delta: 1,
            kind: "scroll",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b[A", "view"), {
            delta: -1,
            kind: "scroll",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("k", "view"), {
            delta: -1,
            kind: "scroll",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b[D", "view"), {
            direction: "previous",
            kind: "select",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b[C", "view"), {
            direction: "next",
            kind: "select",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\u001b", "view"), {
            kind: "close",
        });
        assert.deepEqual(routeTmuxPaneBrowseInput("\r", "view"), {
            kind: "activate",
        });
    });
}
