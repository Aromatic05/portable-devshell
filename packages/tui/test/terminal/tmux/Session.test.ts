...
import {
    type TuiTmuxInspectPane,
    type TuiTmuxInputResult,
} from "../../../src/app/operation/Tmux.ts";
import {
    TuiTmuxPaneTerminalSession,
    type TuiTmuxPaneTerminalOperations,
    type TuiTmuxPaneTerminalScheduler,
} from "../../../src/terminal/tmux/Session.ts";
import {
    parseTmuxInspectLines,
...

{
    ...
    type PaneFixture = Omit<TuiTmuxListPane, "workspace"> & {
        workspace?: string;
    };
    type InspectFixture = Omit<TuiTmuxInspectPane, "workspace"> & {
        workspace?: string;
    };
        ...
        const operations: TuiTmuxPaneTerminalOperations = {
            ...
            async listPanes(instance) {
                ...
                return panes.map((pane) => ({
                    ...pane,
                    workspace: pane.workspace ?? testWorkspace,
                }));
            },
            async sendInput(instance, workspace, task, input) {
                calls.push({
                    args: [instance, workspace, task, input],
                    method: "sendInput",
                });
                return (
                    inputResponses.shift() ?? {
                        output: [],
                        status: "running",
                        task,
                    }
    ...
    function activeLines(session: TuiTmuxPaneTerminalSession): string[] {
        return (session.getSnapshot().active?.lines ?? []).map((line) =>
            line.segments
                .map((segment) => segment.text)
                .join("")
                .trimEnd(),
    ...
    test("refresh preserves the selected pane identity when pane ordering changes", async () => {
        ...
        assert.equal(
            session.getSnapshot().panes[session.getSnapshot().selectedIndex]
                ?.id,
            "%2",
        );
    ...
    test("activating a view-only pane opens an unattached panel without a warning", async () => {
        ...
        assert.equal(
            harness.calls.some(
                (call) =>
                    call.method === "inspectPane" && call.args[2] === "%0",
            ),
            true,
        );
    });
    ...
    test("component viewport updates recompute the exact visible pane rows", async () => {
        ...
        await session.bind("alpha");
        await session.activate();
        assert.deepEqual(viewLines(session), [
            "one",
            "two",
            "three",
            "four",
    ...
    test("attached input is encoded and forwarded to the selected running task id", async () => {
        ...
        assert.deepEqual(
            ...
            {
                args: ["alpha", testWorkspace, "task-9", "hello^M"],
                method: "sendInput",
            },
        );
        const active = session.getSnapshot().active;
    ...
    test("Esc exits Attach without forwarding to tmux_input", async () => {
        ...
        assert.equal(
            harness.calls.some((call) => call.method === "sendInput"),
            false,
        );
        const active = session.getSnapshot().active;
        assert.equal(active?.attached, false);
    ...
    test("polling is bounded: it refreshes per tick and stops after maxTicks", async () => {
        ...
        await session.bind("alpha");
        const listCallsAfterBind = harness.calls.filter(
            (call) => call.method === "listPanes",
        ).length;
        ...
        const listCallsAfterPolling = harness.calls.filter(
            (call) => call.method === "listPanes",
        ).length;
        ...
        await flush();
        assert.equal(
            harness.calls.filter((call) => call.method === "listPanes").length,
            listCallsAfterPolling,
        );
    });
    ...
    test("polling never reopens a pane view after Esc returns to the full-width pane list", async () => {
        ...
        assert.equal(session.getSnapshot().active, undefined);
        assert.equal(
            harness.calls.filter((call) => call.method === "inspectPane")
                .length,
            1,
        );
    });
    ...
    test("handleRawInput in attach mode forwards input and exits on Esc without forwarding", async () => {
        ...
        assert.deepEqual(
            harness.calls.find((call) => call.method === "sendInput"),
            {
                args: ["alpha", testWorkspace, "task-9", "hi^M"],
                method: "sendInput",
            },
        );
        ...
        assert.equal(
            harness.calls.filter((call) => call.method === "sendInput").length,
            1,
        );
    });
    ...
    test("session inspects the selected pane by stable id when pane names are duplicated", async () => {
        ...
        assert.deepEqual(
            harness.calls
                .filter((call) => call.method === "inspectPane")
                .at(-1),
    ...
    test("a stale polling inspect cannot overwrite output returned by tmux_input", async () => {
        let inspectCount = 0;
        let resolveStaleInspect:
            ((detail: TuiTmuxInspectPane | undefined) => void) | undefined;
        const operations: TuiTmuxPaneTerminalOperations = {
            async inspectPane() {
                inspectCount += 1;
                if (inspectCount <= 2) {
                ...
                return await new Promise<TuiTmuxInspectPane | undefined>(
                    (resolve) => {
    ...
    function flatSegments(lines: TuiTerminalLine[]): Array<{
    ...
    test("parseTmuxInspectLines keeps SGR red, green, and bold as styled segments", async () => {
        const lines = await parseTmuxInspectLines([
    ...
    test("parseTmuxInspectLines keeps background color segments", async () => {
        ...
        const alert = flatSegments(lines).find(
            (segment) => segment.text === "alert",
        );
