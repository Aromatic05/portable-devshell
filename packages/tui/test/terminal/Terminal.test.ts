...

{
    ...
    test("terminal viewport region exists only for the Instances terminal tab", () => {
        ...
        assert.ok(
            buildTuiTerminalViewportRegion(store.getState(), {
                columns: 120,
                rows: 40,
            }),
        );
    ...
    test("terminal buffer follows new output at the bottom but holds position after scrolling up", async () => {
        ...
        assert.equal(terminal.getSnapshot().scroll.atBottom, true);
        assert.equal(
            lineText(terminal.getSnapshot().lines[2]!).slice(0, 4),
            "five",
        );
        assert.notEqual(
            lineText(terminal.getSnapshot().lines[2]!),
            bottomLastLine,
        );
        ...
        assert.equal(terminal.getSnapshot().scroll.atBottom, true);
        assert.equal(
            lineText(terminal.getSnapshot().lines[2]!).slice(0, 5),
            "seven",
        );
        terminal.dispose();
    });
    ...
    test("terminal buffer forwards SGR and legacy mouse reports only when requested", async () => {
        ...
        assert.equal(
            writes.at(-1),
            `\u001B[M${String.fromCharCode(32, 34, 35)}`,
        );
        terminal.dispose();
    });
    ...
    test("terminal input router emits source toggle and flushes a standalone Esc without breaking split CSI", () => {
        ...
        assert.deepEqual(router.push("[B"), [
            { data: "\u001B[B", type: "data" },
        ]);
    ...
    test("terminal graphics renderer detects support and positions native protocol output", () => {
        ...
        assert.equal(
            frame.includes("\u001B[10;32H\u001B_Ga=T;AAAA\u001B\\"),
            true,
        );
    ...
    test("terminal image renderer emits Kitty PNG and iTerm2 fallback frames", () => {
        ...
        assert.match(
            unsupported.reason ?? "",
            /does not advertise Kitty or iTerm2/u,
        );
