...
import { projectTodoSummaries } from "./Goal.js";
import { currentTuiRoute } from "../../../state/route/State.js";
import { buildTodoDetailBoxes } from "./Detail.js";
...
export function buildTodoGoalBoxes(
    state: TuiAppState,
    instance: string,
): BoxModel[] {
...
function goalBox(
    ...
    return makeBox(state, "todo", instance, {
        ...
        summaryLines: [
            compactSummary(
                ["progress", `${completed}/${goal.steps.length}`],
                ["status", goal.status],
                ["workspace", workspaceFolderName(goal.workspace)],
            ),
        ],
...
export function buildTodoOverviewBoxes(
    state: TuiAppState,
    instance: string,
): BoxModel[] {
    ...
    return [
        ...
        ...(activeTaskId === undefined || todo === undefined
            ? []
            : todo.items.map((item) =>
                  subTaskBox(state, instance, activeTaskId, item),
              )),
    ];
}
...
export function buildTodoPageBoxes(
    ...
    return [
        ...buildTodoGoalBoxes(state, instanceName),
        ...(route.view === "overview"
