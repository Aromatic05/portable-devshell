...
export function buildTodoDetailBoxes(
    state: TuiAppState,
    instance: string,
    todoId: string,
): BoxModel[] {
    const todo = state.readModel.instanceState[instance]?.todo;
    const summary = projectTodoSummaries(todo).find(
        (candidate) => candidate.taskId === todoId,
    );
...
function summaryBox(
    ...
    const current =
        todo.summary.currentItemId === undefined
            ? undefined
            : todo.items.find((item) => item.id === todo.summary.currentItemId);
