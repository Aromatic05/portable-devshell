...
import {
    buildSelectedInstancePageContext,
    compactSummary,
    makeBox,
} from "../Support.js";
import {
...
export function buildConfigPageBoxes(
    state: TuiAppState,
    instanceName: string,
): BoxModel[] {
    const { instance, snapshot } = buildSelectedInstancePageContext(
        state,
        instanceName,
    );
    const fallback = comparableInstanceDraft(
        instanceDraft(state, instanceName),
    );
    ...
    return [
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...
                ...editorErrorLine(state, "config", "configuration", [
                    "enabled",
                    "provider",
                ]),
            ],
            ...
            status: configStatus(
                state,
                ["enabled", "provider"],
                instance?.enabled === false ? "disabled" : "normal",
            ),
            summaryLines: [
                compactSummary([
                    "provider",
                    stringValue(readPath(draft, "provider"), "unknown"),
                ]),
            ],
            title: "General",
        }),
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...providerLines(draft),
                ...editorErrorLine(state, "config", "provider", [
                    "provider",
                    "ssh",
                    "container",
                    "dockerBinary",
        ...
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...
                {
                    id: "mcp-path",
                    text: `mcp.path           ${stringValue(readPath(draft, "mcp.path"), `/${instanceName}/mcp`)} (fixed)`,
                },
            ...
            summaryLines: [
                compactSummary(
                    [
                        "enabled",
                        stringValue(readPath(draft, "mcp.enabled"), "false"),
                    ],
                    [
                        "context",
                        stringValue(
        ...
        makeBox(state, "config", instanceName, {
            detailLines: [
                choiceLine(
                    "workspace.enabled",
                    "workspace.enabled",
                    readPath(draft, "workspace.enabled"),
                ),
            ...
            status: configStatus(
                state,
                ["workspace"],
                readPath(draft, "workspace.enabled") === false
                    ? "disabled"
                    : "normal",
            ),
            summaryLines: [
        ...
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...
                ...editorErrorLine(state, "config", "model-extensions", [
                    "extensions",
                ]),
            ],
            id: "model-extensions",
            ...
            summaryLines: [
                compactSummary([
                    "allowed",
                    stringValue(readPath(draft, "extensions.model"), "none"),
                ]),
            ],
        ...
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...
                ...editorErrorLine(state, "config", "security", [
                    "security",
                    "approvalPolicy",
                ]),
            ],
        ...
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...
                ...editorErrorLine(state, "config", "tool-runtime", [
                    "tools",
                    "scheduler",
                ]),
            ],
        ...
        makeBox(state, "config", instanceName, {
            detailLines: [
                ...
                ...(dirty
                    ...
                    : [
                          "",
                          {
                              ...
                              text: "No pending changes. Hot apply is available.",
                ...
                buttonLine(
                    "save-restart",
                    "Save & Restart",
                    !running || !restartRequired || selfManaged,
                ),
            ...
            status:
                state.interaction.editor?.kind === "config" &&
                state.interaction.editor.error !== undefined
                    ...
                    : dirty
                      ...
                      : restartRequired
                        ...
                        : "ready",
            summaryLines: [
                compactSummary(["apply", restartRequired ? "restart" : "hot"]),
            ],
...
function configStatus(
    ...
    fallback: "disabled" | "normal",
): "disabled" | "failed" | "normal" {
    const error =
        state.interaction.editor?.kind === "config"
            ? state.interaction.editor.error
            : undefined;
    return error !== undefined && fields.some((field) => error.includes(field))
        ? "failed"
        : fallback;
}
...
): "disabled" | "failed" | "normal" | "ready" | "warning" {
    const validation = configStatus(
        state,
...
function providerLines(
    ...
    if (provider !== "docker" && provider !== "podman") {
        return ["No provider-specific settings."].map((text, index) => ({
            id: `provider-info:${index}`,
            text,
        }));
    }
...
function managedContainerLines(
    draft: Record<string, JsonValue>,
): Array<{ id: string; text: string }> {
    return [
        fieldLine(
...
function comparableInstanceDraft(
