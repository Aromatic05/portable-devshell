...
export function buildAuditCallBoxes(state: TuiAppState, instance: string, ctxId: string, callId: string): BoxModel[] {
    const call = (state.toolCallsByInstance[instance] ?? []).find((candidate) => candidate.callId === callId && candidate.ctxId === ctxId);
    ...
    return [makeBox(state, "audit", instance, {
        ...
        summaryLines: [`${call.startedAt} → ${call.completedAt ?? "running"}`],
