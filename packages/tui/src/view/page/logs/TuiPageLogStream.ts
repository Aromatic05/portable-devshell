...
export function buildLogContextBoxes(state: TuiAppState, instance: string, ctxId: string): BoxModel[] {
    ...
    const unseen = following || pausedAt === undefined ? 0 : sourceEntries.filter((entry) => entry.seq > pausedAt).length;
    const stderr = sourceEntries.filter((entry) => entry.stream === "stderr").length;
    ...
    return [
        makeBox(state, "logs", instance, {
            detailLines: [
                ...
                buttonLine("clear-buffer", "Clear Buffer", sourceEntries.length === 0)
            ],
            id: "logs-controls",
        ...
        makeBox(state, "logs", instance, {
            detailLines: filtered.length === 0
                ? [query.length === 0 ? "No logs loaded for this source." : `No logs match filter: ${query}`]
                : filtered.map((entry) => ({
                      id: `log:${entry.seq}`,
