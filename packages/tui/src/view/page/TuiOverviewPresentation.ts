...
export function selectTuiOverviewPresentation(state: TuiAppState): TuiOverviewPresentation {
    ...
    for (const todo of overview.todos) {
        ...
        activeTodoCountByInstance.set(todo.instance, (activeTodoCountByInstance.get(todo.instance) ?? 0) + 1);
    }

    const instances = overview.instances
        .map((instance) => toInstanceRow(
            instance,
            activeTodoCountByInstance.get(instance.name) ?? 0,
            state.ui.mainFocusId === `overview-instance:${instance.name}`
        ))
        .filter((row) => query.length === 0 || instanceSearchText(row).includes(query))
        .sort((left, right) => {
    ...
    return {
        ...
        alerts: [...overview.alerts]
            .sort((left, right) => {
                const severity = left.severity === right.severity ? 0 : left.severity === "critical" ? -1 : 1;
...
export function selectTuiOverviewFocusIds(state: TuiAppState): string[] {
    return selectTuiOverviewPresentation(state).instances.map((instance) => instance.id);
}

export function selectTuiOverviewInstanceName(focusId: string | undefined): string | undefined {
    const prefix = "overview-instance:";
    return focusId?.startsWith(prefix) ? focusId.slice(prefix.length) : undefined;
}
...
function instanceTone(instance: OperationalOverviewInstance): TuiOverviewTone {
    ...
    if (snapshot.status === "running" || snapshot.daemonState === "running" || snapshot.connectionState === "connecting") {
        return "accent";
    }
    return "warning";
}

function toActivityRow(activity: OperationalOverviewActivity): TuiOverviewActivityRowModel {
    return {
...
function activityTone(status: OperationalOverviewActivity["status"]): TuiOverviewTone {
    if (status === "failed" || status === "denied" || status === "queueTimeout") return "danger";
...
function systemMeters(overview: OperationalOverview): TuiOverviewMeterModel[] {
    ...
    const memoryUsed = Math.max(0, system.memoryTotalBytes - system.memoryAvailableBytes);
    ...
    if (system.diskTotalBytes !== undefined && system.diskAvailableBytes !== undefined) {
        const diskUsed = Math.max(0, system.diskTotalBytes - system.diskAvailableBytes);
        meters.push({
