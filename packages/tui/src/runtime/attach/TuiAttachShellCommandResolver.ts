...
import {
    ...
    type TuiAttachShellCommand,
    type TuiAttachShellProvider,
    type TuiAttachShellResolutionInput
...
export class TuiAttachShellCommandResolver {
    ...
    #local(input: TuiAttachShellResolutionInput): TuiAttachShellCommand {
    ...
    #ssh(configured: Record<string, unknown> | undefined): TuiAttachShellCommand {
        ...
        if (argv.length === 0) {
            throw new TuiAttachShellResolutionError("Attach Shell requires configured ssh.command.");
        }
    ...
    #container(
        ...
        input: TuiAttachShellResolutionInput
    ): TuiAttachShellCommand {
        if (input.snapshot?.daemonState !== "running") {
            throw new TuiAttachShellResolutionError("Container is not running. Use Start Worker first.");
        }
        ...
        if (container?.mode === "compose") {
            ...
            if (compose === undefined || service === undefined) {
                throw new TuiAttachShellResolutionError("Attach Shell requires configured compose service.");
            }
        ...
        if (name === undefined) {
            throw new TuiAttachShellResolutionError("Attach Shell requires configured container identity.");
        }
...
function readInstanceConfig(configView: TuiAttachShellResolutionInput["configView"], name: string): Record<string, unknown> | undefined {
...
function readProvider(configured: Record<string, unknown> | undefined, fallback: string | undefined): TuiAttachShellProvider {
    ...
    throw new TuiAttachShellResolutionError("Attach Shell requires provider information from control.");
}
...
function composePrefix(binary: string, compose: Record<string, unknown>): string[] {
    ...
    if (file === undefined) {
        throw new TuiAttachShellResolutionError("Attach Shell requires configured compose project/service.");
    }

    return [binary, "compose", "--file", file, ...(projectName === undefined ? [] : ["--project-name", projectName]), "exec"];
}

function withShellFallback(prefix: string[], target: string, readinessCheck: TuiAttachShellCommand["readinessCheck"]): TuiAttachShellCommand {
    return {
