...
export function tuiRouteContextKey(page: TuiPageId, instance: string | undefined): string {
    return `${page}${CONTEXT_SEPARATOR}${instance ?? "-"}`;
}

export function tuiRouteViewKey(page: TuiPageId, instance: string | undefined, route: TuiRoute): string {
...
export function currentTuiRouteStack(state: TuiAppState): readonly TuiRoute[] {
    const key = tuiRouteContextKey(state.ui.selectedPage, state.ui.selectedInstance);
    return state.ui.routeStacks[key] ?? [rootTuiRoute(state.ui.selectedPage)];
}

export function currentTuiRouteViewState(state: TuiAppState): TuiRouteViewState {
    const route = currentTuiRoute(state);
    return state.ui.routeViewStates[tuiRouteViewKey(state.ui.selectedPage, state.ui.selectedInstance, route)] ?? defaultTuiRouteViewState();
}
...
export function pushTuiRoute(state: TuiAppState, route: TuiRoute): TuiAppState {
    ...
    const saved = persistCurrentTuiRouteView(state);
    const key = tuiRouteContextKey(saved.ui.selectedPage, saved.ui.selectedInstance);
    const stack = saved.ui.routeStacks[key] ?? [rootTuiRoute(saved.ui.selectedPage)];
    return restoreCurrentTuiRouteView({
        ...saved,
        ui: {
            ...saved.ui,
            routeStacks: { ...saved.ui.routeStacks, [key]: [...stack, route] }
        }
    }, true);
}
...
export function replaceTuiRoute(state: TuiAppState, route: TuiRoute): TuiAppState {
    assertRoutePage(state, route);
    const saved = persistCurrentTuiRouteView(state);
    const key = tuiRouteContextKey(saved.ui.selectedPage, saved.ui.selectedInstance);
    const stack = saved.ui.routeStacks[key] ?? [rootTuiRoute(saved.ui.selectedPage)];
    return restoreCurrentTuiRouteView({
        ...
        ui: {
            ...saved.ui,
            routeStacks: { ...saved.ui.routeStacks, [key]: [...stack.slice(0, -1), route] }
        }
    }, true);
}
...
export function resetTuiRoute(state: TuiAppState): TuiAppState {
    ...
    return restoreCurrentTuiRouteView({
        ...
        ui: {
            ...saved.ui,
            routeStacks: { ...saved.ui.routeStacks, [key]: [rootTuiRoute(saved.ui.selectedPage)] }
        }
    });
}
...
export function updateCurrentTuiRouteView(
    ...
    const route = currentTuiRoute(state);
    const key = tuiRouteViewKey(state.ui.selectedPage, state.ui.selectedInstance, route);
    const current = state.ui.routeViewStates[key] ?? defaultTuiRouteViewState();
    return {
        ...state,
        ui: {
            ...state.ui,
            routeViewStates: { ...state.ui.routeViewStates, [key]: update(current) }
        }
    };
}
...
export function selectBreadcrumbSegments(state: TuiAppState): string[] {
    ...
    switch (route.page) {
        ...
        case "connections":
            ...
            if (route.view === "oauth") segments.push("oauth", truncateTuiBreadcrumbSegment(route.providerId));
            if (route.view === "reverse") segments.push("reverse", truncateTuiBreadcrumbSegment(route.instanceId));
            break;
    }
    return segments;
}
...
function persistCurrentTuiRouteView(state: TuiAppState): TuiAppState {
    ...
    const previous = state.ui.routeViewStates[key] ?? defaultTuiRouteViewState();
        ...
        ui: {
            ...
            routeViewStates: {
                ...
                [key]: {
                    ...previous,
                    focusRegion,
                    scrollOffset: state.ui.scrollOffsets[scrollKey] ?? previous.scrollOffset,
...
function restoreCurrentTuiRouteView(state: TuiAppState, forceMainFocus = false): TuiAppState {
    ...
    return {
        ...
        ui: {
            ...
            scrollOffsets: { ...state.ui.scrollOffsets, [scrollKey]: view.scrollOffset }
        }
    };
}

function isPersistentFocusScope(scope: TuiAppState["interaction"]["focusScope"]): boolean {
    return scope === "sidebarPages" || scope === "sidebarInstances" || scope === "mainBoxes" || scope === "boxDetail" || scope === "form" || scope === "wizard" || scope === "terminal";
}

function isTuiRouteResourceValid(state: TuiAppState, route: TuiRoute, instance: string | undefined): boolean {
...
function assertRoutePage(state: TuiAppState, route: TuiRoute): void {
    if (route.page !== state.ui.selectedPage) {
        throw new Error(`Cannot navigate to ${route.page} while ${state.ui.selectedPage} is selected.`);
    }
}
