...
export function detectTerminalImageSupport(
    ...
    const disabled =
        mode === "none" ||
        environment.DEVSHELL_TUI_GRAPHICS?.trim().toLowerCase() === "none";
    return {
        iterm2:
            !disabled &&
            (program.includes("iterm") || program.includes("wezterm")),
...
export function terminalImageClearSequence(
    support: TuiTerminalImageSupport,
): string {
    return support.kitty ? `${ESCAPE}7${KITTY_DELETE_ALL}${ESCAPE}8` : "";
}

function renderKittyPng(
    image: ArtifactViewImageResult,
    region: TuiTerminalGraphicRegion,
): string {
