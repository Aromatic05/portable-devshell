...
import { editableProviderChoices } from "../../../state/editor/Instance.js";
...
import {
    ...
    parseTuiConfigDraft,
    parseTuiInstanceDraft,
    parseTuiInstancePatch,
    parseTuiMcpPatch,
    parseTuiWebPatch,
    toTuiInstanceEditorRecord,
    tuiEditorRecordsEqual,
} from "../../../state/editor/Config.js";
...
export class TuiCommandDispatcherEditor {
    ...
    async openCreateWizard(): Promise<boolean> {
        try {
            ...
            if (this.#store.getState().ui.formDrafts[key] === undefined) {
                this.#store.setFormDraft(
                    key,
                    {
                        ...
                        extensions: {
                            model: [...schema.defaultModelExtensions],
                        },
                        mcp: {
                            auth: "none",
                            contextMode:
            ...
            if (
                this.#store.getState().ui.expandedBoxes[
            ...
            this.#store.setScreenStatus(
                "instances",
                `Create setup failed: ${readErrorMessage(error)}`,
            );
            return false;
        }
    }
    ...
    openPageEditor(kind: "config" | "connector", boxId: string): boolean {
        ...
        const actionBoxId =
            kind === "config" ? "configuration-actions" : "connector-actions";
    ...
    async activate(): Promise<boolean> {
        ...
        const lineId =
            state.interaction.selectedDetailLineIds[this.#expandedKey(boxId)];
        ...
        if (action?.startsWith("field:")) {
            ...
            if (typeof current === "boolean") {
                this.#setEditorDraft(
                    target,
                    setPath(draft, target.path, !current),
                );
                return true;
            }
            this.#store.setEditor({
                ...editor,
                cursor: inputText(current).length,
                editing: true,
                error: undefined,
            });
            return true;
    ...
    editFocusedField(input: string, backspace: boolean): boolean {
        ...
        const cursor = Math.min(
            Math.max(editor.cursor ?? text.length, 0),
            text.length,
        );
        const next = backspace
            ? `${text.slice(0, Math.max(0, cursor - 1))}${text.slice(cursor)}`
    ...
    moveCursor(direction: "left" | "right"): boolean {
        ...
        const text = inputText(
            readPath(
                this.#editorDraft(target.key, target.fallback),
                target.path,
            ),
        );
        const cursor = Math.min(
    ...
    #choiceValues(
        ...
        if (editor.kind === "create") {
            ...
            if (field === "container.preset") {
                return editor.schema?.container.presets.map(
                    (entry) => entry.preset,
                );
            }
            if (field === "mcp.auth") {
    ...
    async validate(): Promise<boolean> {
        ...
        try {
            if (editor.kind === "create") {
                const draft = parseTuiInstanceDraft(
                    this.#editorDraft(editor.key, createDefaultInstanceDraft()),
                );
                const summary =
                    await this.#options.onValidateInstanceCreateDraft(draft);
                ...
                instanceDraft !== undefined
                    ? {
                          instance: {
                              ...
                              patch: parseTuiInstancePatch(instanceDraft),
                          },
                      }
                    : {}),
            ...
            if (stoppedForRestart && instance !== undefined) {
                try {
                    ...
                    reported = new AggregateError(
                        [error, restoreError],
                        "Save failed and the previous instance state could not be restored.",
                    );
                }
            }
            this.#store.setEditor({
                ...editor,
                editing: false,
                error: readErrorMessage(reported),
            });
            return false;
        }
    }
    ...
    async reload(confirmed: boolean): Promise<boolean> {
        ...
        const dirty = this.#editorDraftKeys(editor).some(
            (key) => this.#store.getState().ui.dirtyForms[key] === true,
        );
        if (dirty && !confirmed) {
            return await this.#options.dispatch({
    ...
    async #createFromWizard(): Promise<boolean> {
        ...
        try {
            ...
            this.#store.setScreenStatus(
                "instances",
                status ?? "Created through control RPC.",
            );
            return true;
        } catch (error) {
            this.#store.setEditor({
    ...
    async discard(): Promise<boolean> {
        ...
        if (
            this.#editorDraftKeys(editor).some(
                (key) => this.#store.getState().ui.dirtyForms[key] === true,
            )
        ) {
            return await this.#options.dispatch({
    ...
    changeStep(direction: "next" | "previous"): boolean {
        ...
        const step = Math.min(
            6,
            Math.max(1, (editor.step ?? 1) + (direction === "next" ? 1 : -1)),
        );
        ...
        const box =
            boxId === undefined
                ...
                : this.#projection
                      .selectMainScreenModel(this.#store.getState())
                      .boxes.find((candidate) => candidate.id === boxId);
        const line = box?.expandedLines.find(
            (candidate) =>
                candidate.id?.includes(":field:") === true ||
                candidate.id?.includes(":button:") === true,
        );
    ...
    #draftTarget(field: string): {
        ...
        if (editor.kind === "create") {
            return {
                fallback: createDefaultInstanceDraft(),
                key: editor.key,
                path: field,
            };
        }
        if (editor.kind === "connector" && field.startsWith("instance.")) {
            ...
            return {
                ...
                key: `config:${name}`,
                path: field.slice("instance.".length),
            };
        }
        if (editor.kind === "connector" && field.startsWith("web.")) {
            return {
                fallback: this.#webDraft(),
                key: "web",
                path: field.slice("web.".length),
            };
        }
        return {
            fallback:
    ...
    #mcpDraft(): Record<string, JsonValue> {
        return cloneRecord(
            asRecord(this.#store.getState().readModel.configView?.mcp) ?? {
                enabled: false,
    ...
    #fullConfigDraft(includeGlobal: boolean): ConfigDraft {
        ...
        if (includeGlobal) {
            config.mcp = coerceTuiEditorRecord(
                this.#editorDraft("connector", this.#mcpDraft()),
            );
            config.web = coerceTuiEditorRecord(
                this.#editorDraft("web", this.#webDraft()),
            );
        }
...
function describeApplyResult(result: JsonValue, restarted: boolean): string {
    ...
    if (restarted) {
        return controlRestart
            ? "Saved and instance restarted. Control restart is still required for MCP changes."
            : "Saved and instance restarted.";
    }
    if (controlRestart) {
        return "Saved. Control restart is required for MCP changes.";
    }
    return record?.reloadRequired === true
        ? "Saved and hot-applied to future instance operations."
        : "Saved.";
}
...
function applyCreateChoice(
    ...
    if (field === "container.mode" && typeof choice === "string") {
        return setPath(
            draft,
            "container",
            defaultContainerDraft(choice, draft, schema),
        );
    }
    if (field === "container.preset" && typeof choice === "string") {
        const preset = schema?.container.presets.find(
            (entry) => entry.preset === choice,
        );
...
function applyAuthModeChoice(
    ...
    const prefix =
        authPath === "auth" ? "" : authPath.slice(0, -".auth".length);
    ...
    if (choice === "oauth2") {
        return setPath(next, sibling("oauth2"), {
            requiredScopes: [],
            resourceName: "",
        });
    }
