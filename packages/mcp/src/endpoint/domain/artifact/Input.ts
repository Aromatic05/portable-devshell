...
export function readMcpArtifactViewImageInput(input: JsonValue): ArtifactViewImageInput {
    ...
    if (path === undefined) throw invalidArguments("artifact_viewImage requires path when handle is omitted.");
...
function requiredString(value: JsonValue | undefined, field: string): string {
    ...
    if (normalized === undefined) throw invalidArguments(`${field} is required.`);
    return normalized;
}

function optionalString(value: JsonValue | undefined, field: string): string | undefined {
    if (value === undefined || value === null) return undefined;
    if (typeof value !== "string" || value.trim().length === 0) throw invalidArguments(`${field} must be a non-empty string.`);
    return value.trim();
}
