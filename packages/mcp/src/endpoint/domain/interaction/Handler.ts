...
export class McpEndpointHandlerInteraction {
    ...
    constructor(
        ...
        this.#appLeases =
            options.workspaceAppLeases ?? new WorkspaceAppLeaseStore();
        this.#appPresence =
            options.workspaceAppPresence ??
            new WorkspaceAppPresenceStore({ now: options.now });
        this.#legacyV0615 = new McpWorkspaceLegacyV0615(options);
        this.#reentry =
            options.workspaceReentryArbiter ??
            (options.contextRegistry === undefined
                ? undefined
    ...
    async call(
        ...
        switch (toolName) {
            ...
            case "workspace_watch":
                return await this.#watchWorkspace(
                    gateway,
                    input,
                    context,
                    signal,
            ...
            case "workspace_reentry":
                await this.#assertAppToken(input, context);
                if (this.#reentry === undefined)
                    throw new Error(
                        "Workspace re-entry arbitration is unavailable.",
                    );
    ...
    async bootstrapWorkspace(
        ...
        const token = await this.#appLeases.issue(
            this.options.instanceName,
            ctxId,
        );
    ...
    async #askQuestion(
        ...
        const taskAssociation =
            attachedGoal === undefined
                ? await currentTodoAssociation(
                      gateway,
                      this.options.instanceName,
                      ctxId,
                  )
                : { kind: "none" as const };
        const goalStep = attachedGoal?.steps.find(
        ...
        const wait = await gateway.createWait(this.options.instanceName, {
            ...
            ...(attachedGoal === undefined
                ? {}
                : {
                      goalId: attachedGoal.goalId,
                      goalProgressAt: attachedGoal.lastProgressAt,
                      goalRevision: attachedGoal.revision,
                  }),
        ...
        try {
            ...
            if (signal?.aborted === true) {
                await gateway
                    .detachWait(this.options.instanceName, wait.waitId)
                    .catch(() => undefined);
            }
            throw error;
        }
        ...
        try {
            ...
            const current = (
                await gateway.listWaits(this.options.instanceName)
    ...
    async #manageGoal(
        ...
        const gateway = requireGoalGateway(
            this.options.gateway,
            this.options.instanceName,
        );
        ...
        if (request.action === "start") {
            await this.options.contextRegistry?.resumeAutomaticReentry(
                ctxId,
                this.options.instanceName,
            );
    ...
    async #stopGoal(
        ...
        await this.options.contextRegistry?.suppressAutomaticReentry(
            ctxId,
            this.options.instanceName,
            "Workspace Goal stopped by user",
        );
        return { goal: goal ?? null } as unknown as JsonValue;
    }
    ...
    async #watchWorkspace(
        ...
        if (!isMcpWorkspaceGateway(gateway)) {
            throw new Error(
                `Workspace live events are unavailable for ${this.options.instanceName}.`,
            );
        }
        const ctxId = requireCtxId(context);
        const token = await this.#assertAppToken(input, context);
        ...
        try {
            while (true) {
                const batch = await gateway.readWorkspaceEvents(
                    this.options.instanceName,
                    cursor + 1,
                );
                const changed =
                    batch.gap ||
                    batch.lastSeq < cursor ||
                    batch.events.some((event) =>
                        workspaceEventBelongsTo(event, ctxId),
                    );
    ...
    async #workspaceSnapshot(
        ...
        const record = await registry.validateForInstance(
            ctxId,
            this.options.instanceName,
        );
    ...
    async #reconcileGoalWaits(
        ctxId: string,
        goal: GoalSnapshot,
    ): Promise<void> {
        const gateway = this.options.gateway;
        if (!isMcpWaitRecoveryGateway(gateway)) return;
        const currentStepId = goal.steps.find(
            (step) => step.status === "active",
        )?.id;
        const terminal =
            goal.status === "completed" || goal.status === "stopped";
        const waits = await gateway.listWaits(this.options.instanceName);
        for (const wait of waits) {
            if (wait.createdByCtxId !== ctxId || wait.goalId !== goal.goalId)
                continue;
            if (
            ...
            const staleLegacyRevision =
                ...
                wait.goalRevision !== goal.revision;
            if (
                !terminal &&
                !staleStep &&
                !staleProgress &&
            ...
            if (
                ...
                await gateway
                    .cancelWait(this.options.instanceName, wait.waitId)
                    .catch(() => undefined);
                continue;
            }
            await gateway
                .disableWaitRecovery(this.options.instanceName, wait.waitId)
                .catch(() => undefined);
        }
    }
    ...
    async #disableTaskWaits(ctxId: string, taskId: string): Promise<void> {
        ...
        const waits = await gateway.listWaits(this.options.instanceName);
        for (const wait of waits) {
            if (wait.createdByCtxId !== ctxId || wait.taskId !== taskId)
                continue;
            if (
                wait.status === "consumed" ||
                wait.status === "cancelled" ||
    ...
    #workspaceResult(
        ...
        if (!this.#appPresence.has(this.options.instanceName, ctxId)) {
            throw new Error(
                "Workspace App authorization is unavailable for the current Context.",
            );
        }
        if (markAppSeen)
    ...
    async #requireActiveWorkspace(
        ...
        if (
            !(await this.#appPresence.waitUntilActive(
                this.options.instanceName,
                ctxId,
                this.options.workspaceLivenessMs ?? 60_000,
                graceMs,
    ...
    async #answerQuestion(
        ...
        const wait = (await gateway.listWaits(this.options.instanceName)).find(
            (record) => record.waitId === waitId,
        );
        if (
            wait === undefined ||
            wait.kind !== "question" ||
            wait.createdByCtxId !== requireCtxId(context)
        ) {
            throw new Error(
                `Question wait ${waitId} was not found for the current Context.`,
            );
        }
        validateQuestionAnswer(wait, answer);
        const resolved = await gateway.resolveWait(
            this.options.instanceName,
            waitId,
            { answer },
        );
    ...
    async #interruptWait(
        ...
        if (!isMcpWaitTrackingGateway(gateway)) {
            throw new Error(
                `Workspace wait interruption is unavailable for ${this.options.instanceName}.`,
            );
        }
        ...
        if (
            ...
            throw new Error(
                `Interruptible tmux wait ${waitId} was not found for the current Context.`,
            );
        }
        const interrupted = {
            interrupted: true,
    ...
    async #controlTask(
        ...
        if (gateway.controlTodo === undefined) {
            throw new Error(
                `Workspace task control is unavailable for ${this.options.instanceName}.`,
            );
        }
        const { action, revision, taskId } = readTaskControl(input);
        const ctxId = requireCtxId(context);
        const task = asRecord(
            await gateway.readTodo(this.options.instanceName, { taskId }),
        );
        if (
            task?.taskId !== taskId ||
            !taskBelongsToContext(task, taskId, ctxId)
        ) {
            throw new Error(
                `Todo task ${taskId} is not attached to the current Context.`,
            );
        }
        const controlled = await gateway.controlTodo(
            this.options.instanceName,
            taskId,
            action,
            ctxId,
        ...
        if (action === "cancel") {
            ...
            await this.options.contextRegistry?.suppressAutomaticReentry(
                ctxId,
                this.options.instanceName,
                "Workspace task cancelled by user",
            );
        } else if (action === "pause") {
            await this.options.contextRegistry?.suppressAutomaticReentry(
                ctxId,
                this.options.instanceName,
                "Workspace task paused by user",
                "paused",
            );
        } else {
            await this.options.contextRegistry?.resumeAutomaticReentry(
    ...
    async #recoverWait(
        ...
        if (!isMcpWaitRecoveryGateway(gateway)) {
            throw new Error(
                `Workspace recovery is unavailable for ${this.options.instanceName}.`,
            );
        }
        const recovery = readWaitRecovery(input);
        const wait = (await gateway.listWaits(this.options.instanceName)).find(
            (entry) => entry.waitId === recovery.waitId,
        );
        if (
            wait === undefined ||
            wait.createdByCtxId !== requireCtxId(context) ||
            ...
            throw new Error(
                `Recoverable detached wait ${recovery.waitId} was not found for the current Context.`,
            );
        }
    ...
    async #decideApproval(
        ...
        let approval:
            | Awaited<
                  ReturnType<McpInteractionGateway["listApprovals"]>
              >[number]
            | undefined;
        let approvalInstance: string | undefined;
        for (const instance of instances) {
            const candidate = (
                await gateway.listApprovals(instance).catch(() => [])
        ...
        if (
            approval === undefined ||
            approvalInstance === undefined ||
            approval.ctxId !== ctxId ||
            approval.status !== "pending"
        ) {
            throw new Error(
                `Pending approval ${approvalId} was not found for the current Context.`,
            );
        }
        const decided = await gateway.decideApproval(
            approvalInstance,
            approvalId,
            decision,
        );
    ...
    async #assertAppToken(
        ...
        if (
            ...
            throw new Error(
                "Workspace App authorization is invalid for the current Context.",
            );
        }
        this.#appPresence.touch(this.options.instanceName, ctxId);
        return token;
...
function readGoalManageInput(input: JsonValue): GoalManageInput {
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_goal requires an object input.");
    ...
    if (
        ...
        throw new Error(
            "workspace_goal action must be start, get, update, block, resume, finish, or stop.",
        );
    }
    return {
        action,
        ...
        ...(typeof record.status === "string"
            ? { status: record.status as GoalManageInput["status"] }
            : {}),
        ...(typeof record.stepId === "string" ? { stepId: record.stepId } : {}),
        ...(Array.isArray(record.steps)
            ? { steps: record.steps as unknown as GoalManageInput["steps"] }
            : {}),
...
function readQuestion(input: JsonValue): {
    ...
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_ask requires an object input.");
    const question = text(record.question, "question");
    const choices =
        record.choices === undefined
            ? []
            : stringArray(record.choices, "choices");
    const allowText = record.allowText === undefined ? true : record.allowText;
    if (typeof allowText !== "boolean")
        throw new Error("allowText must be a boolean.");
    if (!allowText && choices.length === 0)
        throw new Error(
            "workspace_ask requires choices when allowText is false.",
        );
    return { allowText, choices, question };
}

function readQuestionAnswer(input: JsonValue): {
    answer: string;
    waitId: string;
} {
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_answer requires an object input.");
    return {
...
function readApprovalDecision(input: JsonValue): {
    approvalId: string;
    decision: "approve" | "deny";
} {
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_approval requires an object input.");
...
function readTaskControl(input: JsonValue): {
    ...
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_task requires an object input.");
    const action = record.action;
...
function readWaitRecovery(input: JsonValue): {
    ...
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_recover requires an object input.");
    if (record.action !== "dismiss")
        throw new Error("workspace_recover action must be dismiss.");
    return {
        action: "dismiss",
...
function readWorkspaceCursor(input: JsonValue): number {
    ...
    if (
        ...
        throw new Error(
            "workspace_watch cursor must be a non-negative integer.",
        );
    }
    return cursor;
}

function taskBelongsToContext(
    todo: Record<string, JsonValue>,
    taskId: string,
    ctxId: string,
): boolean {
...
> {
    ...
    if (!Array.isArray(detail?.items) || typeof detail.revision !== "number")
    ...
    if (current.length !== 1 || typeof current[0]?.id !== "string")
        return { kind: "ambiguous" };
    return {
        kind: "one",
        revision: detail.revision,
        taskId,
        todoItemId: current[0].id,
    };
}
...
function readAnswer(result: JsonValue | undefined): string {
    ...
    if (record === undefined)
        throw new Error("Question resolved without an answer.");
...
function stringArray(value: unknown, field: string): string[] {
    if (!Array.isArray(value) || value.length > 12)
        throw new Error(`${field} must be an array with at most 12 entries.`);
...
export function withMcpCommentOutputSchema(
    tool: ToolDefinition,
): ToolDefinition {
    if (
        !isRecord(tool.outputSchema) ||
        !isObjectSchema(tool.outputSchema.type)
    ) {
...
export function attachMcpComments(
    result: JsonValue,
    comments: readonly string[],
): JsonValue {
    if (!isRecord(result)) {
        throw new Error(
            "MCP tool results must be objects when context comments are enabled.",
        );
    }
    if (comments.length === 0) return result;
    const existing = Array.isArray(result.comment)
        ? result.comment.filter(
