...
import { McpToolCatalogArtifact, type McpToolCatalogArtifactName } from "../tool/catalog/McpToolCatalogArtifact.js";
import { McpContextRegistry } from "../context/McpContextRegistry.js";
import { McpToolCatalogEnvironment, mcpEnvironmentToolName } from "../tool/catalog/McpToolCatalogEnvironment.js";
import type { McpInstanceGateway } from "../instance/McpInstanceGateway.js";
import { McpToolCatalogInstance, type McpToolCatalogInstanceName } from "../tool/catalog/McpToolCatalogInstance.js";
import { McpToolCatalogTodo, type McpToolCatalogTodoName } from "../tool/catalog/McpToolCatalogTodo.js";
...
export class McpEndpointWorker {
        ...
        return exposed.map((entry) =>
            this.#adaptTool(
                (entry.owner === "worker" || entry.owner === "artifact") && instanceRoutingEnabled
                    ? withMcpInstanceTarget(entry.definition)
                    : entry.definition
            )
        );
    }
    ...
    async callTool(
        ...
        const extracted = readMcpContextInput(input);
        ...
        if (known?.owner === "todo" || known?.owner === "artifact" || known?.owner === "instance") {
            ...
            return await this.#worker.auditToolCall(
                ...
                async () => {
                    switch (known.owner) {
                        case "todo":
                            return await this.#callTodoTool(toolName as McpToolCatalogTodoName, input, context, signal);
                        case "artifact":
                            return await this.#callArtifactTool(toolName as McpToolCatalogArtifactName, input, signal);
                        case "instance":
                            return await this.#callInstanceTool(toolName as McpToolCatalogInstanceName, input, signal);
                    }
                    throw this.#toolNotExposed(toolName);
                },
                signal
    ...
    async #callArtifactTool(
        ...
        switch (toolName) {
            case "artifact_share":
                ...
                return await waitForMcpEndpointAbortable(gateway.shareArtifact(this.#instanceName, readMcpArtifactShareInput(input)), signal);
            case "artifact_transfer":
                if (gateway.transferArtifact === undefined) {
                    throw this.#toolNotExposed(toolName);
                }
                return await waitForMcpEndpointAbortable(gateway.transferArtifact(this.#instanceName, readMcpArtifactTransferInput(input)), signal);
        }
    }
    ...
    async #callTodoTool(
        ...
        switch (toolName) {
            case "todo_read":
                assertMcpNoArguments(input, toolName);
                return await waitForMcpEndpointAbortable(gateway.readTodo(this.#instanceName), signal);
            case "todo_write":
                return await waitForMcpEndpointAbortable(gateway.writeTodo(this.#instanceName, input, context), signal);
        }
    }
    ...
    async #callInstanceTool(
        ...
        switch (toolName) {
            ...
            case "instance_create":
                return await waitForMcpEndpointAbortable(gateway.createSshInstance(this.#instanceName, readMcpSshCreateInput(input)), signal);
        }
    }

    #adaptTool(tool: ToolDefinition): McpTool {
        const exposed = tool.name === mcpEnvironmentToolName ? tool : withMcpContextId(tool);
