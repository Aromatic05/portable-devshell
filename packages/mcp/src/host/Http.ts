...
import { McpAuthProviderToken } from "../auth/Provider.js";
...
export class HttpHost {
    ...
    readonly #bindings = new Map<
        string,
        { auth?: McpAuthConfig; binding: McpEndpointBinding }
    >();
    ...
    constructor(options: HttpHostOptions) {
        ...
        this.#shutdownGraceMs =
            options.shutdownGraceMs ?? MCP_SHUTDOWN_GRACE_MS;
    ...
    async start(): Promise<void> {
        ...
        this.#server.on("upgrade", (request, socket, head) => {
            ...
            const handler =
                pathname === undefined
                    ? undefined
                    : (this.#upgradeHandlers.get(pathname) ??
                      this.#findUpgradePrefixHandler(pathname));
    ...
    async stop(): Promise<void> {
        ...
        await Promise.race([closed, delay(MCP_FORCE_CLOSE_WAIT_MS)]);
    }
                ...
                (request: Request, response: Response) => {
                    ...
                    if (current?.auth?.provider !== "oauth2") {
                        response.status(404).json({
                            error: "OAuth protected resource not found",
                        });
                        return;
                    }
                    response.json(
                        this.#oauth!.protectedResourceMetadata(
                            resourceServerUrl,
                            current.auth.oauth2,
                        ),
                    );
            ...
            async (request: Request, response: Response) => {
                try {
                    ...
                    if (current === undefined) {
                        response
                            .status(404)
                            .json({ error: "Instance endpoint not found" });
                        return;
                    }
                    const body = await this.#readJsonBody(
                        request as IncomingMessage,
                    );
                    await current.binding.handleRequest(
                        request,
                        response,
                        body,
                    );
    ...
    #createAuthHandlers(path: string): RequestHandler[] {
        ...
        if (
            ...
            this.#oauth.registerResource(resourceServerUrl, auth.oauth2);
            routeHandlers.push(
                this.#oauth.requestAuthHandler(resourceServerUrl, auth.oauth2),
            );
            this.#app.use(
                this.#oauthProtectedResourceMetadataPath(resourceServerUrl),
                this.#oauth.protectedResourceMetadataHandler(
                    resourceServerUrl,
                    auth.oauth2,
                ),
            );
            ...
            routeHandlers.push((_request: Request, response: Response) => {
                response
                    .status(503)
                    .json({ error: "OAuth authentication is unavailable" });
            });
        } else if (auth?.provider === "token") {
            routeHandlers.push(
                (request: Request, response: Response, next: NextFunction) => {
                    const requestAuth = new McpAuthProviderToken(
                        auth.token,
    ...
    #bindingAuthHandler(path: string): RequestHandler {
        return (request: Request, response: Response, next: NextFunction) => {
            ...
            if (
                ...
                void this.#oauth.requestAuthHandler(
                    resourceServerUrl,
                    auth.oauth2,
    ...
    #usesOAuth(): boolean {
        ...
        return [...this.#bindings.values()].some(
            (binding) => binding.auth?.provider === "oauth2",
        );
    }
    ...
    async #readJsonBody(request: IncomingMessage): Promise<JsonValue> {
        ...
        if (
            ...
            contentLength > MCP_MAX_REQUEST_BODY_BYTES
        ) {
            request.resume();
            throw new McpHttpInputError(
                413,
                `Request body exceeds ${MCP_MAX_REQUEST_BODY_BYTES} bytes.`,
            );
        }
...
function normalizeRoutePrefix(path: string): string {
    if (!path.startsWith("/"))
        throw new TypeError("HTTP route prefix must start with '/'.");
