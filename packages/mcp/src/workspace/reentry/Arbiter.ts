...
import { readWorkspaceSnapshot } from "../Snapshot.js";
...
export async function assertWorkspaceWaitRecoveryAssociationAvailable(
    ...
    const gateway = requireInteractionGateway(
        options.gateway,
        options.instanceName,
    );
    const ctxId = requireCtxId(context);
    const explicitHumanResume =
        wait.kind === "question" || asRecord(wait.result)?.interrupted === true;
    if (
        wait.recoveryDisabledAt !== undefined ||
        (wait.automaticRecovery === false && !explicitHumanResume)
    ) {
        throw new Error(
            `Wait ${wait.waitId} is not available for automatic recovery.`,
        );
    }
    if (
        wait.workspace !== undefined &&
        context.workspace !== undefined &&
        wait.workspace !== context.workspace
    ) {
        throw new Error(`Wait ${wait.waitId} belongs to another workspace.`);
    }
    if (wait.goalId !== undefined) {
        const goalGateway = requireGoalGateway(
            options.gateway,
            options.instanceName,
        );
        ...
        if (
            ...
            (goal.status !== "active" &&
                !(goal.status === "blocked" && explicitHumanResume))
        ) {
            throw new Error(
                `Workspace Goal ${wait.goalId} is not available for automatic recovery.`,
            );
        }
        if (
            wait.workspace !== undefined &&
            goal.workspace !== undefined &&
            wait.workspace !== goal.workspace
        ) {
            throw new Error(
                `Workspace Goal ${wait.goalId} moved to another workspace.`,
            );
        }
        if (wait.goalStepId !== undefined) {
            const step = goal.steps.find(
                (candidate) => candidate.id === wait.goalStepId,
            );
            if (step === undefined || step.status !== "active") {
                throw new Error(
                    `Workspace Goal step ${wait.goalStepId} is no longer available for automatic recovery.`,
                );
            }
        } else if (
            wait.goalProgressAt !== undefined &&
            goal.lastProgressAt !== wait.goalProgressAt
        ) {
            throw new Error(
                `Workspace Goal ${wait.goalId} progressed since wait ${wait.waitId} was created.`,
            );
        } else if (
            wait.goalProgressAt === undefined &&
            wait.goalRevision !== undefined &&
            goal.revision !== wait.goalRevision
        ) {
            throw new Error(
                `Workspace Goal ${wait.goalId} changed since wait ${wait.waitId} was created.`,
            );
        }
        return;
    }
    ...
    const todo = asRecord(
        await gateway.readTodo(options.instanceName, { taskId: wait.taskId }),
    );
    ...
    if (task === undefined || task.status !== "in_progress") {
        throw new Error(
            `Durable task ${wait.taskId} is not available for automatic recovery.`,
        );
    }
    if (wait.todoItemId !== undefined) {
        const item = Array.isArray(todo?.items)
            ? todo.items
                  .map(asRecord)
                  .find((entry) => entry?.id === wait.todoItemId)
            : undefined;
        if (item?.status !== "in_progress") {
            throw new Error(
                `Durable task item ${wait.todoItemId} is no longer available for automatic recovery.`,
            );
        }
    } else if (
        wait.taskRevision !== undefined &&
        todo?.revision !== wait.taskRevision
    ) {
        throw new Error(
            `Durable task ${wait.taskId} changed since wait ${wait.waitId} was created.`,
        );
    }
}
...
export class McpWorkspaceReentryArbiter {
    ...
    async control(
        ...
        if (request.action === "get") {
            return (await registry.readAutomaticReentry(
                ctxId,
                this.options.instanceName,
        ...
        if (request.action === "resume") {
            const state = await registry.resumeAutomaticReentry(
                ctxId,
                this.options.instanceName,
            );
        ...
        if (request.action === "claim") {
            if (
                ...
                request.intent !== "automatic"
            ) {
                if (request.sourceId === undefined)
                    throw new Error(
                        "Explicit Workspace re-entry requires sourceId.",
                    );
                return await this.#claimExplicit(
                    gateway,
                    context,
                    claimId,
                    request.intent,
        ...
        if (request.action === "validate")
            return await this.#validate(gateway, context, claimId);
        if (request.action === "attempt")
            return await this.#attempt(gateway, context, claimId);
        if (request.action === "report") {
            if (request.outcome === undefined)
                throw new Error("workspace_reentry report requires outcome.");
        ...
        const state = await registry.releaseAutomaticReentry(
            ctxId,
            this.options.instanceName,
            claimId,
        );
    ...
    async observeExecutionStart(ctxId: string): Promise<number | undefined> {
        ...
        return (
            await this.options.contextRegistry
                .observeExecutionActivity(ctxId, this.options.instanceName)
                .catch(() => undefined)
        )?.executionEpoch;
    }
    ...
    async executionActive(
        ctxId: string,
        ownerCallId?: string,
    ): Promise<boolean> {
        try {
            const state =
                await this.options.contextRegistry.readAutomaticReentry(
                    ctxId,
                    this.options.instanceName,
                );
    ...
    async #claimAutomatic(
        ...
        const waitDelivery = await this.#claimResolvedWaitDelivery(
            gateway,
            context,
            claimId,
        );
        ...
        const goalGateway = isMcpGoalGateway(this.options.gateway)
            ? this.options.gateway
            : undefined;
        if (goalGateway === undefined)
            return await this.#releaseUnclaimedContext(ctxId, claimId);
        const goal = await goalGateway.readGoal(
            this.options.instanceName,
            ctxId,
        );
        if (goal?.continuationDue !== true)
            return await this.#releaseUnclaimedContext(ctxId, claimId);
        ...
        const claimedGoal = await goalGateway.readGoal(
            this.options.instanceName,
            ctxId,
        );
        ...
        const delivery = goalReentryDelivery(
            claimedGoal,
            typeof claimed.continuationCount === "number"
                ? claimed.continuationCount
                : undefined,
            await this.#workspaceSnapshot(gateway, ctxId),
        );
        const state = await this.options.contextRegistry.readAutomaticReentry(
            ctxId,
            this.options.instanceName,
        );
        return {
    ...
    async #claimExplicit(
        ...
        if (
            !(await this.#explicitSourceAvailable(
                gateway,
                ctxId,
                intent,
                sourceId,
        ...
        if (intent === "goal-retry") {
            try {
                ...
                await this.#rollbackContextClaimAfterFailure(
                    ctxId,
                    claimId,
                    error,
                );
        ...
        return {
            ...
            claimed: true,
            claimId,
            delivery: explicitReentryDelivery(
                intent,
                sourceId,
                await this.#workspaceSnapshot(gateway, ctxId),
            ),
    ...
    async #claimContext(
        ...
        const arbitration =
            await this.options.contextRegistry.claimAutomaticReentry(
                ctxId,
                this.options.instanceName,
                claimId,
            );
        if (!arbitration.claimed)
            return { claimed: false as const, state: arbitration.state };
        if (await this.#contextExecutionBusy(gateway, ctxId)) {
            await this.#consumeResolvedWaitsDuringExecution(gateway, ctxId);
            const state =
    ...
    async #validate(
        ...
        const ctxId = requireCtxId(context);
        const validation =
            await this.options.contextRegistry.validateAutomaticReentry(
                ctxId,
                this.options.instanceName,
                claimId,
            );
        ...
        const sourceKind =
            validation.state.sourceKind ??
            (claimedWait === undefined ? "goal" : "wait");
        if (await this.#contextExecutionBusy(gateway, ctxId)) {
            if (claimedWait !== undefined)
        ...
        if (sourceKind === "wait") {
            ...
            try {
                await assertWorkspaceWaitRecoveryAssociationAvailable(
                    this.options,
                    claimedWait,
                    context,
                );
        ...
        if (
            sourceKind === "goal-resume" ||
            sourceKind === "goal-retry" ||
            sourceKind === "task-resume"
        ) {
    ...
    async #attempt(
        ...
        const validated = asRecord(
            await this.#validate(gateway, context, claimId),
        );
        ...
        const claimedWait = await this.#claimedWait(gateway, ctxId, claimId);
        const sourceKind =
            typeof validated.sourceKind === "string"
                ? validated.sourceKind
                : claimedWait === undefined
                  ? "goal"
                  : "wait";
        const state =
            await this.options.contextRegistry.markAutomaticReentryAttempted(
                ctxId,
                this.options.instanceName,
                claimId,
            );
        try {
            if (sourceKind === "wait") {
                if (claimedWait === undefined)
                    throw new Error(
                        "Workspace wait re-entry claim is no longer active.",
                    );
                await requireWaitRecoveryGateway(
                    this.options.gateway,
                    this.options.instanceName,
                ...
                if (attempted?.attempted !== true)
                    throw new Error(
                        "Workspace Goal continuation attempt was rejected.",
                    );
            }
        } catch (error) {
            try {
    ...
    async #report(
        ...
        const claimedWait = await this.#claimedWait(gateway, ctxId, claimId);
        const sourceKind =
            reentry.claimId === claimId
                ? reentry.sourceKind
                : claimedWait === undefined
                  ? undefined
                  : "wait";
        if (sourceKind === "wait" && claimedWait !== undefined) {
            const recoveryGateway = requireWaitRecoveryGateway(
                this.options.gateway,
                this.options.instanceName,
            );
            if (outcome === "accepted") {
                if (
                    claimedWait.goalId !== undefined &&
                    this.options.gateway?.recordGoalReentry !== undefined
                ) {
                    await this.options.gateway.recordGoalReentry(
                        this.options.instanceName,
                        ctxId,
                        claimedWait.recoveryGoalProgressEpoch ??
                            claimedWait.goalProgressEpoch,
                    );
                }
                await recoveryGateway.completeWaitRecovery(
                    this.options.instanceName,
                    claimedWait.waitId,
                    claimId,
                );
            ...
            const goal = await requireGoalGateway(
                this.options.gateway,
                this.options.instanceName,
        ...
        const state =
            outcome === "rejected"
                ...
                : await this.#markContextAcceptedOrConfirmSettled(
                      ctxId,
                      claimId,
                  );
    ...
    async #claimResolvedWaitDelivery(
        ...
        for (const wait of candidates) {
            ...
            try {
                claimed = await recoveryGateway.claimWaitRecovery(
                    this.options.instanceName,
                    wait.waitId,
                    claimId,
                );
            ...
            try {
                ...
                await this.#rollbackUnboundSourceClaim(
                    ...
                    claimId,
                    async () =>
                        await recoveryGateway.releaseWaitRecovery(
                            this.options.instanceName,
                            wait.waitId,
                            claimId,
                        ),
    ...
    async #releaseSource(
        ...
        if (reentry.sourceKind === "wait") {
            const claimedWait = await this.#claimedWait(
                gateway,
                ctxId,
                claimId,
            );
            if (
                claimedWait !== undefined &&
                isMcpWaitRecoveryGateway(this.options.gateway)
            ) {
        ...
        if (
            reentry.sourceKind === "goal" &&
            isMcpGoalGateway(this.options.gateway)
        ) {
    ...
    async #markContextAcceptedOrConfirmSettled(ctxId: string, claimId: string) {
        try {
    ...
    async #runGoalSourceTransitionOrConfirmSettled(
        ...
        try {
            ...
            let goal;
            try {
                goal = await goalGateway.readGoal(
                    this.options.instanceName,
                    ctxId,
                );
            } catch (readError) {
    ...
    async #settleAttemptedFromExecution(ctxId: string): Promise<void> {
        let state;
        try {
            state = await this.options.contextRegistry.readAutomaticReentry(
                ctxId,
                this.options.instanceName,
            );
        } catch {
        ...
        try {
            if (state.sourceKind === "wait") {
                if (
                    sourceId === undefined ||
                    this.options.gateway?.completeWaitRecovery === undefined
                ) {
                    settled = false;
                } else {
                    try {
                        await this.options.gateway.completeWaitRecovery(
                            this.options.instanceName,
                            sourceId,
                            claimId,
                        );
                        ...
                        const wait = (
                            await this.options.gateway.listWaits?.(
                                this.options.instanceName,
                            )
                ...
                state.sourceKind === "goal-resume" ||
                state.sourceKind === "goal-retry"
            ) {
                if (this.options.gateway?.recordGoalReentry !== undefined) {
                    const goal = await this.options.gateway.readGoal?.(
                        this.options.instanceName,
                        ctxId,
                    );
                    if (
                        goal !== undefined &&
                        (sourceId === undefined || goal.goalId === sourceId)
                    ) {
                        await this.options.gateway.recordGoalReentry(
                            this.options.instanceName,
                            ctxId,
    ...
    async #explicitSourceAvailable(
        ...
        if (intent === "task-resume") {
            const todo = asRecord(
                await gateway.readTodo(this.options.instanceName, {
                    taskId: sourceId,
                }),
            );
            if (todo?.taskId !== sourceId || !Array.isArray(todo.tasks))
                return false;
            const task = todo.tasks
                .map(asRecord)
                .find(
                    (entry) =>
                        entry?.taskId === sourceId && entry.ctxId === ctxId,
                );
    ...
    async #claimedWait(
        ...
        ctxId: string,
        claimId: string,
    ): Promise<WaitRecord | undefined> {
        return (await gateway.listWaits(this.options.instanceName)).find(
            (wait) =>
                wait.createdByCtxId === ctxId &&
                wait.status === "resolved" &&
                wait.recoveryClaimId === claimId,
        );
    }

    async #resolvedAutomaticWaits(
        gateway: McpInteractionGateway,
        ctxId: string,
    ): Promise<WaitRecord[]> {
        return (await gateway.listWaits(this.options.instanceName))
            ...
            .sort(
                (left, right) =>
                    (left.resolvedAt ?? left.updatedAt).localeCompare(
                        right.resolvedAt ?? right.updatedAt,
                    ) || left.waitId.localeCompare(right.waitId),
            );
    }

    async #hasBlockingAutomaticWait(
        gateway: McpInteractionGateway,
        ctxId: string,
    ): Promise<boolean> {
        return (await gateway.listWaits(this.options.instanceName)).some(
            (wait) =>
                wait.createdByCtxId === ctxId &&
                wait.status !== "consumed" &&
                wait.status !== "cancelled" &&
    ...
    #waitEligibleForAutomaticNotification(wait: WaitRecord): boolean {
        ...
        const explicitHumanResume =
            wait.kind === "question" ||
            asRecord(wait.result)?.interrupted === true;
        return wait.automaticRecovery !== false || explicitHumanResume;
    }

    async #consumeResolvedWaitsDuringExecution(
        gateway: McpInteractionGateway,
        ctxId: string,
    ): Promise<void> {
        ...
        for (const instance of instances) {
            try {
                ...
                if (gateway.readToolCalls !== undefined) {
                    const calls = await gateway.readToolCalls(
                        instance,
                        ctxId,
                        64,
                    );
    ...
    async #hasPendingContextApproval(
        gateway: McpInteractionGateway,
        ctxId: string,
    ): Promise<boolean> {
    ...
    async #workspaceSnapshot(
        ...
        const record = await this.options.contextRegistry.validateForInstance(
            ctxId,
            this.options.instanceName,
        );
        const reentry = await this.options.contextRegistry.readAutomaticReentry(
        ...
        return await readWorkspaceSnapshot(
            ...
            ctxId,
            {
                instances: record.environments.map(
...
function readReentryControl(input: JsonValue): {
    ...
    if (record === undefined)
        throw new Error("workspace_reentry requires an object input.");
    ...
    if (
        ...
        throw new Error(
            "workspace_reentry action must be get, yield, resume, claim, validate, attempt, report, or release.",
        );
    }
    ...
    if (
        ...
        throw new Error(
            "workspace_reentry intent must be automatic, goal-resume, goal-retry, or task-resume.",
        );
    }
    ...
    if (
        ...
        throw new Error(
            "workspace_reentry outcome must be accepted, rejected, or uncertain.",
        );
    }
    return {
        action,
...
function requireInteractionGateway(
    ...
    throw new Error(
        `Workspace interaction backend is unavailable for ${instanceName}.`,
    );
}

function requireGoalGateway(
    gateway: McpInstanceGateway | undefined,
    instanceName: string,
) {
    if (isMcpGoalGateway(gateway)) return gateway;
    throw new Error(
        `Workspace Goal backend is unavailable for ${instanceName}.`,
    );
}

function requireWaitRecoveryGateway(
    gateway: McpInstanceGateway | undefined,
    instanceName: string,
) {
    if (isMcpWaitRecoveryGateway(gateway)) return gateway;
    throw new Error(
        `Workspace wait recovery backend is unavailable for ${instanceName}.`,
    );
}
