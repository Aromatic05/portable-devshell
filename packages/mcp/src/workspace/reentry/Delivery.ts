...
const WAIT_CONTINUATION_MESSAGE =
...
export type WorkspaceExplicitReentryKind =
    "goal-resume" | "goal-retry" | "task-resume";
...
export function explicitReentryDelivery(
    ...
    return {
        kind: "explicit",
        message,
        modelContext: workspaceModelContext(
            snapshot,
            continuation as unknown as JsonValue,
        ),
        sourceId,
    };
}

export function waitReentryDelivery(
    wait: WaitRecord,
    snapshot: JsonValue,
): WorkspaceReentryDelivery {
    return {
        ...
        ...(wait.recoveryMessageId === undefined
            ? {}
            : { messageId: wait.recoveryMessageId }),
...
export function goalReentryDelivery(
    ...
    return {
        kind: "goal",
        message: [enforcement, GOAL_CONTINUATION_MESSAGE]
            .filter(Boolean)
            .join("\n\n"),
        ...(goal.continuationMessageId === undefined
            ? {}
            : { messageId: goal.continuationMessageId }),
        modelContext: workspaceModelContext(
            snapshot,
            goalContinuation(goal, attempt),
        ),
        sourceId: goal.goalId,
    };
}

export function workspaceModelContext(
    snapshotValue: JsonValue,
    continuation?: JsonValue,
): JsonValue {
    const snapshot = record(snapshotValue);
    const tasks = Array.isArray(snapshot?.tasks) ? snapshot.tasks : [];
    const background = Array.isArray(snapshot?.background)
        ? snapshot.background
        : [];
    const state = compact({
...
function waitContinuation(wait: WaitRecord): JsonValue {
    ...
    const afterResult =
        !isQuestion &&
        (result.waitReason === "timeout" || result.timedOut === true)
            ? {
                  operation: {
                      kind: "blocking-wait",
                      taskId,
...
function goalContinuation(goal: GoalSnapshot, attempt: number): JsonValue {
    const currentItem =
        goal.steps.find((step) => step.status === "active") ??
        goal.steps.find((step) => step.status === "pending");
    const currentIndex =
        currentItem === undefined ? -1 : goal.steps.indexOf(currentItem);
    const terminalItem = {
        id: "finish-goal",
        kind: "goal-terminal",
        status: "pending",
...
function goalContinuationAttempt(
    goal: GoalSnapshot,
    continuationCount: number | undefined,
): number {
    if (Number.isFinite(goal.noActionStreak))
        return Math.max(1, Math.floor(goal.noActionStreak!) + 1);
    if (Number.isFinite(continuationCount))
        return Math.max(1, Math.floor(continuationCount!));
