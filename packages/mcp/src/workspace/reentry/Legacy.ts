...
export class McpWorkspaceLegacyV0615 {
    ...
    async call(
        ...
        switch (toolName) {
            ...
            default:
                throw new Error(
                    `Unsupported v0.6.15 Workspace app tool: ${toolName}.`,
                );
        }
    }

    async #controlReentry(
        input: JsonValue,
        context: ToolCallContext,
    ): Promise<JsonValue> {
        const registry = this.options.contextRegistry;
        if (registry === undefined)
            throw new Error("Workspace re-entry arbitration is unavailable.");
        ...
        if (request.action === "claim") {
            const claimId =
                request.claimId ?? `workspace-reentry-${randomUUID()}`;
            const result = await registry.claimAutomaticReentry(
                ctxId,
                this.options.instanceName,
                claimId,
            );
        ...
        if (request.action === "validate") {
            if (request.claimId === undefined)
                throw new Error(
                    "workspace_reentry_control validate requires claimId.",
                );
            const result = await registry.validateAutomaticReentry(
                ctxId,
                this.options.instanceName,
                request.claimId,
            );
        ...
        if (request.claimId === undefined)
            throw new Error(
                "workspace_reentry_control release requires claimId.",
            );
    ...
    async #continueGoal(
        ...
        if (request.action !== "report" && request.action !== "reset") {
            const workspaceGateway = isMcpWorkspaceGateway(interactionGateway)
                ? interactionGateway
                : undefined;
            const instances = await this.#contextInstances(ctxId);
            const [goal, waits, approvalSlices, activeCallSlices] =
                await Promise.all([
                    goalGateway.readGoal(this.options.instanceName, ctxId),
                    interactionGateway.listWaits(this.options.instanceName),
                    Promise.allSettled(
                        instances.map(async (instance) =>
                            interactionGateway.listPendingApprovals ===
                            undefined
                                ? (
                                      await interactionGateway.listApprovals(
                                          instance,
                                      )
                                  ).filter(
                                      (approval) =>
                                ...
                                : await interactionGateway.listPendingApprovals(
                                      instance,
                                      ctxId,
                                  ),
                        ),
                    ...
                    Promise.allSettled(
                        instances.map(async (instance) => {
                            if (
                                workspaceGateway?.hasActiveToolCalls !==
                                undefined
                            ) {
                            ...
                            const calls =
                                await (workspaceGateway?.readToolCalls(
                                    instance,
                                    ctxId,
                                    64,
                                ) ?? []);
    ...
    async #recoverWait(
        ...
        if (wait.status === "consumed")
            return completedWaitCompatibility(wait, recovery.action);
        ...
        if (recovery.action === "attempt") {
            ...
            return {
                attempted: true,
                ...(attempted.recoveryMessageAttemptedAt === undefined
                    ? {}
                    : {
                          recoveryMessageAttemptedAt:
                              attempted.recoveryMessageAttemptedAt,
                      }),
                ...(attempted.recoveryMessageId === undefined
                    ? {}
                    : { recoveryMessageId: attempted.recoveryMessageId }),
                waitId: attempted.waitId,
            };
        }
        if (recovery.action === "sent") {
            const sent = await gateway.completeWaitRecovery(
                this.options.instanceName,
                recovery.waitId,
                recovery.claimId,
            );
            if (
                sent.goalId !== undefined &&
        ...
        if (recovery.action === "release") {
            await gateway.releaseWaitRecovery(
                this.options.instanceName,
                recovery.waitId,
                recovery.claimId,
            );
        ...
        if (recovery.action === "reject") {
            await gateway.rejectWaitRecovery(
                this.options.instanceName,
                recovery.waitId,
                recovery.claimId,
            );
        ...
        if (recovery.action === "complete") {
            const consumed = await gateway.completeWaitRecovery(
                this.options.instanceName,
                recovery.waitId,
                recovery.claimId,
            );
        ...
        const claimed = await gateway.claimWaitRecovery(
            this.options.instanceName,
            recovery.waitId,
            claimId,
        );
        return {
            ...
            ...(claimed.recoveryMessageAttemptedAt === undefined
                ? {}
                : {
                      recoveryMessageAttemptedAt:
                          claimed.recoveryMessageAttemptedAt,
                  }),
            ...(claimed.recoveryMessageId === undefined
                ? {}
                : { recoveryMessageId: claimed.recoveryMessageId }),
            ...(claimed.recoveryMessageSentAt === undefined
                ? {}
                : { recoveryMessageSentAt: claimed.recoveryMessageSentAt }),
...
function completedWaitCompatibility(
    wait: WaitRecord,
    action: string,
): JsonValue {
...
function readGoalContinuationInput(input: JsonValue): GoalContinuationInput {
    ...
    if (record === undefined)
        throw new Error("workspace_goal_continue requires an object input.");
    ...
    if (
        ...
        throw new Error(
            "workspace_goal_continue action must be claim, validate, attempt, report, or reset.",
        );
    }
    ...
    if (record.claimId !== undefined)
        request.claimId = text(record.claimId, "claimId");
...
function readReentryControl(input: JsonValue): {
    ...
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_reentry_control requires an object input.");
    const action = record.action;
    if (
        action !== "get" &&
        action !== "yield" &&
        ...
        throw new Error(
            "workspace_reentry_control action must be get, yield, resume, claim, validate, or release.",
        );
    }
    return {
        action,
        ...(record.claimId === undefined
            ? {}
            : { claimId: text(record.claimId, "claimId") }),
...
function readWaitRecovery(input: JsonValue):
    ...
    | {
          action: "attempt" | "complete" | "release" | "reject" | "sent";
          claimId: string;
          waitId: string;
      } {
    const record = asRecord(input);
    if (record === undefined)
        throw new Error("workspace_wait_recover requires an object input.");
    const action = record.action;
    ...
    throw new Error(
        "workspace_wait_recover action must be claim, attempt, sent, complete, release, reject, or dismiss.",
    );
}
...
function requireGoalGateway(
    ...
    if (!isMcpGoalGateway(gateway))
        throw new Error(`Workspace Goal is unavailable for ${instance}.`);
    return gateway;
}
...
function requireInteractionGateway(
    ...
    if (!isMcpInteractionGateway(gateway))
        throw new Error(
            `Workspace interaction is unavailable for ${instance}.`,
        );
    return gateway;
}
