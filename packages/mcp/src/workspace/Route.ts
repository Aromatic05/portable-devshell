...
import type { HttpHost } from "../host/Http.js";
import {
...
import { readWorkspaceSnapshot, workspaceEventBelongsTo } from "./Snapshot.js";
import { WorkspaceAppLeaseStore } from "./app/Lease.js";
import { WorkspaceAppPresenceStore } from "./app/Presence.js";
...
export function workspaceLiveBaseUrl(
    ...
    url.pathname = joinUrlPaths(
        url.pathname,
        workspaceLiveRoutePath(instanceName),
    );
    url.search = "";
    url.hash = "";
...
export function installMcpWorkspaceLiveRoute(
    ...
    const gateway = isMcpInteractionGateway(options.gateway)
        ? options.gateway
        : undefined;
    const workspaceGateway = isMcpWorkspaceGateway(options.gateway)
        ? options.gateway
        : undefined;
    if (gateway === undefined || workspaceGateway === undefined)
        return () => undefined;

    const cleanups = liveRouteBases(
            ...
            async (request, response) => {
                ...
                const abort = () => {
                    if (!response.writableEnded)
                        disconnect.abort("Live Workspace connection closed");
                };
                ...
                try {
                    ...
                    while (!disconnect.signal.aborted) {
                        const batch =
                            await workspaceGateway.readWorkspaceEvents(
                                options.instanceName,
                                cursor + 1,
                            );
                        const changed =
                        ...
                        if (changed || Date.now() - startedAt >= heartbeatMs) {
                            writeJson(response, 200, {
                                changed,
                                cursor,
                                snapshot: await readWorkspaceSnapshot(
                                    gateway,
                                    options.instanceName,
                                    ctxId,
                                    {
                                        instances: authorized.instances,
                                        reentry:
                                            (await options.contextRegistry.readAutomaticReentry(
                                                ctxId,
                                                options.instanceName,
                    ...
                    if (ctxId !== undefined)
                        options.presence.endWatch(options.instanceName, ctxId);
...
async function authorize(
    ...
    if (
        token === undefined ||
        !(await options.leases.verify(options.instanceName, ctxId, token))
    ) {
    ...
    try {
        const record = await options.contextRegistry.validateForInstance(
            ctxId,
            options.instanceName,
        );
        const reentry = await options.contextRegistry.readAutomaticReentry(
            ctxId,
            options.instanceName,
        );
        return {
        ...
        if (/expired|disabled/iu.test(message))
            throw new LiveRouteError(401, message);
        throw new LiveRouteError(
            401,
            "Workspace Context is unavailable for this capability.",
        );
    }
}
...
function readCursor(request: IncomingMessage): number {
    ...
    if (!/^\d+$/u.test(raw))
        throw new LiveRouteError(400, "cursor must be a non-negative integer");
...
function writeLiveError(response: ServerResponse, error: unknown): void {
    ...
    const message =
        error instanceof Error
            ? error.message
            : "Live Workspace request failed";
    const statusCode = /Context|expired|disabled/iu.test(message) ? 401 : 500;
    writeJson(response, statusCode, {
        error: statusCode === 500 ? "Live Workspace request failed" : message,
    });
}
...
function joinUrlPaths(basePathname: string, nextPathname: string): string {
    ...
    const next = nextPathname.startsWith("/")
        ? nextPathname
        : `/${nextPathname}`;
    return `${base}${next}`;
}
