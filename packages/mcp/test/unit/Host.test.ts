...

{
    ...
    test("MCP HTTP host bounds graceful shutdown when an active request never completes", async () => {
        ...
        assert.ok(
            ...
            "shutdown should force-close a stuck active request",
        );
        request.destroy();
    });
    ...
    test("HTTP server serves WebUI assets with browser security and cache headers", async () => {
        ...
        await writeFile(
            ...
            "<!doctype html><title>devshell</title>",
            "utf8",
        );
        await writeFile(
            join(directory, "assets", "app-abc123.js"),
            "export {};",
            "utf8",
        );
        const server = new HttpHost({
        ...
        try {
            ...
            assert.match(
                readHeader(index.headers, "content-security-policy"),
                /frame-ancestors 'none'/u,
            );
            assert.equal(index.headers["x-content-type-options"], "nosniff");
            assert.match(index.body, /<title>devshell<\/title>/u);

            const asset = await requestHttp(
                address.port,
                "/web/assets/app-abc123.js",
            );
