...
import { McpEndpointCatalog } from "../../../../src/endpoint/tool/Catalog.ts";
import { McpEndpointDispatch } from "../../../../src/endpoint/dispatch/Dispatch.ts";
import { McpNativeToolResult } from "../../../../src/endpoint/Endpoint.ts";
import { McpContextRemoteEnvironment } from "../../../../src/context/Environment.ts";
import { McpContextRegistry } from "../../../../src/context/registry/Registry.ts";
...
import { McpWorkspaceReentryArbiter } from "../../../../src/workspace/reentry/Arbiter.ts";
...

{
    ...
    test("queued Comments are not exposed as a standalone MCP tool", () => {
        ...
        assert.equal(
            catalog
                .listTools()
                .some((tool) => tool.name === "context_message_read"),
            false,
        );
    });
    ...
    test("a routed artifact result consumes Comments from the routed instance Context", async () => {
        ...
        const environment = structuredResult<{ ctxId: string }>(
            await dispatch.callTool(
                "environ_info",
        ...
        const handle = (
            await contextRegistry.referenceInstance(environment.ctxId, "beta")
        )?.handle;
        ...
        assert.deepEqual(
            (result as { structuredContent?: JsonValue }).structuredContent,
            {
                blake3: "b".repeat(64),
                bytes: 1,
    ...
    async function createContext(
        ...
        const result = structuredResult<{ ctxId: string }>(
            await dispatch.callTool(
                "environ_info",
                { workspace: "/workspace" },
    ...
    function createHarness(
        ...
        const worker = {
            ...
            async prepareExtensionResource() {
                return {
                    directory:
                        "/workspace/.devshell/alpha/extensions/skill/resources/managed",
                };
            },
...

{
    ...
    test("workspace_ask detaches durable wait state when the host cancels the held call", async () => {
        ...
        assert.equal(
            fake.waits.find((entry) => entry.waitId === wait.waitId)?.status,
            "detached",
        );
        assert.deepEqual(
            await handler.call(
    ...
    test("workspace_ask keeps a resolved answer recoverable when post-answer processing fails", async () => {
        ...
        const recovered = fake.waits.find(
    ...
    test("Workspace authorization stays in hidden metadata and gates app-only tools", async () => {
        ...
        const opened = await handler.call(
            "workspace_open",
            {},
            context,
            "call-open",
        ...
        const meta = opened._meta?.["portable-devshell/workspace"] as
            { token?: unknown } | undefined;
        if (typeof meta?.token !== "string")
            throw new Error("workspace token missing");
        assert.equal(
            JSON.stringify(opened.structuredContent).includes(meta.token),
            false,
        );
        ...
        assert.equal(
            JSON.stringify(snapshot.structuredContent).includes(meta.token),
            false,
        );
        await assert.rejects(
            handler.call(
    ...
    test("Workspace App can re-establish its lifecycle after remount", async () => {
        ...
        const reconnected = await handler.call(
            "workspace_reconnect",
            { token },
            context,
            "call-reconnect",
        ...
        assert.equal(
            (reconnected.structuredContent as { ctxId?: string }).ctxId,
            context.ctxId,
        );
        const meta = reconnected._meta?.["portable-devshell/workspace"] as
    ...
    test("Workspace App capability survives MCP handler restart without rotating the token", async () => {
        ...
        try {
            ...
            assert.equal(
                (
                    reconnected._meta?.["portable-devshell/workspace"] as
                        { token?: string } | undefined
                )?.token,
                token,
            );
            await assert.doesNotReject(
                restartedHandler.call(
                    "workspace_snapshot",
                    { token },
                    context,
    ...
    test("Workspace sibling requests keep their own verified capability token", async () => {
        const root = await createTestTempDirectory(
            "workspace-app-sibling-capabilities",
        );
        const filePath = join(root, "workspace-app-leases.json");
        try {
            const firstStore = new WorkspaceAppLeaseStore({
                filePath,
                tokenFactory: () => "token-sibling-first",
            });
            await firstStore.initialize();
            const firstToken = await firstStore.issue("demo", context.ctxId!);
            const secondStore = new WorkspaceAppLeaseStore({
                filePath,
                tokenFactory: () => "token-sibling-second",
            });
            ...
            assert.equal(
                (
                    firstResult._meta?.["portable-devshell/workspace"] as
                        { token?: string } | undefined
                )?.token,
                firstToken,
            );
            assert.equal(
                (
                    second._meta?.["portable-devshell/workspace"] as
                        { token?: string } | undefined
                )?.token,
                secondToken,
            );
    ...
    test("Workspace snapshot projects only compact task and background state", async () => {
        ...
        const gateway = Object.assign(fake.gateway, {
            ...
            async readToolCalls() {
                throw new Error(
                    "workspace snapshot should not read historical tool calls",
                );
            },
            async readWorkspaceEvents() {
        ...
        const tmuxBackground = snapshot.background?.find(
            (entry) => entry.waitId === "wait-tmux",
        );
        const questionRecovery = snapshot.background?.find(
            (entry) => entry.waitId === "wait-question-resolved",
        );
    ...
    test("Workspace snapshot aggregates remote Context activity and approvals", async () => {
        ...
        await registry.attachEnvironment(context.ctxId!, {
            instance: "remote",
            workspace: "/remote",
        });
        Object.assign(fake.gateway, {
        ...
        assert.equal(snapshot.agentBusy, true);
        assert.deepEqual(
            snapshot.approvals?.map((approval) => approval.approvalId),
            ["approval-remote"],
        );
    });
    ...
    test("workspace_watch reconciles multi-instance Contexts within one second even on MCP fallback", async () => {
        ...
        assert.ok(result instanceof McpNativeToolResult);
        const update = result.structuredContent as {
            changed?: boolean;
            snapshot?: unknown;
        };
    ...
    test("Workspace Goal revision changes disable revision-only detached wait recovery", async () => {
        ...
        const wait = fake.waits.find(
            (entry) => entry.waitId === "wait-goal-revision",
        );
    ...
    test("Workspace Goal metadata revisions preserve progress-bound detached wait recovery", async () => {
        ...
        const wait = fake.waits.find(
            (entry) => entry.waitId === "wait-goal-progress-token",
        );
    ...
    test("Workspace can resume a blocked Goal through the app-only control", async () => {
        ...
        const wait = fake.waits.find(
            (entry) => entry.waitId === "wait-goal-resume",
        );
        assert.equal(wait?.automaticRecovery, false);
        assert.equal(typeof wait?.recoveryDisabledAt, "string");
    });
        ...
        try {
            ...
            const claim = (claimId: string) =>
                handler.call(
                    ...
                    claimed?: boolean;
                    claimId?: string;
                    delivery?: { kind?: string; sourceId?: string };
                    sourceKind?: string;
                }>;
            ...
            assert.equal(
                fake.waits.find((wait) => wait.waitId === "wait-once")?.status,
                "consumed",
            );
    ...
    test("server re-entry arbiter does not settle a Wait delivery when Wait consumption persistence fails", async () => {
        ...
        try {
            ...
            assert.equal(
                (
                    (await call("claim", { intent: "automatic" })) as {
                        claimed?: boolean;
                    }
                ).claimed,
                true,
            ...
            assert.equal(
                ((await call("validate")) as { valid?: boolean }).valid,
                true,
            );
            assert.equal(
                ((await call("attempt")) as { attempted?: boolean }).attempted,
                true,
            );
            await assert.rejects(
            ...
            assert.equal(
                fake.waits.find(
                    (wait) => wait.waitId === "wait-consume-failure",
                )?.status,
                "resolved",
            );
            const reentry = await registry.readAutomaticReentry(
                context.ctxId!,
                "demo",
            );
    ...
    test("server re-entry arbiter keeps its Context claim when releasing the Wait source fails", async () => {
        ...
        try {
            ...
            assert.equal(
                fake.waits.find(
                    (wait) => wait.waitId === "wait-release-failure",
                )?.recoveryClaimId,
                claimId,
            );
        } finally {
    ...
    test("server re-entry arbiter keeps both claims when source-attempt rollback fails", async () => {
        const root = await createTestTempDirectory(
            "reentry-wait-attempt-rollback-failure",
        );
        try {
            const registry = new McpContextRegistry({
                filePath: join(root, "contexts.json"),
            ...
            fake.waits.push({
                ...
                resolvedAt: now,
                result: {
                    task: { id: "task-attempt-rollback-failure", status: "0" },
                },
                status: "resolved",
            ...
            assert.equal(
                fake.waits.find(
                    (wait) => wait.waitId === "wait-attempt-rollback-failure",
                )?.recoveryClaimId,
                claimId,
            );
        } finally {
    ...
    test("server re-entry arbiter rolls back a Goal source claim when Context source binding fails", async () => {
        ...
        try {
            ...
            const goal = {
                ...
                status: "active" as const,
                steps: [
                    { id: "work", status: "active" as const, text: "Work" },
                ],
                updatedAt: now,
            };
            const actions: string[] = [];
            ...
            await assert.rejects(
                handler.call(
                    ...
                    {
                        action: "claim",
                        claimId: "claim-goal-bind-failure",
                        intent: "automatic",
                        token,
                    },
                    context,
            ...
            assert.equal(
                (await registry.readAutomaticReentry(context.ctxId!, "demo"))
                    .claimId,
                undefined,
            );
        } finally {
    ...
    test("server re-entry arbiter does not hide an accepted Goal report persistence failure", async () => {
        ...
        try {
            ...
            await assert.rejects(
                call("report", { outcome: "accepted" }),
                /goal report persistence failed/u,
            );
            const reentry = await registry.readAutomaticReentry(
                context.ctxId!,
                "demo",
    ...
    test("server re-entry arbiter reconciles a source-settled Wait after Context acceptance persistence fails", async () => {
        const root = await createTestTempDirectory(
            "reentry-context-accept-failure",
        );
        try {
            const registry = new McpContextRegistry({
                filePath: join(root, "contexts.json"),
            ...
            fake.waits.push({
                ...
                resolvedAt: now,
                result: {
                    task: { id: "task-context-accept-failure", status: "0" },
                },
                status: "resolved",
            ...
            const markAccepted =
                registry.markAutomaticReentryAccepted.bind(registry);
            Object.assign(registry, {
            ...
            await assert.rejects(
                call("report", { outcome: "accepted" }),
                /context acceptance persistence failed/u,
            );
            assert.equal(
                fake.waits.find(
                    (wait) => wait.waitId === "wait-context-accept-failure",
                )?.status,
                "consumed",
            );
            assert.equal(
            ...
            Object.assign(registry, {
                markAutomaticReentryAccepted: markAccepted,
            });
            await arbiter.observeExecutionStart(context.ctxId!);
            const reconciled = await registry.readAutomaticReentry(
                context.ctxId!,
                "demo",
            );
    ...
    test("server re-entry arbiter recovers a Goal whose validation cleared the source before Context release failed", async () => {
        const root = await createTestTempDirectory(
            "reentry-goal-validation-release-failure",
        );
        try {
            const registry = new McpContextRegistry({
                filePath: join(root, "contexts.json"),
                ...
                async goalContinuation(
                    ...
                    if (input.action === "release") {
                        if (!continuationPending)
                            throw new Error("goal claim is no longer active");
                        continuationPending = false;
                        return { goal: goalSnapshot(), released: true };
                    }
                    return { goal: goalSnapshot() };
            ...
            const releaseContext =
                registry.releaseAutomaticReentry.bind(registry);
            ...
            Object.assign(registry, {
                async releaseAutomaticReentry(
                    ...
                    if (failContextRelease)
                        throw new Error("context release persistence failed");
                    return await releaseContext(
                        ctxId,
                        instance,
                        requestedClaimId,
                    );
        ...
        try {
            ...
            Object.assign(fake.gateway, {
                async goalContinuation(
                    ...
                    if (input.action === "claim" && input.claimId !== undefined)
                        continuationClaims.push(input.claimId);
                    return {
    ...
    test("v0.6.15 Workspace wait sent then release stays idempotent on the current Wait model", async () => {
        ...
        assert.equal(
            fake.waits.find((wait) => wait.waitId === "wait-legacy")?.status,
            "consumed",
        );
        assert.deepEqual(
            await handler.callLegacyV0615(
    ...
    test("server re-entry arbiter consumes a resolved Wait without notification while Context execution is active", async () => {
        ...
        try {
            ...
            assert.equal(
                fake.waits.find((wait) => wait.waitId === "wait-busy")?.status,
                "consumed",
            );
        } finally {
            await rm(root, { force: true, recursive: true });
    ...
    test("server re-entry arbiter revalidates an associated task between claim and attempt", async () => {
        ...
        try {
            ...
            assert.equal(
                fake.waits.find((wait) => wait.waitId === "wait-task-fence")
                    ?.status,
                "consumed",
            );
            assert.equal(
    ...
    test("server re-entry arbiter rejects a claimed Wait after its Goal is replaced", async () => {
        ...
        try {
            ...
            assert.equal(validated.valid, false);
            assert.equal(
                fake.waits.find((wait) => wait.waitId === "wait-goal-fence")
                    ?.status,
                "consumed",
            );
        } finally {
    ...
    test("server re-entry arbiter gives one-shot Wait priority over repeatable Goal continuation", async () => {
        ...
        try {
            ...
            const goalSnapshot = () => ({
                autoContinueExhausted: false,
                continuationAttemptedAt: goalAttempted
            ...
            Object.assign(fake.gateway, {
                async goalContinuation(
                    ...
                    if (input.action === "claim") {
                        goalClaimCalls += 1;
                        if (goalClaimId !== undefined)
                            return { claimed: false, goal: goalSnapshot() };
                    ...
                    if (input.action === "validate") {
                        return {
                            goal: goalSnapshot(),
                            valid: goalClaimId === input.claimId,
                        };
                    }
                    if (input.action === "attempt") {
                        if (goalClaimId !== input.claimId)
                            return { attempted: false, goal: goalSnapshot() };
                        goalAttempted = true;
                        return {
                            attempted: true,
                            goal: goalSnapshot(),
                            messageId: `goal-message-${goalClaimId}`,
                        };
                    }
                    if (input.action === "report") {
                        if (goalClaimId !== input.claimId)
                            throw new Error("goal claim mismatch");
                    ...
                    if (input.action === "release") {
                        if (goalClaimId === input.claimId && !goalAttempted)
                            goalClaimId = undefined;
                        return { goal: goalSnapshot(), released: true };
                    }
            ...
            const waitClaim = await call("claim", "claim-wait", {
                intent: "automatic",
            });
            ...
            assert.equal(
                goalClaimCalls,
                0,
                "Goal must not claim while a one-shot Wait can notify",
            );
            ...
            assert.equal(
                fake.waits.find((wait) => wait.waitId === "wait-priority")
                    ?.status,
                "consumed",
            );

            const firstGoal = await call("claim", "claim-goal-1", {
                intent: "automatic",
            });
            ...
            assert.equal(
                (await call("attempt", "claim-goal-1")).attempted,
                true,
            );
            ...
            nowMs += 60_001;
            const secondGoal = await call("claim", "claim-goal-2", {
                intent: "automatic",
            });
            assert.equal(
                secondGoal.claimed,
                true,
                "Goal remains repeatable after the previous continuation lease expires",
            );
    ...
    test("Workspace tool metadata keeps the explicit reopen compatibility tool and app-only action tools", () => {
        ...
        const open = definitions.find(
            (definition) => definition.name === "workspace_open",
        );
        const answer = definitions.find(
            (definition) => definition.name === "workspace_answer",
        );
        const interrupt = definitions.find(
            (definition) => definition.name === "workspace_interrupt",
        );
        const recover = definitions.find(
            (definition) => definition.name === "workspace_recover",
        );
        const watch = definitions.find(
            (definition) => definition.name === "workspace_watch",
        );
        const reconnect = definitions.find(
            (definition) => definition.name === "workspace_reconnect",
        );
        const ask = definitions.find(
            (definition) => definition.name === "workspace_ask",
        );
        const goal = definitions.find(
            (definition) => definition.name === "workspace_goal",
        );
        const goalResume = definitions.find(
            (definition) => definition.name === "workspace_resume",
        );
        const goalStop = definitions.find(
            (definition) => definition.name === "workspace_stop",
        );
        ...
        assert.equal(
            definitions.some(
                (definition) => definition.name === "workspace_goal_continue",
            ),
            false,
        );
        ...
        assert.ok(goalStop);
        assert.deepEqual(
            (reconnect.inputSchema as { required?: string[] }).required,
            ["token"],
        );
        const adaptedOpen = adapter.toMcpTool(open, open.description);
        const adaptedAnswer = adapter.toMcpTool(answer, answer.description);
        const adaptedInterrupt = adapter.toMcpTool(
            interrupt,
            interrupt.description,
        );
        const adaptedWatch = adapter.toMcpTool(watch, watch.description);
        assert.equal(
            (adaptedOpen._meta as { ui?: { resourceUri?: string } })?.ui
                ?.resourceUri,
            workspaceAppResourceUri,
        );
        assert.equal(
            (adaptedOpen._meta as Record<string, unknown>)["ui/resourceUri"],
            workspaceAppResourceUri,
        );
        assert.equal(
            (adaptedOpen._meta as Record<string, unknown>)[
        ...
        assert.deepEqual(
            (adaptedAnswer._meta as { ui?: { visibility?: string[] } })?.ui
                ?.visibility,
            ["app"],
        );
        assert.deepEqual(
            (adaptedInterrupt._meta as { ui?: { visibility?: string[] } })?.ui
                ?.visibility,
            ["app"],
        );
        assert.deepEqual(
            (adaptedWatch._meta as { ui?: { visibility?: string[] } })?.ui
                ?.visibility,
            ["app"],
        );
        assert.deepEqual(
            (reconnect?._meta as { ui?: { visibility?: string[] } })?.ui
                ?.visibility,
            ["app"],
        );
        const recoveryInputSchema = recover.inputSchema as {
            properties?: {
                action?: { enum?: string[] };
                claimId?: unknown;
                recoveryMessageId?: unknown;
            };
            required?: string[];
        };
        assert.deepEqual(recoveryInputSchema.properties?.action?.enum, [
            "dismiss",
        ]);
        assert.equal(recoveryInputSchema.properties?.claimId, undefined);
        assert.notEqual(
            recoveryInputSchema.properties?.recoveryMessageId,
            undefined,
        );
        assert.deepEqual(recoveryInputSchema.required, [
            "action",
            "recoveryMessageId",
            "waitId",
            "token",
        ...
        assert.deepEqual(
            (goalResume._meta as { ui?: { visibility?: string[] } })?.ui
                ?.visibility,
            ["app"],
        );
        assert.deepEqual(
            (goalStop._meta as { ui?: { visibility?: string[] } })?.ui
                ?.visibility,
            ["app"],
        );

        const compatibilityDefinitions = new McpToolCatalogInteraction().list();
        const compatibilityOpen = compatibilityDefinitions.find(
            (definition) => definition.name === "workspace_open",
        );
        const compatibilitySnapshot = compatibilityDefinitions.find(
            (definition) => definition.name === "workspace_snapshot",
        );
        const compatibilityOpenSchema = compatibilityOpen?.outputSchema as {
            properties?: { ctxId?: unknown; tasks?: unknown };
        };
        const compatibilitySnapshotSchema =
            compatibilitySnapshot?.outputSchema as {
                properties?: {
                    ctxId?: unknown;
                    questions?: {
        ...
        assert.notEqual(
            compatibilitySnapshotSchema.properties?.ctxId,
            undefined,
        );
        assert.equal(
            compatibilitySnapshotSchema.properties?.tasks?.items?.properties
                ?.ctxId,
            undefined,
        );
        assert.equal(
            compatibilitySnapshotSchema.properties?.questions?.items?.properties
                ?.createdByCtxId,
            undefined,
        );
        assert.equal(
    ...
    async function openWorkspace(
        handler: McpEndpointHandlerInteraction,
    ): Promise<string> {
        const opened = await handler.call(
            "workspace_open",
            {},
        ...
        const gateway = {
            ...
            async waitForWait(_instance: string, waitId: string) {
                ...
                return await new Promise<WaitRecord>((resolve, reject) =>
                    pending.set(waitId, { reject, resolve }),
                );
            },
            async listWaits() {
            ...
            async reattachWait(
                ...
                if (record.status !== "detached")
                    throw new Error(`Wait ${waitId} is not detached.`);
            ...
            async claimWaitRecovery(
                ...
                if (
                    ...
                    throw new Error(
                        `Wait ${waitId} recovery is already claimed.`,
                    );
                }
                record.recoveryClaimId = claimId;
            ...
            async markWaitRecoveryAttempted(
                ...
                const record = requireWait(waits, waitId);
                if (record.recoveryClaimId !== claimId)
                    throw new Error(
                        `Wait ${waitId} recovery claim does not match.`,
                    );
                if (goalProgressEpoch !== undefined)
                    record.recoveryGoalProgressEpoch = goalProgressEpoch;
            ...
            async cancelWait(_instance: string, waitId: string) {
                ...
                pending
                    .get(waitId)
                    ?.reject(new Error(`Wait ${waitId} became cancelled.`));
                pending.delete(waitId);
                return structuredClone(record);
            },
            async detachWait(_instance: string, waitId: string) {
                ...
                if (record.status === "waiting") record.status = "detached";
                    ...
                    throw new Error(
                        `Wait ${waitId} cannot be detached while ${record.status}.`,
                    );
