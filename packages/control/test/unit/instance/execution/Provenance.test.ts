...
import { ToolCallProvenanceStore } from "../../../../src/instance/execution/tool/Provenance.ts";
import { createToolRouteModule } from "../../../../src/instance/execution/tool/Route.ts";
...
test("tool call provenance persists in Control and decorates matching Worker audit records", async () => {
    ...
    try {
        ...
        await store.record({
            callId: "call-1",
            explanation:
                "The previous test isolated the failure to Workspace reconnect handling.",
            instance: "alpha",
        ...
        assert.deepEqual(
            await store.decorate("alpha", [workerRecord, otherCall]),
            [
                {
                    ...workerRecord,
                    explanation:
        ...
        assert.deepEqual(await store.decorate("beta", [workerRecord]), [
            workerRecord,
        ]);
...
test("tool call provenance recovers a truncated hot JSONL tail after a crash", async () => {
    const root = await createTestTempDirectory(
        "tool-call-provenance-truncated-tail",
    );
    const filePath = join(root, "tool-call-provenance.jsonl");
    try {
        const store = new ToolCallProvenanceStore(filePath);
        ...
        assert.deepEqual(
            await recovered.decorate("alpha", [toolCall("call-1")]),
            [
                {
                    ...toolCall("call-1"),
                    purpose: "keep me",
        ...
        assert.deepEqual(
            await recovered.decorate("alpha", [toolCall("call-2")]),
            [
                {
                    ...toolCall("call-2"),
                    purpose: "still writable",
...
test("tool call provenance record does not initialize cold archives", async () => {
    const root = await createTestTempDirectory(
        "tool-call-provenance-lazy-cold",
    );
    const filePath = join(root, "tool-call-provenance.jsonl");
    ...
    try {
        ...
        await writeFile(
            join(archiveDirectory, "0000000000000-corrupt.jsonl.zst"),
            "not-zstd",
            "utf8",
        );
        const store = new ToolCallProvenanceStore(filePath);
        ...
        await assert.rejects(
            store.decorate("alpha", [toolCall("call-cold-miss")]),
        );
    } finally {
        await rm(root, { force: true, recursive: true });
    }
...
test("tool.listCalls merges Control provenance without changing Worker records", async () => {
    ...
    try {
        const store = new ToolCallProvenanceStore(
            join(root, "tool-call-provenance.jsonl"),
        );
        await store.record({
            callId: "call-1",
            instance: "alpha",
            purpose: "Review the failed command",
        });
        const workerRecord: ToolCallRecord = {
            callId: "call-1",
            inputSummary: "{}",
        ...
        const operation = module.operations.find(
            (entry) => entry.name === "listCalls",
        );
        if (operation === undefined)
            throw new Error("tool.listCalls operation is missing");
        const result = await operation.handle(
            { id: "1", name: "listCalls", payload: {} },
            {} as never,
        );
        assert.deepEqual(result, [
            { ...workerRecord, purpose: "Review the failed command" },
        ]);
...
test("tool call provenance rotates hot JSONL into compressed cold archives and reloads it", async () => {
    ...
    try {
        ...
        const archives = (await readdir(`${filePath}.archive`)).filter((name) =>
...
test("tool call provenance retention removes expired hot and cold records", async () => {
    const root = await createTestTempDirectory(
        "tool-call-provenance-retention",
    );
    ...
    try {
        ...
        assert.deepEqual(await cold.decorate("alpha", [toolCall("cold-old")]), [
            toolCall("cold-old"),
        ]);
        assert.deepEqual(await hot.decorate("alpha", [toolCall("hot-old")]), [
            toolCall("hot-old"),
        ]);
        ...
        assert.deepEqual(
            await coldReloaded.decorate("alpha", [toolCall("cold-old")]),
            [toolCall("cold-old")],
        );
        assert.deepEqual(
            await hotReloaded.decorate("alpha", [toolCall("hot-old")]),
            [toolCall("hot-old")],
        );
        assert.equal(
            (await readdir(`${coldPath}.archive`)).filter((name) =>
                name.endsWith(".jsonl.zst"),
            ).length,
            0,
        );
...
test("tool call provenance cold budget evicts oldest archives", async () => {
    ...
    try {
        ...
        const newestBytes = (await stat(join(archiveDirectory, names[1]!)))
            .size;
        ...
        assert.deepEqual(
            await bounded.decorate("alpha", [toolCall("call-old")]),
            [toolCall("call-old")],
        );
        assert.deepEqual(
            await bounded.decorate("alpha", [toolCall("call-new")]),
            [
                {
                    ...toolCall("call-new"),
                    purpose: "new archive",
        ...
        assert.equal(
            (await readdir(archiveDirectory)).filter((name) =>
                name.endsWith(".jsonl.zst"),
            ).length,
            1,
        );
