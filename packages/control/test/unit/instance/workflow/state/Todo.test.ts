...
import { TodoStore } from "../../../../../src/instance/workflow/todo/Store.ts";
...
test("TodoState owns validation, transitions, summaries, and associations", () => {
    ...
    assert.equal(
        (created.events[0]?.data as { ctxId?: string }).ctxId,
        "ctx-1",
    );
    ...
    assert.equal(
        "tasks" in state.readResult(created.document, { taskId: "task-fixed" }),
        false,
    );
    assert.equal(
        "tasks" in
            state.readResult(created.document, { taskId: "task-missing" }),
        false,
    );
    assert.deepEqual(
        state.readResult(created.document).tasks?.map((task) => task.taskId),
        ["task-fixed"],
    );
    assert.equal(
        state.activeSummaries(created.document)[0]?.status,
        "in_progress",
    );
    ...
    assert.deepEqual(
        state.activeSummaries(parallel).map((summary) => summary.title),
        ["Implement", "Verify"],
    );
...
test("TodoState reads and updates an existing task by stable taskId", () => {
    ...
    assert.equal(
        state.readResult(created.document, { taskId: "task-fixed" }).title,
        "Stable task",
    );
    const updated = state.transition(
        created.document,
        {
            revision: 1,
            taskId: "task-fixed",
    ...
    assert.deepEqual(
        state.readResult(updated.document, { taskId: "task-fixed" }).items,
...
test("TodoState persists checkpoints and task controls model re-entry without rewriting items", () => {
    ...
    assert.deepEqual(
        state.readResult(created.document, { taskId: "task-fixed" }).checkpoint,
        {
            next: "Continue implementation",
            summary: "Core path is ready",
    ...
    assert.equal(
        state.readResult(paused.document).tasks?.[0]?.status,
        "paused",
    );
    ...
    assert.equal(
        state.readResult(resumed.document).tasks?.[0]?.status,
        "in_progress",
    );
    ...
    assert.equal(
        state.readResult(updated.document, { taskId: "task-fixed" }).checkpoint
            ?.summary,
        "Core path is ready",
    );

    const cancelled = state.control(
        updated.document,
        "task-fixed",
        "cancel",
        "ctx-2",
    ...
    const archived = state.readResult(cancelled.document, {
        taskId: "task-fixed",
    });
...
test("TodoState exposes only actionable work as active todos", () => {
    ...
    const transition = (status: "cancelled" | "completed" | "failed") =>
        state.transition(
            state.emptyDocument(),
            {
                revision: 0,
...
test("TodoState lists live titles and isolates concurrent task revisions", () => {
    ...
    const first = state.transition(
        state.emptyDocument(),
        {
            ...
            title: "First",
            todos: [
                { content: "First work", id: "first", status: "in_progress" },
            ],
        },
        "ctx-first",
    );
    const second = state.transition(
        first.document,
        {
            ...
            title: "Second",
            todos: [
                { content: "Second work", id: "second", status: "pending" },
            ],
        },
        "ctx-second",
    );
    ...
    assert.deepEqual(
        state.readResult(second.document).tasks?.map((task) => task.title),
        ["First", "Second"],
    );
    assert.equal(state.readResult(second.document, "First").taskId, "task-1");
    assert.equal(
        state.currentAssociation(second.document, "ctx-first")?.taskId,
        "task-1",
    );
    assert.equal(
        state.currentAssociation(second.document, "ctx-second"),
        undefined,
    );
});
...
test("TodoState resolves a unique in-progress association across every task owned by one ctxId", () => {
    ...
    const pending = state.transition(
        state.emptyDocument(),
        {
            ...
            title: "Pending",
            todos: [
                { content: "Pending work", id: "pending", status: "pending" },
            ],
        },
        "ctx-shared",
    );
    const active = state.transition(
        pending.document,
        {
            ...
            title: "Active",
            todos: [
                { content: "Active work", id: "active", status: "in_progress" },
            ],
        },
        "ctx-shared",
    );
    ...
    const ambiguous = state.transition(
        active.document,
        {
            revision: 1,
            title: "Pending",
    ...
    assert.equal(
        state.currentAssociation(ambiguous.document, "ctx-shared"),
        undefined,
    );
});
...
test("TodoStore persists and reloads normalized state independently", async () => {
    ...
    assert.equal(
        state.readResult(reloaded.read(), "Persist").summary.currentItemId,
        "write",
    );
});
...
test("TodoStore discards obsolete comments while loading legacy documents", async () => {
    ...
    const version1 = new TodoStore({
        filePath: version1Path,
        instanceName: "aromatic-pc",
        state,
    }).read();
    assert.equal(version1.version, 4);
    assert.deepEqual(version1.active, [
        { ...version1State, title: "task-legacy" },
    ]);
    ...
    const version2 = new TodoStore({
        filePath: version2Path,
        instanceName: "aromatic-pc",
        state,
    }).read();
    ...
    assert.equal(
        "comments" in state.readResult(version2, "Legacy task"),
        false,
    );
    ...
    const version3Store = new TodoStore({
        filePath: version3Path,
        instanceName: "aromatic-pc",
        state,
    });
    const version3 = version3Store.read();
    assert.equal(version3.version, 4);
    assert.deepEqual(version3.active, [storedState]);
    assert.equal("comments" in version3, false);

    await version3Store.write(version3);
    const persisted = JSON.parse(
        await readFile(version3Path, "utf8"),
    ) as Record<string, unknown>;
    assert.equal(persisted.version, 4);
    assert.equal("comments" in persisted, false);

    const version4Path = join(root, "todo-v4.json");
    await writeFile(
        version4Path,
        JSON.stringify({
            active: [storedState],
    ...
    const version4 = new TodoStore({
        filePath: version4Path,
        instanceName: "aromatic-pc",
        state,
    }).read();
    assert.equal(version4.version, 4);
    assert.deepEqual(version4.active, [storedState]);
