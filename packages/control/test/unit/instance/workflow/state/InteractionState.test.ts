...
import { WaitState } from "../../../../../src/instance/workflow/wait/State.ts";
import { WaitStore } from "../../../../../src/instance/workflow/wait/Store.ts";
...
test("interaction state stores defer persisted state loading until first access", async () => {
    ...
    await Promise.all(
        Object.values(files).map(
            async (path) => await writeFile(path, "not-json\n", "utf8"),
        ),
    );
    ...
    const todo = new TodoStore({
        filePath: files.todo,
        instanceName: "alpha",
        state: new TodoState("alpha"),
    });
    const wait = new WaitStore({
        filePath: files.wait,
        instanceName: "alpha",
        state: new WaitState(),
    });
