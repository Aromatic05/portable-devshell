...
import { ExtensionAssetCapabilityControl } from "../../../../../src/control/extension/generation/capability/Resource.ts";
...
test("Extension assets install immutable content-addressed bundles from archives and directories", async (t) => {
    ...
    const archiveDigest = createHash("sha256")
        .update(await readFile(h.bundle))
        .digest("hex");
    ...
    assert.equal(
        await readFile(join(fromArchive.directory, "payload.txt"), "utf8"),
        "provider payload\n",
    );
    assert.equal(
        await readFile(join(fromDirectory.directory, "payload.txt"), "utf8"),
        "provider payload\n",
    );
    assert.deepEqual(
        await capability.resolveBundle(fromArchive.generation),
        fromArchive,
    );
    assert.deepEqual(
        (await capability.listBundles())
...
test("Extension assets refuse undeclared access and symlink sources", async (t) => {
    ...
    await assert.rejects(
        denied.installBundle(h.bundle),
        /did not declare the assets capability/u,
    );
    await assert.rejects(
        denied.installDirectory(h.source),
        /did not declare the assets capability/u,
    );
...
test("Extension assets remove and resolve only validated bundle generations", async (t) => {
    ...
    assert.equal(
        await capability.resolveBundle(installed.generation),
        undefined,
    );
    await assert.rejects(
        capability.removeBundle("../escape"),
        /Invalid Extension asset bundle generation/u,
    );
});
...
test("Extension assets project installed bundles into a named Worker resource collection", async (t) => {
    ...
    assert.deepEqual(
        await capability.projectBundle({
