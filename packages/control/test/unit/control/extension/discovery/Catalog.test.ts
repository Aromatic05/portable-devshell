...
import { ExtensionCatalog } from "../../../../../src/control/extension/generation/discovery/Catalog.ts";
...
test("Extension catalog validates domain declarations without runtime bindings", () => {
    ...
    const current = manifest("example", "a", {
        "cli.native-commands": [
            { id: "example", summary: "Run example", title: "Example" },
        ],
    ...
    assert.deepEqual(
        catalog.get("web.applications", "example-web")?.declaration,
        {
            id: "example-web",
            title: "Example Web",
        },
...
test("Extension catalog permits native builtin overlays and independent model commands with the same id", () => {
    ...
    assert.equal(
        catalog.get("cli.native-commands", "status")?.declaration.title,
        "Native status",
    );
    assert.equal(
        catalog.get("cli.model-commands", "status")?.declaration.title,
        "Model status",
    );
});
...
test("Extension catalog detects conflicts across inactive Extensions and keeps replacement atomic", () => {
    ...
    assert.equal(
        catalog.get("cli.native-commands", "shared")?.extensionId,
        "first",
    );
