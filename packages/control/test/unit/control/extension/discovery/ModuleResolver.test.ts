...
import { ExtensionHostModuleResolver } from "../../../../../src/control/extension/generation/discovery/ModuleResolver.ts";
...
test("Extension host module resolver falls back to shared application dependencies only for a live generation", async (t) => {
    ...
    const loadedCjs = await import(
        `${pathToFileURL(join(generation, "one.cjs")).href}?registered=1`
    );
    assert.equal(loadedCjs.default.value, "function");
    await assert.rejects(
        import(
            `${pathToFileURL(join(generation, "undeclared.mjs")).href}?registered=1`
        ),
        (error: unknown) =>
            error instanceof Error && error.message.includes("smol-toml"),
    );
    await assert.rejects(
        import(
            `${pathToFileURL(join(generation, "internal.mjs")).href}?registered=1`
        ),
        (error: unknown) =>
            error instanceof Error &&
            error.message.includes("@portable-devshell/control"),
    );
