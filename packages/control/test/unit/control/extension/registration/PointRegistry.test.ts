...
import { ExtensionPointRegistry } from "../../../../../src/control/extension/generation/registration/PointRegistry.ts";
    ...
    const registry = new ExtensionPointRegistry([
        {
            ...
            validateBinding(binding, context) {
                events.push(
                    `binding:${context.extensionId}:${context.id}:${String(binding)}`,
                );
            },
            async validateBindingResources(binding, context) {
                events.push(
                    `resources:${context.codeDirectory}:${context.id}:${String(binding)}`,
                );
            },
        },
    ]);
...
test("Extension Point registry rejects an unsupported point before owner validation", async () => {
    ...
    await assert.rejects(
        registry.validateBindingResources(
            "future.point",
            {},
            {
                codeDirectory: "/extension/code",
