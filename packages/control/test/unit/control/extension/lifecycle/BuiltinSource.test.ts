...
} from "../../../../../src/control/extension/install/BuiltinSource.ts";
...
test("builtin Extension source parser rejects malformed, relative, or duplicate identities", () => {
    const key = CONTROL_BUILTIN_EXTENSION_SOURCES_ENV;
    assert.throws(
        () => readBuiltinExtensionSources({ [key]: "{" }),
        /JSON array/u,
    );
    assert.throws(
        () =>
            readBuiltinExtensionSources({
                [key]: JSON.stringify([{ id: "skill", path: "relative" }]),
            }),
    ...
    assert.throws(
        () =>
            readBuiltinExtensionSources({
                [key]: JSON.stringify([{ id: "Bad_ID", path: "/one" }]),
            }),
    ...
    assert.throws(
        () =>
            readBuiltinExtensionSources({
                [key]: JSON.stringify({ id: "skill", path: "/one" }),
            }),
        /JSON array/u,
    );
});
