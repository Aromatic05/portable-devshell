...
import { ExtensionSandboxPointCodecRegistry } from "../../../../../src/control/extension/generation/sandbox/bridge/PointCodec.ts";
...
test("Extension sandbox codec registry delegates descriptor and invocation semantics", async () => {
    ...
    const registry = new ExtensionSandboxPointCodecRegistry([
        {
            describeBinding(binding, owner) {
                events.push(
                    `describe:${owner.extensionId}:${owner.id}:${String(binding)}`,
                );
                return { kind: "test" };
            },
            id: "test.point",
            async invokeBinding(binding, input, signal, owner) {
                events.push(
                    `invoke:${owner.extensionId}:${owner.id}:${String(binding)}:${String(input)}`,
                );
    ...
    assert.deepEqual(
        registry.describeBinding("test.point", "handler", context),
        { kind: "test" },
    );
    assert.deepEqual(
