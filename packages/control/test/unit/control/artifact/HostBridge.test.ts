...
import {
    ArtifactHostBridge,
    type ArtifactHostAccessContext,
} from "@portable-devshell/control/testing";
import { createTestTempDirectory } from "../../../../../../test/TestTempDirectory.ts";

function context(
    overrides?: Partial<ArtifactHostAccessContext>,
): ArtifactHostAccessContext {
    return {
        appendControlEvent: async (_type: string, _data?: JsonValue) =>
            undefined,
...
async function readPayload(
    endpoint: ReturnType<ArtifactHostBridge["endpointFor"]>,
    payloadId: string,
    bytes: number,
) {
    const chunks: Buffer[] = [];
...
test("host source snapshots arbitrary paths when security is disabled and persists across bridge restart", async (t) => {
    ...
    const reopenedBridge = new ArtifactHostBridge({
        homeDirectory,
        storageDir,
    });
...
test("host receive initialization cleans only its private staging directory", async (t) => {
    ...
    assert.equal(
        await readFile(join(downloadDirectory, "unrelated.txt"), "utf8"),
        "keep",
    );
    await assert.rejects(
        readFile(join(stagingDirectory, "orphan.payload"), "utf8"),
        { code: "ENOENT" },
    );
    t.after(() => rm(root, { force: true, recursive: true }));
});
...
test("host source workspace mode permits only a local provider workspace and rejects link escapes", async (t) => {
    ...
    const local = bridge.endpointFor(
        context({ provider: "local", securityMode: "workspace" }),
    );
    ...
    assert.equal(
        (
            await readPayload(
                local,
                inside.payloadId,
                inside.descriptor.payloadBytes,
            )
        ).toString(),
        "inside",
    );
    ...
    const remote = bridge.endpointFor(
        context({ provider: "ssh", securityMode: "workspace" }),
    );
    await assert.rejects(
...
test("host target redirects any path to a direct child of ~/Download", async (t) => {
    ...
    assert.equal(
        finished.targetPath,
        join(homeDirectory, "Download", "passwd"),
    );
...
test("host directory payload round-trips through tar.zst and restores executable mode", async (t) => {
    ...
    const archive = await readPayload(
        endpoint,
        opened.payloadId,
        opened.descriptor.payloadBytes,
    );
    ...
    assert.equal(
        await readFile(
            join(homeDirectory, "Download", "app", "index.html"),
            "utf8",
        ),
        "index",
    );
    if (process.platform !== "win32") {
        assert.equal(
            (
                await stat(
                    join(homeDirectory, "Download", "app", "assets", "app.sh"),
                )
            ).mode & 0o777,
            0o755,
        );
...
test("artifact service routes hidden host source and target with the real authority instance", async (t) => {
    ...
    const { ArtifactService } =
        await import("@portable-devshell/control/testing");
    const service = new ArtifactService({
        resolveEndpoint: (name, authorityInstance) => {
            authorities.push(authorityInstance);
            return name === "host" &&
                authorityInstance === authority.authorityInstance
    ...
    assert.equal(
        await readFile(join(homeDirectory, "Download", "copy.txt"), "utf8"),
        "host bridge",
    );
    assert.ok(authorities.length >= 2);
    assert.ok(
        authorities.every((value) => value === authority.authorityInstance),
    );
});
