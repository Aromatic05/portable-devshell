...
import {
    ...
    readShareId,
    readTransferId,
} from "../../../../src/control/artifact/route/Input.ts";
import {
    readMcpApprovalDecision,
    readMcpApprovalId,
} from "../../../../src/composition/mcp/route/Input.ts";
import { readReverseInstanceName } from "../../../../src/control/reverse/Input.ts";
import {
    limitRuntimeLogResponse,
    readRuntimeLogQuery,
    readRuntimeSubscriptionFromSeq,
} from "../../../../src/instance/execution/runtime/Route.ts";
import { readTodoSubscriptionFromSeq } from "../../../../src/instance/workflow/todo/Route.ts";
import {
    ...
    readToolCall,
    readToolCallQuery,
} from "../../../../src/instance/execution/tool/Input.ts";
...
test("MCP and reverse route inputs accept only their closed decision and identity contracts", () => {
    ...
    assert.equal(
        readReverseInstanceName({ instance: "reverse-one" }),
        "reverse-one",
    );
...
test("runtime route inputs clamp log queries and strictly validate subscription cursors", () => {
    assert.deepEqual(readRuntimeLogQuery(), {
        fromSeq: undefined,
        limit: 100,
        maxDecodedBytes: 1024 * 1024,
    });
    assert.deepEqual(readRuntimeLogQuery({ fromSeq: 10, limit: 0 }), {
        fromSeq: 10,
        limit: 1,
        maxDecodedBytes: 1024 * 1024,
    });
    assert.deepEqual(readRuntimeLogQuery({ fromSeq: 10, limit: 500 }), {
        fromSeq: 10,
        limit: 100,
        maxDecodedBytes: 1024 * 1024,
    });
    assert.deepEqual(readRuntimeLogQuery({ fromSeq: 10, limit: 1.5 }), {
        fromSeq: 10,
        limit: 100,
        maxDecodedBytes: 1024 * 1024,
    });
    assert.deepEqual(readRuntimeLogQuery({ maxDecodedBytes: 256 * 1024 }), {
        fromSeq: undefined,
        limit: 100,
        maxDecodedBytes: 256 * 1024,
    });
    assert.deepEqual(
        readRuntimeLogQuery({ maxDecodedBytes: 2 * 1024 * 1024 }),
        { fromSeq: undefined, limit: 100, maxDecodedBytes: 1024 * 1024 },
    );

    for (const cursor of [0, 1, Number.MAX_SAFE_INTEGER]) {
        assert.equal(
            readRuntimeSubscriptionFromSeq({ fromSeq: cursor }),
            cursor,
        );
    ...
    for (const cursor of [-1, 1.5, Number.NaN, Number.MAX_SAFE_INTEGER + 1]) {
        ...
        assertTargetInvalid(() =>
            readTodoSubscriptionFromSeq({ fromSeq: cursor }),
        );
    }
});
...
test("runtime log response limiting stays within one MiB and retains the newest fitting suffix", () => {
    ...
    assert.equal(
        Buffer.byteLength(JSON.stringify(limited), "utf8") <= 1024 * 1024,
        true,
    );
});
...
test("tool route inputs preserve call defaults, filters, and approval metadata", () => {
    assert.deepEqual(
        readToolCall({ toolName: "bash_run", workspace: "/workspace" }),
        {
            input: null,
            toolName: "bash_run",
            workspace: "/workspace",
    ...
    assert.equal(
        readToolApprovalId({ approvalId: "approval-1" }, "tool.getApproval"),
        "approval-1",
    );
    assert.deepEqual(readToolApprovalListOptions(), { pendingOnly: false });
    assert.deepEqual(readToolApprovalListOptions({ pendingOnly: true }), {
        pendingOnly: true,
    });
    assert.deepEqual(
        readToolApprovalDecision({
...
test("tool call responses stay bounded while preserving pagination direction", () => {
    ...
    assert.deepEqual(
        newest.map((record) => record.callId),
        ["call-2", "call-3"],
    );
    assert.equal(
        Buffer.byteLength(JSON.stringify(newest), "utf8") <= 8 * 1024 * 1024,
        true,
    );

    const forward = limitToolCallResponse(records, {
        after: "call-0",
        limit: 200,
    });
    assert.deepEqual(
        forward.map((record) => record.callId),
        ["call-1", "call-2"],
    );
    assert.equal(
        Buffer.byteLength(JSON.stringify(forward), "utf8") <= 8 * 1024 * 1024,
        true,
    );
