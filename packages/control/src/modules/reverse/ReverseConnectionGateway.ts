...
import { ReverseSseRpcChannel } from "./ReverseSseRpcChannel.js";
import { ReverseWebSocketRpcChannel } from "./ReverseWebSocketRpcChannel.js";
...
export class ReverseConnectionGateway {
    ...
    async #handleWebSocketUpgrade(
        ...
        try {
            ...
            this.#webSocketServer.handleUpgrade(request, socket, head, (webSocket) => {
                const channel = new ReverseWebSocketRpcChannel(webSocket);
    ...
    async #handleSse(request: IncomingMessage, response: ServerResponse): Promise<void> {
        try {
            ...
            const channel = new ReverseSseRpcChannel(response, readLastEventId(request));
    ...
    async #handleUpstream(request: IncomingMessage, response: ServerResponse): Promise<void> {
        try {
            ...
            if (
                ...
                !(active.channel instanceof ReverseSseRpcChannel)
            ) {
                throw createError({
