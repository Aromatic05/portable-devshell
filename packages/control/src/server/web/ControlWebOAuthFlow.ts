...
export class ControlWebOAuthFlow {
    ...
    get resourceUrl(): URL {
        return new URL(`${this.#publicBaseUrl}${this.#basePath}`);
    }
    ...
    async #callback(request: IncomingMessage, response: ServerResponse): Promise<void> {
        ...
        response.statusCode = 302;
        response.setHeader("Location", `${this.#publicBaseUrl}${this.#basePath}/`);
        response.setHeader("Cache-Control", "no-store");
        response.end();
    }

    get #redirectUri(): string {
        return `${this.#publicBaseUrl}${this.#basePath}/oauth/callback`;
    }
    ...
    async #discover(): Promise<OAuthEndpoints> {
        ...
        const issuer = stripTrailingSlash(this.#protectedResource.issuerUrl.href);
