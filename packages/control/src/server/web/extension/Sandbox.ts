...
export const webApplicationsSandboxCodec: ExtensionSandboxPointCodec =
    Object.freeze({
        describeBinding(
            binding: unknown,
            ...
            if (binding.source.kind === "files") {
                return Object.freeze({
                    directory: binding.source.directory,
                    kind: "files",
                });
            }
        ...
        async invokeBinding(
            ...
            if (binding.source.kind !== "endpoint") {
                throw new TypeError(
                    `Extension Web application ${context.id} is not endpoint-backed.`,
                );
            }
            const upstream = await binding.source.resolve();
            if (upstream !== undefined && !(upstream instanceof URL)) {
                throw new TypeError(
                    "Extension Web application endpoint resolve() must return a URL or undefined.",
                );
            }
