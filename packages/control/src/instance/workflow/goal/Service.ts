...
import { GoalStore } from "./storage/Store.js";

export class GoalService {
    readonly #appendEvent: (
        type: Extract<InstanceEventType, `goal.${string}`>,
        data: JsonValue,
    ) => Promise<void>;
        ...
        return await this.#runExclusive(async () => {
            ...
            for (const goal of [...document.goals]) {
                if (
                    ...
                    continue;
                const transition = this.#state.manage(
                    document,
                    { action: "stop" },
                    goal.createdByCtxId,
                );
                ...
                if (transition.result !== undefined)
                    stopped.push(transition.result);
            }
            return stopped;
        });
    }
    ...
    async recordReentry(ctxId: string, progressEpoch?: number): Promise<void> {
        await this.#runExclusive(async () => {
            const before = this.#store.read(ctxId);
            const transition = this.#state.reentry(
                before,
                ctxId,
                progressEpoch,
            );
    ...
    async touch(
        ...
        await this.#runExclusive(async () => {
            ...
            try {
                ...
                await this.#persistStructuralTouch(
                    ctxId,
                    kind,
                    this.#store.readStructural(),
                );
                return;
            }
            ...
            if (requiresStructuralTouch(current)) {
                await this.#persistStructuralTouch(
                    ctxId,
                    kind,
                    this.#store.read(ctxId),
                );
                return;
            }
            const transition = this.#state.touch(
                { goals: [current], version: 1 },
                ctxId,
                kind,
            );
    ...
    async #appendGoalEvent(
        ...
        await this.#appendEvent("goal.updated", {
            ctxId,
            ...(goalId === undefined || status === undefined
                ? {}
                : { goalId, status }),
        }).catch(() => undefined);
    }
