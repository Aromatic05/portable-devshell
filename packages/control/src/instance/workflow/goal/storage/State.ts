...
export class GoalState {
    ...
    manage(
        ...
        if (action === "get")
            return { document, result: this.read(document, ctxId) };
        if (action === "start") return this.#start(document, input, ctxId);

        const index = document.goals.findIndex(
            (goal) => goal.createdByCtxId === ctxId,
        );
        if (index === -1)
            throw new Error(
                "No Workspace Goal is attached to the current Context.",
            );
        const current = document.goals[index]!;
        if (
            input.expectedGoalId !== undefined &&
            input.expectedGoalId !== current.goalId
        ) {
            throw new Error(
                `Workspace Goal changed from ${input.expectedGoalId} to ${current.goalId}; refresh before retrying.`,
            );
        }
        if (
            ...
            throw new Error(
                `Workspace Goal ${current.goalId} changed from revision ${input.expectedRevision} to ${current.revision}; refresh before retrying.`,
            );
        }
        if (current.status === "completed" || current.status === "stopped") {
            throw new Error(
                `Workspace Goal ${current.goalId} is already ${current.status}.`,
            );
        }
        if (
            ...
            throw new Error(
                `Workspace Goal ${current.goalId} belongs to ${current.workspace}, not ${input.workspace}.`,
            );
        }
        if (
            ...
            throw new Error(
                "A paused Workspace Goal can only be resumed or stopped by the user.",
            );
        }
        ...
        switch (action) {
            ...
            case "block":
                if (current.status !== "active")
                    throw new Error(
                        "Only an active Workspace Goal can be blocked.",
                    );
                next = {
                ...
                break;
            case "pause":
                if (input.userControl !== true)
                    throw new Error(
                        "Workspace Goal pause is a user-control action.",
                    );
                if (current.status !== "active")
                    throw new Error(
                        "Only an active Workspace Goal can be paused.",
                    );
                next = {
                    ...clearContinuation(current),
                    continuationCount: 0,
            ...
            case "resume":
                if (current.status === "paused" && input.userControl !== true) {
                    throw new Error(
                        "A paused Workspace Goal requires an explicit user resume.",
                    );
                }
                if (
                    ...
                    throw new Error(
                        "Only a blocked or paused Workspace Goal can be resumed.",
                    );
                }
                next = {
                    ...clearContinuation(current),
            ...
            case "finish":
                if (
                    ...
                    throw new Error(
                        "Workspace Goal cannot finish until every step is completed or skipped.",
                    );
                }
                next = {
                    ...clearContinuation(current),
            ...
            default:
                throw new Error(
                    `Unsupported Workspace Goal action: ${String(action)}.`,
                );
        }
        ...
        goals[index] = next;
        return {
            document: this.compact({ goals, version: 1 }),
            result: snapshot(next, now),
        };
    }
    ...
    touch(
        document: GoalDocument,
        ctxId: string,
        kind: GoalActivityKind = "execution",
    ): GoalTransition {
        const index = document.goals.findIndex(
            (goal) => goal.createdByCtxId === ctxId,
        );
        ...
        const current = document.goals[index]!;
        if (current.status !== "active")
            return { document, result: snapshot(current, this.#now()) };
        const now = this.#now();
        ...
        const next = {
            ...
            lastAgentActivityAt: now,
            ...(executionEvidence
                ? { lastExecutionAt: now, noActionStreak: 0 }
                : {}),
        };
        const goals = [...document.goals];
        goals[index] = next;
    ...
    reentry(
        ...
        if (current.status !== "active")
            return { document, result: snapshot(current, this.#now()) };
        const now = this.#now();
        const coveredProgressEpoch = validateReentryProgressEpoch(
            current,
            progressEpoch,
        );
        if (
            current.reentryProgressEpoch !== undefined &&
            coveredProgressEpoch <= current.reentryProgressEpoch
        ) {
    ...
    continuation(
        ...
        if (index === -1) {
            if (input.action === "claim")
                return { document, result: { claimed: false, goal: null } };
            if (input.action === "validate")
                return { document, result: { goal: null, valid: false } };
            if (input.action === "retire")
                return { document, result: { goal: null, retired: false } };
            throw new Error(
                "No Workspace Goal is attached to the current Context.",
            );
        }
        ...
        if (input.goalId !== undefined && input.goalId !== current.goalId) {
            ...
            if (input.action === "claim")
                return { document, result: { claimed: false, goal } };
            if (input.action === "validate")
                return { document, result: { goal, valid: false } };
            if (input.action === "retire")
                return { document, result: { goal, retired: false } };
            throw new Error(
                `Workspace Goal changed from ${input.goalId} to ${current.goalId}; refresh before retrying.`,
            );
        }
        let next = current;
        ...
        if (input.action === "retire") {
            ...
            const eligible =
                input.userInitiated === true
                    ? current.status === "active" &&
                      hasActionableStep(current.steps) &&
            ...
            if (input.available === false || !eligible) {
                ...
                next = {
                    ...
                    continuationClaimProgressEpoch:
                        goalProgressEpoch(evaluated),
            ...
            const valid =
                ...
                claimMatches(current, claimId) &&
                current.status === "active" &&
                current.continuationClaimActivityAt ===
                    current.lastAgentActivityAt;
            if (!valid && claimMatches(current, claimId))
                next = clearContinuation(current);
            result = { goal: snapshot(next, now), valid };
        } else if (input.action === "attempt") {
            const claimId = requiredText(input.claimId, "claimId", 128);
            if (!claimMatches(current, claimId))
                throw new Error(
                    "Workspace Goal continuation claim is no longer active.",
                );
            if (input.available === false) {
                ...
                result = {
                    attempted: true,
                    goal: snapshot(next, now),
                    ...(next.continuationMessageId === undefined
                        ? {}
                        : { messageId: next.continuationMessageId }),
                };
            } else {
                result = {
                    attempted: true,
            ...
            if (typeof input.accepted !== "boolean")
                throw new Error(
                    "accepted must be a boolean for continuation report.",
                );
            if (
                input.accepted &&
                current.continuationAttemptedAt === undefined
            ) {
                throw new Error(
                    "Workspace Goal continuation was not marked attempted.",
                );
            }
            next = {
                ...clearContinuation(current),
                continuationCount:
                    current.continuationCount + (input.accepted ? 1 : 0),
                ...(input.accepted
                    ? {
                          continuationRetryAfter: undefined,
                          lastReentryAt: now,
                          reentryProgressEpoch:
                              current.continuationClaimProgressEpoch ??
                              goalProgressEpoch(current),
                      }
                    : {
                          continuationRetryAfter: new Date(
                              Date.parse(now) + GOAL_CONTINUATION_RETRY_MS,
                          ).toISOString(),
                      }),
            };
            ...
            if (current.continuationAttemptedAt !== undefined) {
                throw new Error(
                    "Attempted Workspace Goal continuation cannot be released.",
                );
            }
            next = clearContinuation(current);
            ...
            throw new Error(
                `Unsupported Workspace Goal continuation action: ${String(input.action)}.`,
            );
        }
    ...
    compact(
        document: GoalDocument,
        maxTerminalGoals = MAX_TERMINAL_GOALS,
    ): GoalDocument {
        ...
        return {
            goals: [...live, ...terminal].sort((left, right) =>
    ...
    #start(
        ...
        const existing = document.goals.find(
            (goal) => goal.createdByCtxId === ctxId,
        );
        if (
            existing !== undefined &&
            ...
            throw new Error(
                `Workspace Goal ${existing.goalId} is still ${existing.status}; finish or stop it first.`,
            );
        }
        const now = this.#now();
        ...
        const goals = document.goals.filter(
            (goal) => goal.createdByCtxId !== ctxId,
        );
        goals.push(next);
        return {
...
function updateGoal(
    ...
    if (input.stepId !== undefined) {
        ...
        if (index === -1)
            throw new Error(`Workspace Goal step ${stepId} was not found.`);
    ...
    if (
        ...
        throw new Error(
            "workspace_goal update requires objective, steps, stepId, or note.",
        );
    }
    const nextObjective =
        input.objective === undefined
    ...
    const progressed =
        ...
        goalStepsProgressed(current.steps, steps);
    ...
    return {
        ...
        steps,
        updatedAt: now,
        ...(current.workspace === undefined && input.workspace !== undefined
...
function normalizeSteps(
    ...
    if (value.length > GOAL_MAX_STEPS)
        throw new Error(`steps may contain at most ${GOAL_MAX_STEPS} entries.`);
    ...
    const steps = value.map((entry, index) => {
        ...
        if (ids.has(id))
            throw new Error(`Workspace Goal step id must be unique: ${id}.`);
        ids.add(id);
        return {
            id,
            ...
            status:
                entry.status === undefined
                    ? "pending"
                    : normalizeStepStatus(entry.status),
            text: requiredText(
                entry.text,
                `steps[${index}].text`,
                GOAL_STEP_TEXT_LIMIT,
...
function normalizeStepStatus(value: unknown): GoalStepStatus {
    if (
        ...
        return value;
    throw new Error(
        "Goal step status must be pending, active, completed, or skipped.",
    );
}
...
function normalizeStoredGoal(value: unknown): GoalRecord {
    ...
    const legacyAutoCompleted =
        storedStatus === "active" && allStepsTerminal(steps);
    const record: GoalRecord = {
        ...
        createdByCtxId: requiredStoredString(
            value.createdByCtxId,
            "createdByCtxId",
        ),
        goalId: requiredStoredString(value.goalId, "goalId"),
        lastAgentActivityAt: requiredStoredString(
            value.lastAgentActivityAt,
            "lastAgentActivityAt",
        ),
        ...(typeof value.lastControlAt === "string" &&
        value.lastControlAt.length > 0
            ? { lastControlAt: value.lastControlAt }
            : {}),
        ...(typeof value.lastExecutionAt === "string" &&
        value.lastExecutionAt.length > 0
            ? { lastExecutionAt: value.lastExecutionAt }
            : {}),
        ...(typeof value.lastReentryAt === "string" &&
        value.lastReentryAt.length > 0
            ? { lastReentryAt: value.lastReentryAt }
            : typeof value.lastContinuationAt === "string" &&
                value.lastContinuationAt.length > 0
              ? { lastReentryAt: value.lastContinuationAt }
              : {}),
        lastProgressAt:
            typeof value.lastProgressAt === "string" &&
            value.lastProgressAt.length > 0
                ? value.lastProgressAt
                : requiredStoredString(
                      value.lastAgentActivityAt,
                      "lastAgentActivityAt",
                  ),
    ...
    for (const key of [
        ...
        "continuationClaimActivityAt",
        "continuationClaimedAt",
        "continuationClaimId",
        "continuationMessageId",
        "continuationRetryAfter",
        "lastStreakEvaluatedReentryAt",
        "note",
    ] as const) {
    ...
    if (typeof value.reentryProgressEpoch === "number") {
        record.reentryProgressEpoch = nonNegativeInteger(
            value.reentryProgressEpoch,
            "reentryProgressEpoch",
        );
    ...
    if (typeof value.continuationClaimProgressEpoch === "number") {
        record.continuationClaimProgressEpoch = nonNegativeInteger(
            value.continuationClaimProgressEpoch,
            "continuationClaimProgressEpoch",
        );
    }
...
function snapshot(record: GoalRecord, now: string): GoalSnapshot {
    ...
    const executionMs =
        record.lastExecutionAt === undefined
            ? Date.parse(record.lastProgressAt)
            : Date.parse(record.lastExecutionAt);
    const controlMs =
        record.lastControlAt === undefined
            ? Number.NEGATIVE_INFINITY
            : Date.parse(record.lastControlAt);
    const reentryMs =
        record.lastReentryAt === undefined
            ? Number.NEGATIVE_INFINITY
            : Date.parse(record.lastReentryAt);
    const dueAtMs =
        Math.max(executionMs, controlMs, reentryMs) + GOAL_EXECUTION_LEASE_MS;
    const uncertain =
        record.continuationPending &&
        record.continuationAttemptedAt !== undefined;
    const claimFresh =
        uncertain ||
        (record.continuationPending &&
            record.continuationClaimedAt !== undefined &&
            nowMs - Date.parse(record.continuationClaimedAt) <
                GOAL_CONTINUATION_CLAIM_TTL_MS);
    const retryReady =
        record.continuationRetryAfter === undefined ||
        nowMs >= Date.parse(record.continuationRetryAfter);
    const progressEpoch = goalProgressEpoch(record);
    const reentryProgressEpoch =
        record.reentryProgressEpoch ??
        (record.lastReentryAt === undefined ? undefined : progressEpoch);
    const progressedSinceReentry =
        reentryProgressEpoch === undefined ||
        progressEpoch > reentryProgressEpoch;
    return {
        autoContinueExhausted: false,
        ...(record.continuationAttemptedAt === undefined
            ? {}
            : { continuationAttemptedAt: record.continuationAttemptedAt }),
        continuationCount: record.continuationCount,
        ...(record.continuationMessageId === undefined
            ? {}
            : { continuationMessageId: record.continuationMessageId }),
        continuationDue:
            record.status === "active" &&
            hasActionableStep(record.steps) &&
            progressedSinceReentry &&
            !claimFresh &&
            retryReady &&
            nowMs >= dueAtMs,
        ...
        ...(record.continuationRetryAfter === undefined
            ? {}
            : { continuationRetryAfter: record.continuationRetryAfter }),
        ...
        ...(record.lastReentryAt === undefined
            ? {}
            : {
                  lastReentryAt: record.lastReentryAt,
                  lastContinuationAt: record.lastReentryAt,
              }),
...
function evaluatePreviousReentry(record: GoalRecord): GoalRecord {
    ...
    if (
        reentryAt === undefined ||
        record.lastStreakEvaluatedReentryAt === reentryAt
    )
        return record;
...
function expireStaleClaim(record: GoalRecord, now: string): GoalRecord {
    ...
    if (
        Date.parse(now) - Date.parse(record.continuationClaimedAt) <
        GOAL_CONTINUATION_CLAIM_TTL_MS
    )
        return record;
...
function validateReentryProgressEpoch(
    record: GoalRecord,
    ...
    if (!Number.isSafeInteger(value) || value < 0 || value > current) {
        throw new Error(
            `Workspace Goal re-entry progress epoch ${value} is invalid for current epoch ${current}.`,
        );
    }
    return value;
}
...
function settleAttemptedReentry(record: GoalRecord, now: string): GoalRecord {
    ...
    const coveredProgressEpoch =
        record.continuationClaimProgressEpoch ?? goalProgressEpoch(record);
    const previousCovered = record.reentryProgressEpoch;
    return {
        ...clearContinuation(record),
        continuationCount: record.continuationCount + 1,
        ...(previousCovered !== undefined &&
        previousCovered >= coveredProgressEpoch
            ? {}
            : {
                  lastReentryAt: now,
                  reentryProgressEpoch: coveredProgressEpoch,
