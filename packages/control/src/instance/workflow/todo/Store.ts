...
export class TodoStore {
    ...
    readActive(): TodoDocument {
        return {
            active: structuredClone(this.#current().active),
            archived: [],
            version: 4,
        };
    }
    ...
    async update(
        operation: (
            document: TodoDocument,
        ) => TodoDocument | Promise<TodoDocument>,
    ...
    async transition<T>(
        operation: (document: TodoDocument) => {
            document: TodoDocument;
            result: T;
        },
    ): Promise<T> {
...
export class TodoState {
    ...
    normalizeDocument(value: unknown): TodoDocument {
        if (!isRecord(value) || !Array.isArray(value.archived)) {
            throw new Error(
                "todo document must contain a supported version and archived array",
            );
        }
        ...
        if (version === 1) {
            ...
            if (!Array.isArray(value.active)) {
                throw new Error(
                    `todo document version ${version} must contain an active array`,
                );
            }
            if (version === 4 && Object.hasOwn(value, "comments")) {
                throw new Error(
                    "todo document version 4 must not contain comments",
                );
            }
        ...
        const active = activeValues.map((entry) =>
            this.#normalizeStoredState(entry, version === 1),
        );
        const titles = new Set(active.map((entry) => entry.title));
        if (titles.size !== active.length)
            throw new Error("active todo titles must be unique");
        return this.compact({
            active,
            archived: value.archived.map((entry) =>
                this.#normalizeStoredState(entry, version === 1),
            ),
            version: 4,
        });
    ...
    transition(
        ...
        const previous =
            previousIndex === -1 ? undefined : document.active[previousIndex];
        ...
        if (previous === undefined) {
            if (normalized.taskId !== undefined) {
                throw invalidTodo(
                    `todo task ${normalized.taskId} was not found`,
                );
            }
            ...
            if (previous.title !== normalized.title) {
                throw invalidTodo(
                    `todo task ${previous.taskId} title is immutable`,
                );
            }
            ...
            events.push(
                todoEvent(
                    !isCompleted(previous) && isCompleted(next)
                        ? "todo.completed"
                        : "todo.updated",
                    next,
                ),
        ...
        if (isTerminal(next)) {
            ...
            const activeIndex = active.findIndex(
                (entry) => entry.taskId === next.taskId,
            );
    ...
    control(
        ...
        const previousIndex = document.active.findIndex(
            (entry) => entry.taskId === taskId,
        );
        if (previousIndex === -1) {
            const archived = document.archived.find(
                (entry) => entry.taskId === taskId,
            );
        ...
        if (
            ...
            throw invalidTodo(
                `todo task ${taskId} changed from revision ${expectedRevision} to ${previous.revision}; refresh before retrying`,
            );
        }
        if (action === "pause" && previous.pausedAt !== undefined)
            return { document, events: [] };
        if (action === "resume" && previous.pausedAt === undefined)
            return { document, events: [] };
        ...
        if (action === "cancel") {
            ...
            return {
                document: this.compact({ active, archived, version: 4 }),
                events: [
                    todoEvent("todo.updated", next),
                    todoEvent("todo.archived", archivedState),
                ],
            };
        }
    ...
    delete(document: TodoDocument, taskId: string): TodoTransition {
        ...
        return {
            document: {
                active: document.active.filter(
                    (entry) => entry.taskId !== taskId,
                ),
                archived: document.archived.filter(
                    (entry) => entry.taskId !== taskId,
                ),
                version: 4,
            },
        ...
        return document.active.flatMap((state) => {
            ...
            const current =
                summary.currentItemId === undefined
                    ? undefined
                    : state.items.find(
                          (item) => item.id === summary.currentItemId,
                      );
            return [
                {
    ...
    #createState(input: TodoWriteInput, ctxId: string): SharedTodoState {
        ...
        return {
            ...
            ...(input.checkpoint === undefined
                ? {}
                : { checkpoint: checkpoint(input.checkpoint, now) }),
            createdAt: now,
    ...
    #normalizeStoredState(
        ...
        const activeCtxId = optionalString(
            value.activeCtxId ?? value.activeSessionId,
        );
        ...
        const storedCheckpoint =
            value.checkpoint === undefined
                ? undefined
                : normalizeStoredCheckpoint(value.checkpoint);
        const pausedAt = optionalString(value.pausedAt);
        const taskId = requiredString(value.taskId, "taskId");
        const title =
            optionalString(value.title) ??
            (allowMissingTitle ? taskId : requiredString(value.title, "title"));
        ...
        return document.active.map((state) => {
            ...
            return {
                ...
                ...(state.checkpoint === undefined
                    ? {}
                    : { checkpoint: { ...state.checkpoint } }),
                ...(state.activeCtxId === undefined
                    ? {}
                    : { ctxId: state.activeCtxId }),
                currentItem: current?.content,
                ...(state.pausedAt === undefined
                    ? {}
                    : { pausedAt: state.pausedAt }),
...
function normalizeInput(input: TodoWriteInput): TodoWriteInput {
    if (!isRecord(input))
        throw invalidTodo("todo_write requires an object input");
    return {
        ...(input.checkpoint === undefined
            ? {}
            : { checkpoint: normalizeCheckpointInput(input.checkpoint) }),
...
function normalizeItems(value: unknown): TodoItem[] {
    ...
    if (value.length > TODO_MAX_ITEMS)
        throw invalidTodo(`todos may contain at most ${TODO_MAX_ITEMS} items`);
    const ids = new Set<string>();
    let inProgress = 0;
    return value.map((entry, index) => {
        if (!isRecord(entry))
            throw invalidTodo(`todos[${index}] must be an object`);
...
function normalizeTextArray(value: unknown, field: string): string[] {
    ...
    if (value.length > TODO_MAX_CHECKPOINT_BLOCKERS)
        throw invalidTodo(
            `${field} may contain at most ${TODO_MAX_CHECKPOINT_BLOCKERS} entries`,
        );
    return value.map((entry, index) =>
        normalizeText(entry, `${field}[${index}]`, TODO_MAX_TEXT_LENGTH),
    );
}
...
function deriveStateStatus(
    state: SharedTodoState,
): ActiveTodoSummary["status"] {
