...
export function createContextMessageRouteModule(service: Pick<ContextMessageService, "list" | "queue">): PrefixRouteModuleDefinition {
    return routeModule("contextMessage", {
        ...
        queue: async (request) => await service.queue(readQueueInput(request.payload ?? {})) as unknown as JsonValue
    });
}
