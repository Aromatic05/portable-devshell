...
import {
    ExtensionGeneration,
    type ExtensionGenerationLease,
} from "./generation/registration/Generation.js";
import {
    ExtensionCatalog,
...
import type { ExtensionRegistryPort } from "./state/Store.js";
...
export class ExtensionHost {
    ...
    acquire(id: string): ExtensionGenerationLease {
        if (this.#stopping)
            throw extensionFailure(
                id,
                undefined,
                new Error("Extension host is stopping."),
            );
        ...
        if (active.state === "faulted") {
            throw extensionFailure(
                ...
                active.generation,
                active.faultError ??
                    new Error(`Extension ${id} sandbox faulted.`),
            );
        }
    ...
    async acquireRegistration(
        ...
        registration: NonNullable<
            ReturnType<ExtensionGenerationLease["registrations"]["get"]>
        >;
    }> {
        ...
        if (
            active?.generation === catalog.generation &&
            active.state === "active"
        ) {
            return this.#acquireRegistrationFromGeneration(
                catalog.extensionId,
                active,
                pointId,
                id,
        ...
        return await this.#exclusive(async () => {
            ...
            if (current === undefined) {
                throw new Error(
                    `No Extension registration for ${pointId}/${id}.`,
                );
            }
            ...
            if (currentActive?.state === "faulted") {
                throw extensionFailure(
                    ...
                    currentActive.faultError ??
                        new Error(
                            `Extension ${current.extensionId} sandbox faulted.`,
                        ),
                );
            }
            if (
                currentActive?.generation === current.generation &&
            ...
            if (
                activated === undefined ||
                activated.generation !== resolved.generation
            ) {
    ...
    #acquireRegistrationFromGeneration(
        ...
        if (registration === undefined) {
            lease.release();
            throw new Error(
                `Extension registration disappeared during acquisition: ${pointId}/${id}.`,
            );
        }
            ...
            try {
                ...
                if (candidate.state === "faulted") {
                    throw (
                        ...
                        new Error(
                            `Extension generation ${generation} faulted during validation.`,
                        )
                    );
                }
            } catch (error) {
    ...
    async reload(id: string): Promise<void> {
        await this.#exclusive(async () => {
            ...
            if (generation === undefined)
                throw extensionInvalid(id, "has no selected generation");
            let candidate: ExtensionGeneration;
            try {
                candidate = await this.#loadCandidate(id, generation);
            } catch (error) {
    ...
    async waitForDrain(id: string): Promise<void> {
        ...
        const settled = await Promise.allSettled(
            retired.map(async (generation) => await generation.retire()),
        );
        ...
        if (failures.length > 0) {
            throw new AggregateError(
                failures,
                `Extension ${id} failed to drain cleanly.`,
            );
        }
    }
    ...
    async forget(id: string): Promise<void> {
        await this.#exclusive(async () => {
            this.#assertRunning();
            const snapshot = this.#requireRegistry();
            if (snapshot.extensions[id] === undefined)
            ...
            if (
                ...
                throw extensionInvalid(
                    id,
                    "still has active or draining generations",
                );
            }
            const next = cloneExtensionRegistry(snapshot);
        ...
        await Promise.all(
            [...this.#active.entries()].map(async ([id, generation]) => {
                try {
                    ...
                    failures.push(
                        new Error(
                            `Extension ${id} failed to retire resources for instance ${instance}.`,
                            { cause: error },
                        ),
                    );
                }
    ...
    async list(): Promise<ExtensionRuntimeRecord[]> {
        const snapshot =
            this.#registrySnapshot ?? (await this.#registry.read());
    ...
    async stop(): Promise<void> {
        ...
        if (failures.length > 0)
            throw new AggregateError(
                failures,
                "Extension generations failed to dispose cleanly.",
            );
    }
    ...
    async #catalogEntry(
        ...
        if (candidates.length === 0) {
            const error = new Error(
                `Extension ${id} has no selected generation.`,
            );
        ...
        const failure =
            selectedFailure ??
            new Error(`Extension ${id} has no usable manifest generation.`);
    ...
    async #activateCatalogEntry(id: string): Promise<void> {
        ...
        if (catalog === undefined)
            throw extensionNotActive(id, "not present in the static catalog");
        ...
        if (
            ...
            existing.generation === catalog.generation
        )
            return;

        const candidates = [
            ...new Set(
                [catalog.generation, entry.lastKnownGoodGeneration].filter(
    ...
    async #commitCandidate(
        ...
        try {
            ...
            throw new ExtensionCandidatePublicationError(
                candidate.generation,
                cause,
            );
        }
        ...
        if (candidate.state !== "active") {
            const failure =
                candidate.faultError instanceof Error
                    ? candidate.faultError
                    : new Error(
                          `Extension generation ${candidate.generation} faulted before publication.`,
                      );
            const rollbackFailures: unknown[] = [];
            await this.#registry
                .write(previousRegistry)
    ...
    #publish(id: string, candidate: ExtensionGeneration): void {
        if (candidate.state !== "active") {
            throw new Error(
                `Extension generation ${candidate.generation} is not publishable from ${candidate.state}.`,
            );
        }
    ...
    #record(id: string, entry: ExtensionRegistryEntry): ExtensionRuntimeRecord {
        const active = this.#active.get(id);
        const manifest =
            active?.manifest ?? this.#catalog.getExtension(id)?.manifest;
        const activeFailure =
            active?.state === "faulted"
                ? {
                      generation: active.generation,
                      message:
                          active.faultError instanceof Error
                              ? active.faultError.message
                              : String(
                                    active.faultError ??
                                        "Extension sandbox faulted.",
                                ),
                  }
                : undefined;
        const failure = activeFailure ?? this.#failures.get(id);
        const retired = [...(this.#retired.get(id) ?? [])].map(
            (generation) => ({
    ...
    #assertRunning(): void {
        if (!this.#started)
            throw extensionFailure(
                "host",
                undefined,
                new Error("Extension host has not started."),
            );
        if (this.#stopping)
            throw extensionFailure(
                "host",
...
class ExtensionCandidatePublicationError extends Error {
    constructor(generation: string, cause: unknown) {
        ...
        super(
            `Extension generation ${generation} failed before publication: ${detail}`,
            { cause },
        );
