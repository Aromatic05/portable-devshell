...
export class ExtensionInstallService {
    ...
    async #install(
        ...
        if (!isAbsolute(sourcePath)) {
            throw extensionInstallError(
                "Extension install source must be an absolute local path.",
            );
        }
        ...
        const canonicalArchive = join(
            this.#paths.codeRoot,
            `.staging-${transactionId}.dsext`,
        );
        ...
        try {
            const source = await lstat(sourcePath).catch((error: unknown) => {
                throw extensionInstallError(
                    "Extension install source is unavailable.",
                    error,
                );
            });
            if (source.isSymbolicLink()) {
                throw extensionInstallError(
                    "Extension install source must not be a symbolic link.",
                );
            }
            ...
            if (source.isDirectory()) {
                ...
                await createArtifactDirectoryArchive(
                    sourcePath,
                    canonicalArchive,
                );
                ownsArchive = true;
                ...
                if (source.size > this.#limits.maxCompressedBytes) {
                    throw extensionInstallError(
                        "Extension bundle exceeds the compressed byte limit.",
                    );
                }
                archivePath = sourcePath;
            } else {
                throw extensionInstallError(
                    "Extension install source must be a directory or .dsext archive.",
                );
            }
            ...
            await assertPlainDirectory(
                extensionDirectory,
                `Extension directory for ${manifest.id}`,
            );

            const existing = await lstat(installedDirectory).catch(
                (error: unknown) => {
            ...
            if (existing === undefined) {
                ...
                if (existing.isSymbolicLink() || !existing.isDirectory()) {
                    throw extensionInstallError(
                        `Extension generation path is not a directory: ${generation}.`,
                    );
                }
                const existingDigest = await hashExtensionDirectory(
                    installedDirectory,
                    this.#limits,
                );
                if (existingDigest !== digest) {
                    throw extensionInstallError(
                        `Extension generation ${generation} is corrupt or collides with different bytes.`,
                    );
                }
                await rm(stagingDirectory, { force: true, recursive: true });
            }

            const current = (await this.#host.list()).find(
            ...
            if (ownsArchive) {
                await rm(canonicalArchive, { force: true }).catch(
                    () => undefined,
                );
            }
        }
    ...
    async remove(id: string, purge = false): Promise<ExtensionRemoveResult> {
        ...
        await rm(join(this.#paths.codeRoot, id), {
            force: true,
            recursive: true,
        }).catch((error) => failures.push(error));
        await rm(join(this.#paths.runtimeRoot, id), {
            force: true,
            recursive: true,
        }).catch((error) => failures.push(error));
        if (purge) {
            await rm(this.#paths.stateDirectory(id), {
                force: true,
                recursive: true,
            }).catch((error) => failures.push(error));
            await rm(this.#paths.dataDirectory(id), {
                force: true,
                recursive: true,
            }).catch((error) => failures.push(error));
        }
        ...
        if (failures.length > 1) {
            throw new AggregateError(
                failures,
                `Extension ${id} was deregistered but filesystem cleanup was incomplete.`,
            );
        }
...
async function readStagedManifest(
    ...
    const metadata = await lstat(path).catch((error: unknown) => {
        throw extensionInstallError(
            "Extension bundle is missing devshell-extension.json.",
            error,
        );
    });
    if (metadata.isSymbolicLink() || !metadata.isFile()) {
        throw extensionInstallError(
            "Extension manifest must be a regular file.",
        );
    }
    try {
...
function assertInstallableManifest(
    manifest: ExtensionManifest,
    builtinId?: string,
): void {
    if (builtinId === undefined && BUILTIN_EXTENSION_IDS.has(manifest.id)) {
        throw extensionInstallError(
            `Extension id ${manifest.id} is reserved for a builtin Extension.`,
        );
    }
    if (builtinId !== undefined && manifest.id !== builtinId) {
        throw extensionInstallError(
            `Builtin Extension source declares id ${manifest.id}, expected ${builtinId}.`,
        );
    }
}

async function hashExtensionDirectory(
    root: string,
    limits: ExtensionInstallLimits,
): Promise<string> {
    ...
    const walk = async (directory: string, prefix: string): Promise<void> => {
        ...
        for (const child of children) {
            ...
            if (metadata.isSymbolicLink()) {
                throw extensionInstallError(
                    `Extension source contains symbolic link: ${relative}.`,
                );
            }
            entries += 1;
            if (
                ...
                throw extensionInstallError(
                    "Extension source exceeds the entry limit.",
                );
            }
            ...
            if (!metadata.isFile()) {
                throw extensionInstallError(
                    `Extension source contains unsupported member: ${relative}.`,
                );
            }
            if (
                ...
                throw extensionInstallError(
                    `Extension file exceeds the byte limit: ${relative}.`,
                );
            }
            ...
            if (
                ...
                throw extensionInstallError(
                    "Extension source exceeds the logical byte limit.",
                );
            }
            hash.update(
                `F\0${relative}\0${metadata.mode & 0o777}\0${metadata.size}\0`,
            );
            ...
            try {
                ...
                if (!current.isFile() || current.size !== metadata.size) {
                    throw extensionInstallError(
                        `Extension source changed while hashing: ${relative}.`,
                    );
                }
...
async function requireRuntimeRecord(
    host: ExtensionInstallHost,
    id: string,
): Promise<ExtensionRuntimeRecord> {
    ...
    throw extensionInstallError(
        `Extension ${id} activated but is missing from the runtime registry.`,
    );
}
