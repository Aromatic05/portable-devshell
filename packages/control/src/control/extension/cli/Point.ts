...
export const cliNativeCommandsExtensionPointDefinition: ExtensionPointDefinition =
    Object.freeze({
        ...
        parseDeclaration: (declaration: ExtensionPointDeclaration) =>
            parseCliCommandDeclaration(declaration, nativeCommands.id),
        validateBinding(
            binding: unknown,
...
export const cliModelCommandsExtensionPointDefinition: ExtensionPointDefinition =
    Object.freeze({
        createSandboxBinding: createCliModelSandboxBinding,
        id: modelCommands.id,
        parseDeclaration: (declaration: ExtensionPointDeclaration) =>
            parseCliCommandDeclaration(declaration, modelCommands.id),
        validateBinding(
            binding: unknown,
            context: ExtensionPointValidationContext,
        ) {
...
function parseCliCommandDeclaration(
    ...
    return Object.freeze({
        ...
        ...(record.summary === undefined
            ? {}
            : { summary: readString(record.summary, "summary", pointId) }),
        title: readString(record.title, "title", pointId),
        ...(record.usage === undefined
            ? {}
            : { usage: readString(record.usage, "usage", pointId) }),
    });
}
...
function readString(
    ...
    if (typeof value === "string" && value.length > 0 && value.trim() === value)
        return value;
    throw new TypeError(
        `${pointId} declaration ${field} must be a non-empty trimmed string.`,
    );
}
