...
export function parseExtensionRegistry(
    ...
    if (!isRecord(value))
        throw new TypeError("Extension registry must be an object.");
    assertOnlyKeys(value, ["extensions", "schemaVersion"]);
    if (value.schemaVersion !== EXTENSION_REGISTRY_SCHEMA_VERSION) {
        throw new TypeError(
            `Unsupported Extension registry schemaVersion: ${String(value.schemaVersion)}.`,
        );
    }
    if (!isRecord(value.extensions))
        throw new TypeError("Extension registry extensions must be an object.");
    ...
    for (const [id, rawEntry] of Object.entries(value.extensions)) {
        ...
        if (!isRecord(rawEntry))
            throw new TypeError(
                `Extension registry entry ${id} must be an object.`,
            );
        assertOnlyKeys(rawEntry, [
            "enabled",
        ...
        if (typeof rawEntry.enabled !== "boolean") {
            throw new TypeError(
                `Extension registry entry ${id}.enabled must be boolean.`,
            );
        }
        extensions[id] = {
            ...
            ...(rawEntry.lastKnownGoodGeneration === undefined
                ? {}
                : {
                      lastKnownGoodGeneration: readGeneration(
                          rawEntry.lastKnownGoodGeneration,
                          id,
                      ),
                  }),
            ...(rawEntry.selectedGeneration === undefined
                ? {}
                : {
                      selectedGeneration: readGeneration(
                          rawEntry.selectedGeneration,
                          id,
                      ),
                  }),
        };
...
export function cloneExtensionRegistry(
    snapshot: ExtensionRegistrySnapshot,
): ExtensionRegistrySnapshot {
    return {
        extensions: Object.fromEntries(
            Object.entries(snapshot.extensions).map(([id, entry]) => [
                id,
                { ...entry },
            ]),
        ),
...
function readGeneration(value: unknown, id: string): string {
    if (typeof value !== "string") {
        throw new TypeError(
            `Extension registry generation for ${id} must be a string.`,
        );
    }
...
function assertOnlyKeys(
    ...
    if (unknown.length > 0)
        throw new TypeError(`Unknown Extension registry field: ${unknown[0]}.`);
}
