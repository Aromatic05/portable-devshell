...
import { readExtensionDeclarations } from "../registration/Registration.js";
...
export class ExtensionCatalog {
    ...
    replace(
        ...
        this.#extensions.set(
            extensionId,
            Object.freeze({ generation, manifest }),
        );
        for (const registration of registrations) {
            this.#registrations.set(
                registrationKey(registration.pointId, registration.id),
                registration,
            );
        }
    }
    ...
    #prepare(
        ...
        const registrations = readExtensionDeclarations(
            manifest,
            this.#points,
        ).map((entry) =>
            Object.freeze({
        ...
        for (const registration of registrations) {
            const existing = this.#registrations.get(
                registrationKey(registration.pointId, registration.id),
            );
            if (existing === undefined || existing.extensionId === extensionId)
                continue;
            throw new TypeError(
