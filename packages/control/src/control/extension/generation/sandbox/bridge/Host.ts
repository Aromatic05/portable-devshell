...
export const DEFAULT_EXTENSION_SANDBOX_RESOURCE_LIMITS: Readonly<ResourceLimits> =
    Object.freeze({
...
export class ExtensionSandboxHost implements ExtensionPointSandboxBridge {
    ...
    constructor(options: ExtensionSandboxHostOptions) {
        ...
        this.#externalMemoryLimitBytes =
            positiveMegabytes(
                options.externalMemoryLimitMb ??
                    DEFAULT_EXTENSION_SANDBOX_EXTERNAL_MEMORY_LIMIT_MB,
        ...
        this.#hostCallbackTimeoutMs =
            options.hostCallbackTimeoutMs ?? DEFAULT_HOST_CALLBACK_TIMEOUT_MS;
        ...
        this.#worker = factory(sandboxWorkerModuleUrl(), {
            ...
            resourceLimits:
                ...
                DEFAULT_EXTENSION_SANDBOX_RESOURCE_LIMITS,
            workerData,
        });
        this.#ready = new Promise<ExtensionSandboxReadyDescriptor>(
            (resolve, reject) => {
        ...
        this.#worker.on("exit", (code) => {
            if (!this.#closing && this.#faulted === undefined) {
                this.#fault(
                    new Error(
                        `Extension sandbox worker exited unexpectedly with code ${code}.`,
                    ),
                );
            }
        });
    ...
    async invokeBinding(
        ...
        if (options.timeoutLabel !== undefined) {
            return await this.#invokeHostCallback(
                operation,
    ...
    async #accept(message: ExtensionSandboxToHostMessage): Promise<void> {
        switch (message.type) {
            ...
            case "capabilityCancel":
                this.#capabilityControllers
                    .get(message.id)
                    ?.abort(deserializeSandboxError(message.error));
                return;
            case "interfaceRequest": {
                const interfacePort = this.#invocationInterfaces.get(
                ...
                if (interfacePort === undefined) {
                    this.#send({
                        error: serializeSandboxError(
                            new Error(
                                "Extension sandbox interface invocation is unavailable.",
                            ),
                        ),
                        id: message.id,
                        type: "interfaceError",
                ...
                try {
                    const value = await interfacePort.request(
                        message.operation,
                        message.input,
                    );
                    this.#send({
    ...
    async #executeCapability(
        ...
        try {
            const value = await this.#dispatchCapability(
                id,
                operation,
                input,
                controller.signal,
    ...
    async #dispatchCapability(
        ...
        switch (operation) {
            case "artifacts.cancelTransfer":
                return await this.#artifactCapability.cancelTransfer(
                    readStringField(input, "transferId"),
                );
            case "artifacts.createShare":
                return await this.#artifactCapability.createShare(
                    input as SandboxArtifactShareInput,
                );
            case "artifacts.getTransfer":
                return await this.#artifactCapability.getTransfer(
                    readStringField(input, "transferId"),
                );
            ...
            case "artifacts.revokeShare":
                return await this.#artifactCapability.revokeShare(
                    readStringField(input, "shareId"),
                );
            case "artifacts.startTransfer":
                return await this.#artifactCapability.startTransfer(
                    input as SandboxArtifactTransferInput,
                );
            case "artifacts.waitForTransfer":
                return await this.#artifactCapability.waitForTransfer(
                    readStringField(input, "transferId"),
                );
            case "assets.installBundle":
                return await this.#assets.installBundle(
                    readStringField(input, "sourcePath"),
                );
            case "assets.installDirectory":
                return await this.#assets.installDirectory(
                    readStringField(input, "sourcePath"),
                );
            case "assets.listBundles":
                return await this.#assets.listBundles();
            case "assets.resolveBundle":
                return await this.#assets.resolveBundle(
                    readStringField(input, "generation"),
                );
            case "assets.removeBundle":
                await this.#assets.removeBundle(
                    readStringField(input, "generation"),
                );
                return undefined;
            ...
            case "instances.delete":
                return await this.#instanceCapability.delete(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.disable":
                return await this.#instanceCapability.disable(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.enable":
                return await this.#instanceCapability.enable(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.list":
            ...
            case "instances.readLogs": {
                ...
                return await this.#instanceCapability.readLogs(
                    value.name,
                    value.query,
                );
            }
            case "instances.refresh":
                return await this.#instanceCapability.refresh(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.snapshot":
                return await this.#instanceCapability.snapshot(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.start":
                return await this.#instanceCapability.start(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.stop":
                return await this.#instanceCapability.stop(
                    (input as SandboxInstanceNameInput).name,
                );
            case "instances.validateCreate":
                return await this.#instanceCapability.validateCreate(
                    (input as SandboxInstanceCreateInput).draft,
                );
            ...
            case "processes.start": {
                const started = await this.#processCapability.start(
                    input as SandboxProcessStartInput,
                );
                if (this.#faulted !== undefined || this.#closing) {
                    ...
                    throw (
                        this.#faulted ??
                        new Error(
                            "Extension sandbox stopped accepting processes.",
                        )
                    );
                }
                ...
                void started.closed
                    .then((exit) => {
                        if (this.#processes.get(processId) !== registration)
                            return;
                ...
                return {
                    processId,
                } satisfies ExtensionSandboxProcessDescriptor;
            }
            case "processes.send": {
            ...
            case "processes.terminate": {
                ...
                await this.#requireProcess(value.processId).terminate(
                    value.signal,
                );
                return undefined;
            }
            case "workers.openSession": {
                const opened = await this.#workerCapability.openSession(
                    input as SandboxWorkerOpenInput,
                );
                return await this.#registerWorkerSession(opened);
            }
            case "delegatedWorkers.openSession": {
                const opened =
                    await this.#delegatedWorkerCapability.openSession(
                        input as SandboxWorkerOpenInput,
                    );
                return await this.#registerWorkerSession(opened);
            }
    ...
    async #registerWorkerSession(
        opened: ExtensionWorkerSession,
    ): Promise<ExtensionSandboxWorkerSessionDescriptor> {
        if (
            this.#faulted !== undefined ||
            this.#closing ||
            this.#sessionCleanup !== undefined
            ...
            throw (
                this.#faulted ??
                new Error(
                    "Extension sandbox stopped accepting Worker sessions.",
                )
            );
        }
    ...
    #requireSession(id: string): ExtensionWorkerSession {
        ...
        throw new Error(
            `Extension sandbox Worker session is unavailable: ${id}.`,
        );
    }
    ...
    #requireProcess(id: string): ExtensionManagedProcess {
        ...
        throw new Error(
            `Extension sandbox managed process is unavailable: ${id}.`,
        );
    }
    ...
    #send(message: ExtensionHostToSandboxMessage): void {
        ...
        assertExtensionSandboxMessage(
            message,
            "Extension sandbox inbound message",
        );
    ...
    #fault(error: Error): void {
        ...
        for (const controller of this.#capabilityControllers.values())
    ...
    async #closeSessionsOnce(): Promise<void> {
        ...
        if (failures.length > 1) {
            throw new AggregateError(
                failures,
                "Extension sandbox Worker sessions failed to close.",
            );
        }
    }
    ...
    async #terminate(): Promise<void> {
        ...
        this.#port.close();
        this.#worker.unref();
        const termination = this.#worker.terminate().then(
            () => undefined,
            () => undefined,
        );
    ...
    async #checkExternalMemory(): Promise<void> {
        ...
        try {
            ...
            if (statistics.external_memory > this.#externalMemoryLimitBytes) {
                const observedMiB = Math.ceil(
                    statistics.external_memory / (1024 * 1024),
                );
                const limitMiB = Math.floor(
                    this.#externalMemoryLimitBytes / (1024 * 1024),
                );
                this.#fault(
                    new Error(
    ...
    #clearHealthProbe(): void {
        this.#healthProbeId = undefined;
        if (this.#healthDeadlineTimer !== undefined)
            clearTimeout(this.#healthDeadlineTimer);
...
function sourceSandboxExecArgv(): string[] {
    return [
        ...
        `--import=${new URL("../SourceLoader.mjs", import.meta.url).href}`,
    ];
}
...
function readStringField(input: unknown, field: string): string {
    if (typeof input !== "object" || input === null || Array.isArray(input)) {
        throw new TypeError(
            `Extension sandbox capability ${field} input must be an object.`,
        );
    }
    const value = (input as Record<string, unknown>)[field];
    if (typeof value !== "string" || value.length === 0) {
        throw new TypeError(
            `Extension sandbox capability ${field} must be a non-empty string.`,
        );
    }
    return value;
}
