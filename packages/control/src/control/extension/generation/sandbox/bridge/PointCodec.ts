...
export class ExtensionSandboxPointCodecRegistry {
    ...
    constructor(codecs: readonly ExtensionSandboxPointCodec[]) {
        for (const codec of codecs) {
            if (!/^[a-z][a-z0-9-]*(?:\.[a-z][a-z0-9-]*)+$/u.test(codec.id)) {
                throw new TypeError(
                    `Extension sandbox point id is invalid: ${codec.id}.`,
                );
            }
            if (this.#codecs.has(codec.id)) {
                throw new TypeError(
                    `Extension sandbox point codec ${codec.id} is registered more than once.`,
                );
            }
    ...
    describeBinding(
        ...
        return this.#require(pointId, context.extensionId).describeBinding(
            binding,
            context,
        );
    }
    ...
    async invokeBinding(
        ...
        return await this.#require(pointId, context.extensionId).invokeBinding(
            binding,
            input,
            signal,
            context,
    ...
    #require(pointId: string, extensionId: string): ExtensionSandboxPointCodec {
        ...
        throw new TypeError(
            `Extension ${extensionId} cannot sandbox unsupported Extension Point ${pointId}.`,
        );
    }
}
