...
import { createArtifactDirectoryArchive } from "./Archive.js";
import { artifactHashFile, createArtifactHasher } from "../Hash.js";
import type { ArtifactHostAccessContext } from "../Model.js";
...
export class ArtifactHostPayloadStore {
    ...
    constructor(options: {
        ...
        this.#maxActivePayloads =
            options.maxActivePayloads ?? DEFAULT_MAX_ACTIVE_PAYLOADS;
    ...
    async openPath(
        ...
        if (sourceMetadata.isSymbolicLink()) {
            throw artifactError(
                ...
                "Host source must not be a symbolic link.",
            );
        }
        ...
        try {
            if (sourceMetadata.isFile()) {
                const { blake3, bytes } = await copyAndHashFile(
                    sourcePath,
                    temporaryPath,
                );
                descriptor = {
                ...
                const manifest = await createArtifactDirectoryArchive(
                    sourcePath,
                    temporaryPath,
                );
    ...
    async read(
        ...
        try {
            const requested = Math.min(
                maxBytes,
                stored.descriptor.payloadBytes - offsetBytes,
            );
        ...
        await this.#runCountExclusive(async () => {
            ...
            if (this.#activePayloadCount >= this.#maxActivePayloads) {
                throw artifactError(
                    ...
                    "Host payload count limit exceeded.",
                );
            }
    ...
    async #releasePayloadSlot(): Promise<void> {
        await this.#runCountExclusive(() => {
            if (this.#activePayloadCount !== undefined) {
                this.#activePayloadCount = Math.max(
                    0,
                    this.#activePayloadCount - 1,
                );
            }
    ...
    async #load(payloadId: string): Promise<StoredPayload> {
        ...
        if (
            data === undefined ||
            !data.isFile() ||
            data.size !== value.descriptor.payloadBytes
        ) {
            throw artifactError(
                "artifact.contentUnavailable",
                "Host payload content is unavailable.",
            );
        }
        return value;
    }
...
async function resolveSourcePath(
    ...
    if (rawPath.length === 0) {
        throw artifactError(
            ...
            "Host source path must not be empty.",
        );
    }
    if (!isAbsolute(rawWorkspace)) {
        throw artifactError(
            "artifact.hostPathDenied",
            "Host workspace must be an absolute path.",
        );
    }
    const workspace = await realpath(rawWorkspace).catch((error: unknown) => {
        throw artifactError(
            "artifact.hostPathDenied",
            "Host workspace is unavailable.",
            error,
        );
    });
    const expanded =
    ...
    const requestedMetadata = await lstat(requested).catch((error: unknown) => {
        throw artifactError(
            "artifact.hostPathDenied",
            "Host source path is unavailable.",
            error,
        );
    });
...
async function copyAndHashFile(
    ...
    try {
        ...
        if (!sourceMetadata.isFile()) {
            throw artifactError(
                "artifact.directoryUnsafe",
                "Host source is not a regular file.",
            );
        }
    ...
    while (offset < bytes.length) {
        ...
        if (bytesWritten <= 0) {
            throw artifactError(
                ...
                "Host payload snapshot stopped making progress.",
            );
        }
        offset += bytesWritten;
    }
...
function requireName(path: string): string {
    ...
    if (name.length === 0 || name === "." || name === "..") {
        throw artifactError(
            "artifact.directoryUnsafe",
            "Host source has no usable file name.",
        );
    }
    return name;
}
