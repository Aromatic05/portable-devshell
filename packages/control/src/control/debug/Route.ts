...
export function createDebugRouteModule(
    port: DebugPatchPort,
    ...
    return routeModule("debug", {
        ...
        load: async (request, context) => {
            ...
            return (await port.load(
                readLoadRequest(request.payload),
            )) as unknown as JsonValue;
        },
        release: (request, context) => {
            requireLocalDebug(context);
            return port.release(
                readPatchId(request.payload, "debug.release"),
            ) as unknown as JsonValue;
        },
        ...
        unload: async (request, context) => {
            requireLocalDebug(context);
            return (await port.unload(
                readPatchId(request.payload, "debug.unload"),
            )) as unknown as JsonValue;
        },
    });
}
...
function requireLocalDebug(context: PrefixRouteContext): void {
    if (context.peer === "cli" && context.subject?.kind === "local-owner")
        return;
    throw createError({
        code: errorCodes.controlDebugAccessDenied,
        message:
            "Debug patch operations are restricted to the local owner CLI.",
        retryable: false,
    });
}
...
function readLoadRequest(
    ...
    if (
        ...
        throw invalid(
            "debug.load name must be a non-empty string when supplied.",
        );
    }
    const scope =
    ...
    if (
        scope !== undefined &&
        (typeof scope.ctxId !== "string" || scope.ctxId.length === 0)
    ) {
    ...
    if (
        ...
        throw invalid(
            "debug.load scope toolName must be a non-empty string when supplied.",
        );
    }
    return {
        ...
        ...(scope === undefined
            ...
            : {
                  scope: {
                      ...
                      ...(scope.toolName === undefined
                          ? {}
                          : { toolName: scope.toolName as string }),
                  },
              }),
        source: value.source,
        target: value.target,
