    ...
    decode(document: ConfigTomlDocument): ConfigGlobalDraft {
        ...
        if (version !== 1 && version !== 2) {
            throw configInputError(
                "parse",
                ["version"],
                "config.document.versionUnsupported",
                "must be 1 or 2",
            );
        }
    ...
    encode(config: ControlGlobalConfig): ConfigTomlDocument {
        return compact({
            ...
            web: {
                ...
                oauth2:
                    config.web.auth.mode === "oauth2"
                        ? compact(config.web.auth.oauth2)
                        : undefined,
                publicBaseUrl: config.web.publicBaseUrl,
                token:
                    config.web.auth.mode === "token"
                        ? config.web.auth.token
                        : undefined,
            },
        });
    }
...
export class ControlInstanceTomlDocument {
    decode(document: ConfigTomlDocument): ConfigInstanceDraft {
        ...
        rejectLegacyField(
            record,
            "sshBinary",
            "is not supported; use ssh.command",
        );
        if (version === 3) {
            rejectLegacyField(
                record,
                "workspace",
                "is not supported; select an absolute workspace when creating an environment context",
            );
        }
            ...
            mcp: {
                ...
                oauth2:
                    instance.mcp.auth.mode === "oauth2"
                        ? compact(instance.mcp.auth.oauth2)
                        : undefined,
                path: instance.mcp.path,
                token:
                    instance.mcp.auth.mode === "token"
                        ? instance.mcp.auth.token
                        : undefined,
            },
            logs: instance.logs,
