...
mod windows_tests {
    ...
    #[test]
    fn real_conpty_replays_output_resizes_and_kills_explicitly() {
        let workspace = crate::testing::temp_dir();
        let manager = TerminalManager::with_limits(
            workspace.path().to_path_buf(),
            1024 * 1024,
        ...
        loop {
            let current = manager
                ...
                .expect("read ConPTY exit state");
            if current.exit.is_some() {
                break;
            }
            assert!(
                Instant::now() < deadline,
                "ConPTY terminal did not exit after kill"
            );
    ...
    fn wait_for_output(
        ...
        loop {
            ...
            assert!(
                Instant::now() < deadline,
                "missing ConPTY terminal output: {expected}"
            );
            std::thread::sleep(Duration::from_millis(20));
        }
    }
