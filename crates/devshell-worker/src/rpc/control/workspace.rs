...
use crate::storage::{
    devshell_home,
    permissions::{ensure_dir, ensure_file_mode},
};
...
fn prepare(workspace: &Path) -> Result<WorkspacePrepareResult, RpcError> {
    ...
    let home =
        devshell_home().map_err(|error| RpcError::new("workspace.storageUnavailable", error))?;
...
fn touch_context_temporary_directory(path: &Path) -> Result<(), RpcError> {
    let home =
        devshell_home().map_err(|error| RpcError::new("workspace.temporaryUnavailable", error))?;
