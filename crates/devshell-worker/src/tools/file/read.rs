...
impl ToolHandler for FileReadTool {
    ...
    fn catalog_entry(&self) -> ToolCatalogEntry {
        ToolCatalogEntry {
            ...
            description: "Read UTF-8 text files with anchored line numbers.".to_string(),
    ...
    fn call(&self, call: ToolCall) -> Result<serde_json::Value, ToolError> {
        ...
        let ranges = if default_read && !text.lines.is_empty() {
            vec![(1, text.lines.len().min(200))]
        } else {
            ranges
        };
        ...
        serde_json::to_value(FileReadOutput {
            ...
            truncated: default_read && text.lines.len() > 200,
            next_start_line: if default_read && text.lines.len() > 200 {
                Some(201)
            } else {
                None
            },
...
fn validate_ranges(
    ranges: Option<&[crate::tools::file::types::LineRange]>,
    total: usize,
    ...
    if ranges.len() > 16 {
        return Err(ToolError::new(
            "file.invalidRange",
            "at most 16 ranges are allowed",
        ));
    }
    let mut result = Vec::new();
    let mut previous_end = 0;
    for range in ranges {
        if range.start_line == 0 || range.line_count == 0 || range.start_line <= previous_end {
            return Err(ToolError::new(
                "file.invalidRange",
                "ranges must be positive, sorted, and non-overlapping",
            ));
        }
        let end = range
            .start_line
            .saturating_add(range.line_count - 1)
            .min(total);
        if range.start_line <= total {
            result.push((range.start_line, end));
            previous_end = end;
        }
    }
    Ok(result)
