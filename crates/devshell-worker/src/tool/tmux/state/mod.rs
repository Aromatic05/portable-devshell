...
use crate::tool::tmux::state::replay::ReplayCache;
use crate::tool::tmux::state::task::{
...
use crate::tool::tmux::transcript::TranscriptCursor;
    ...
    fn run_once(&self, call: &ToolCall, params: TmuxRunParams) -> Result<TmuxRunOutput, ToolError> {
        ...
        let task_started_at;
        {
            ...
            if let Err(error) = verify_pane_cwd(&pane, &cwd) {
                ...
                let _ = crate::tool::tmux::transcript::ring::remove(
...
mod tests {
    ...
    use crate::instance::sandbox::path::{parse_requested_path, resolve_existing_target};
    use crate::tool::tmux::backend::BackendPane;
