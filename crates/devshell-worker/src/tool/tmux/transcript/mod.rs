...
use self::ring::RingWriter;
use super::warning;

use crate::tool::ToolError;
use crate::tool::tmux::model::TmuxWarning;
...
impl TranscriptCursor {
    ...
    fn read_segment(&self, offset: u64) -> Result<TranscriptSegment, ToolError> {
        let Some(ring) = ring::snapshot(&self.ring_name)
    ...
    fn logical_end(&self) -> Result<u64, ToolError> {
        Ok(ring::snapshot(&self.ring_name)
            .map_err(transcript_error)?
...
mod tests {
    ...
    use crate::tool::tmux::transcript::ring::{RingWriter, remove, snapshot};
    use uuid::Uuid;
