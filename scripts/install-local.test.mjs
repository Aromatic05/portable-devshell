...
const CURRENT_DEVELOPMENT_VERSION = "0.7.6"; // version-state:current-development
...
test(
    ...
    async () => {
        ...
        const activatedVersionDirectory = resolve(
            installRoot,
            "versions",
            "9.8.7-activated",
        );
        const runningVersionDirectory = resolve(
            installRoot,
            "versions",
        ...
        try {
            await mkdir(resolve(activatedVersionDirectory, "custom"), {
                recursive: true,
            });
            await mkdir(resolve(runningVersionDirectory, "test-runtime"), {
                recursive: true,
            });
            ...
            await writeFile(
                resolve(activatedVersionDirectory, "package.json"),
                `${JSON.stringify({
            ...
            const result = await runProcess(
                process.execPath,
                [resolve(repositoryRoot, "scripts", "install-local.mjs")],
                {
                    ...process.env,
                    HOME: home,
...
test(
    ...
    async () => {
        ...
        try {
            ...
            await mkdir(resolve(activatedVersionDirectory, "test-runtime"), {
                recursive: true,
            });
            await mkdir(resolve(devshellHome, "control"), { recursive: true });
            await mkdir(workerBinDirectory, { recursive: true });
...
test(
    ...
    async () => {
        ...
        try {
            ...
            const recoveryDirectories = (await readdir(installRoot)).filter(
                (name) => name.startsWith(".worker-activation-backup-"),
            );
            assert.equal(recoveryDirectories.length, 1);
            assert.equal(
                await readFile(
                    resolve(installRoot, recoveryDirectories[0], "host-worker"),
                    "utf8",
                ),
                "old-worker\n",
            );
