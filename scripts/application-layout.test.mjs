...
test("CLI argument normalization removes only the pnpm separator", () => {
    ...
    assert.deepEqual(normalizeCliArguments(["status"]), ["status"]);
    assert.deepEqual(normalizeCliArguments(["instance", "status", "alpha"]), [
        "instance",
        "status",
        "alpha",
    ]);
...
test("application materialization breaks deploy hardlinks and dereferences symlinks", async () => {
    ...
    try {
        ...
        await mkdir(resolve(deploy, "node_modules", "pkg"), {
            recursive: true,
        });
        ...
        if (process.platform !== "win32") {
            await symlink(
                "index.js",
                resolve(deploy, "node_modules", "pkg", "alias.js"),
            );
        }
...
test("package bin resolver rejects missing, absolute, and escaping entries", () => {
    ...
    assert.throws(
        () =>
            resolvePackageBinPath(
                "/app",
                { name: "cli", bin: { devshell: "/tmp/cli.js" } },
                "devshell",
            ),
        /must be relative/u,
    );
            ...
            JSON.stringify({
                ...
                dependencies: {
                    "@portable-devshell/control":
                        "@portable-devshell/control@file:///build/portable-devshell/packages/control",
                },
            }),
            "utf8",
        );
