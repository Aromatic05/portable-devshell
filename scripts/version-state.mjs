...
export function parseVersion(value) {
    ...
    if (match === null) {
        throw new Error(
            `version must use x.y.z without a prefix or prerelease suffix: ${value}`,
        );
    }
...
export async function readProjectVersions(root = defaultRepoRoot) {
    ...
    const cargoToml = await readFile(
        resolve(root, "crates/devshell-worker/Cargo.toml"),
        "utf8",
    );
...
export async function setProjectVersion(version, root = defaultRepoRoot) {
    ...
    const installLocalTestPath = resolve(
        root,
    ...
    await writeFile(
        cargoLockPath,
        replaceCargoLockVersion(cargoLock, version),
        "utf8",
    );
    await writeFile(
        installLocalTestPath,
        replaceCurrentDevelopmentVersion(installLocalTest, version),
        "utf8",
    );
}
...
export async function checkDevelopmentVersion(root = defaultRepoRoot) {
    ...
    if (
        ...
        compareVersions(current, latestRelease) <= 0
    ) {
        throw new Error(
...
export async function checkReleaseVersion(tag, root = defaultRepoRoot) {
    ...
    if (tag !== `v${current}`) {
        throw new Error(
            `release tag ${tag} does not match synchronized project version ${current}`,
        );
    }
    return current;
}
...
export async function advanceAfterRelease(tag, root = defaultRepoRoot) {
    ...
    if (comparison < 0) {
        throw new Error(
            `default branch version ${current} is behind released version ${released}`,
        );
    }
    if (comparison > 0) {
...
function readLatestReleaseVersion(root) {
    ...
    try {
        ...
        throw new Error(
            `unable to read release tags: ${error instanceof Error ? error.message : String(error)}`,
        );
    }
...
function readCargoPackageVersion(content, source) {
    ...
    const version =
        packageBlock === undefined
            ? undefined
            : /^version = "([^"]+)"$/mu.exec(packageBlock)?.[1];
...
function readCargoLockVersion(content) {
    ...
    if (version === undefined)
        throw new Error("Cargo.lock devshell-worker version is missing");
...
function replaceCargoPackageVersion(content, version) {
    ...
    if (
        packageBlock === undefined ||
        !/^version = "[^"]+"$/mu.test(packageBlock)
    ) {
        throw new Error("Cargo.toml package version is missing");
    }
    return content.replace(
        packageBlockPattern,
        packageBlock.replace(/^version = "[^"]+"$/mu, `version = "${version}"`),
    );
}
...
function replaceCurrentDevelopmentVersion(content, version) {
    const pattern =
        /(const CURRENT_DEVELOPMENT_VERSION = ")[^"]+("; \/\/ version-state:current-development)/u;
    if (!pattern.test(content))
        throw new Error(
            "scripts/install-local.test.mjs current development version marker is missing",
        );
    return content.replace(pattern, `$1${version}$2`);
}
...
async function main(args) {
    ...
    switch (command) {
        case "set":
            if (value === undefined)
                throw new Error("usage: version-state.mjs set <x.y.z>");
        ...
        case "check-release":
            if (value === undefined)
                throw new Error(
                    "usage: version-state.mjs check-release <vX.Y.Z>",
                );
        ...
        case "advance-after-release": {
            if (value === undefined)
                throw new Error(
                    "usage: version-state.mjs advance-after-release <vX.Y.Z>",
                );
            const result = await advanceAfterRelease(value, root);
            process.stdout.write(
                `${result.changed ? "advanced" : "already advanced"} ${result.version}\n`,
            );
            return;
        }
        default:
