...
export function runDevTagGate(options = {}) {
    ...
    if (platform !== "linux" || arch !== "x64") {
        throw new Error(
            "dev tag push gate requires a Linux x64 host so it can reproduce Linux x64 development CI.",
        );
    }
...
function validateCommitSha(commitSha) {
    if (!/^[0-9a-f]{40}$/u.test(commitSha)) {
        throw new Error(
            "dev tag gate proof requires a full 40-character commit SHA.",
        );
    }
}
...
if (
    ...
    try {
        ...
        const proofDirectory = gitOutput([
            "rev-parse",
            "--git-path",
            "portable-devshell/dev-tag-gate",
        ]);
        if (mode === "--check") {
            if (
                commitSha === undefined ||
                !hasDevTagGateProof(commitSha, proofDirectory)
            ) {
                throw new Error(
                    `dev tag push blocked: commit ${commitSha ?? "<missing>"} has not passed the local Linux x64 development gate. Run: node scripts/run-dev-tag-gate.mjs`,
                );
            }
        } else if (mode !== undefined) {
            throw new Error(
                "usage: run-dev-tag-gate.mjs [--check <commit-sha>]",
            );
        } else {
            ...
            if (!result.ok) {
                ...
                if (currentCommit !== initialCommit || !isWorktreeClean()) {
                    throw new Error(
                        "dev tag gate passed, but HEAD or the working tree changed while it was running; proof was not recorded.",
                    );
                }
                writeDevTagGateProof(initialCommit, proofDirectory);
                console.log(
                    `Recorded local dev tag gate proof for ${initialCommit}.`,
                );
            }
        }
    } catch (error) {
