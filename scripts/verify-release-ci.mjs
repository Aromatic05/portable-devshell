...
export async function fetchRepositoryWorkflowRuns({
    ...
    for (let page = 1; page <= 5; page += 1) {
        ...
        if (!response.ok) {
            throw new Error(
                `GitHub Actions lookup failed with HTTP ${response.status}: ${await response.text()}`,
            );
        }
        const payload = await response.json();
        if (!Array.isArray(payload.workflow_runs)) {
            throw new Error(
                "GitHub Actions response did not contain workflow_runs.",
            );
        }
        ...
        for (const workflow of result.workflows) {
            ...
            if (workflow.successful !== undefined) {
                process.stderr.write(
                    `${workflow.label}: passed via ${workflow.successful.head_branch} (${workflow.successful.html_url})\n`,
                );
                ...
                process.stderr.write(
                    `${workflow.label}: no dev-tag run found for ${commitSha}.\n`,
                );
            }
        }
        throw new Error(
            `Release blocked: the commit ${commitSha} has not passed every required development CI workflow.`,
        );
    }
