...
if (process.platform === "win32") {
    throw new Error(
        "smoke-agent-package.mjs currently validates the Unix release path.",
    );
}
...
const [
    ...
    extensionBundle,
    piProviderBundle,
    openCodeProviderBundle,
    worker,
] = resolveAgentSmokeArtifacts(process.argv.slice(2));
...
try {
    await Promise.all([
        ...
        mkdir(resolve(devshellHome, "control", "instances"), {
            recursive: true,
        }),
    ...
    run(
        process.execPath,
        [cli.absolutePath, "extension", "install", extensionBundle],
        environment,
    );
    const bundledProviders = JSON.parse(
        run(
            process.execPath,
    ...
    if (bundledPi?.enabled !== true || bundledPi?.state !== "ready") {
        throw new Error(
            `Agent Extension did not ensure-install bundled Pi: ${JSON.stringify(bundledPi)}`,
        );
    }
    const initialDefault = JSON.parse(
        run(
            process.execPath,
    ...
    if (initialDefault?.provider !== "pi") {
        throw new Error(
            `bundled Pi was not selected as the initial default provider: ${JSON.stringify(initialDefault)}`,
        );
    }
    const piCommand = resolve(home, ".local", "bin", "pi");
    const piCliVersion = run(
        piCommand,
        ["--version"],
        environment,
    ).stdout.trim();
    if (!/^\d+\.\d+\.\d+(?:[-+].*)?$/u.test(piCliVersion)) {
        throw new Error(
            `Agent Extension pi command returned an invalid version: ${JSON.stringify(piCliVersion)}`,
        );
    }
    try {
        await lstat(resolve(devshellHome, "pi", "workspaces"));
        throw new Error(
            "direct pi created a DevShell-private workspace instead of preserving its invoking cwd",
        );
    } catch (error) {
    ...
    run(
        process.execPath,
        [cli.absolutePath, "agent", "provider", "update", piProviderBundle],
        environment,
    );
    run(
        process.execPath,
    ...
    if (piProvider?.enabled !== true || piProvider?.state !== "ready") {
        throw new Error(
            `installed Pi provider is not ready: ${JSON.stringify(piProvider)}`,
        );
    }
    const openCodeProvider = Array.isArray(providers)
        ...
        : undefined;
    if (
        openCodeProvider?.enabled !== true ||
        openCodeProvider?.state !== "ready"
    ) {
        throw new Error(
            `installed OpenCode provider is not ready: ${JSON.stringify(openCodeProvider)}`,
        );
    }
    const selectedDefault = JSON.parse(
        run(
            process.execPath,
            [cli.absolutePath, "agent", "provider", "default"],
            environment,
    ...
    if (selectedDefault?.provider !== "pi") {
        throw new Error(
            `installing another provider unexpectedly changed the default: ${JSON.stringify(selectedDefault)}`,
        );
    }
    ...
    if (
        ...
        !/^\d+\.\d+\.\d+(?:[-+].*)?$/u.test(piVersion)
    ) {
        throw new Error(
            `packaged Pi provider returned an invalid version: ${JSON.stringify(piVersion)}`,
        );
    }
    smokePiAgentLifecycle(cli.absolutePath, environment);
    const openCodeVersion = await smokeOpenCodeProvider(
        openCodeProvider,
        environment,
    );
    ...
    process.stdout.write(
        `Agent package smoke passed (pi CLI ${piCliVersion}, Pi provider ${piVersion}, OpenCode ${openCodeVersion})\n`,
    );
} finally {
    if (controlStarted) {
        run(
...
function smokePiAgentLifecycle(cli, env) {
    ...
    const started = JSON.parse(
        run(process.execPath, [cli, "agent", target], env).stdout,
    );
    if (
        started?.provider !== "pi" ||
        ...
        throw new Error(
            `Pi Agent did not start through the packaged provider: ${JSON.stringify(started)}`,
        );
    }

    const listed = JSON.parse(
        run(process.execPath, [cli, "agent", "list"], env).stdout,
    );
    if (
        !Array.isArray(listed) ||
        !listed.some(
            (agent) =>
                agent?.agentId === started.agentId && agent?.provider === "pi",
        )
    ) {
        throw new Error(
            `started Pi Agent is missing from list: ${JSON.stringify(listed)}`,
        );
    }
    ...
    if (
        Array.isArray(afterStop) &&
        afterStop.some((agent) => agent?.agentId === started.agentId)
    ) {
        throw new Error(
            `stopped Pi Agent is still listed: ${JSON.stringify(afterStop)}`,
        );
    }
}
...
async function smokeOpenCodeProvider(provider, env) {
    ...
    if (typeof generation !== "string" || generation.length === 0) {
        throw new Error(
            `installed OpenCode provider has no selected generation: ${JSON.stringify(provider)}`,
        );
    }
    ...
    if (!installation.command.startsWith(`${providerDirectory}/`)) {
        throw new Error(
            `packaged OpenCode provider escaped its private bundle: ${installation.command}`,
        );
    }
    const version = run(installation.command, ["--version"], env).stdout.trim();
    if (version !== installerModule.OPENCODE_RUNTIME_VERSION) {
        throw new Error(
            `packaged OpenCode runtime returned ${JSON.stringify(version)}`,
        );
    }
...
async function smokeOpenCodeChild(providerDirectory, command, env) {
    const child = fork(
        resolve(
            providerDirectory,
            "dist/provider/opencode/OpenCodeAgentChild.js",
        ),
        ["10000"],
        {
            cwd: root,
    ...
    try {
        ...
        const ready = await nextChildMessage(
            child,
            (message) =>
                message?.type === "ready" && message.id === "smoke-init",
            stderr,
        );
        if (ready.ok !== true)
            throw new Error(
                `packaged OpenCode ACP init failed: ${JSON.stringify(ready)}\n${stderr}`,
            );
        child.send({ command: "stop", id: "smoke-stop", type: "command" });
        const stopped = await nextChildMessage(
            child,
            (message) =>
                message?.type === "result" && message.id === "smoke-stop",
            stderr,
        );
        if (stopped.ok !== true)
            throw new Error(
                `packaged OpenCode ACP stop failed: ${JSON.stringify(stopped)}\n${stderr}`,
            );
        child.disconnect();
...
function nextChildMessage(child, predicate, stderr) {
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            ...
            reject(
                new Error(
                    `timed out waiting for packaged OpenCode child\n${stderr}`,
                ),
            );
        }, 30_000);
        const cleanup = () => {
        ...
        const onExit = (code, signal) => {
            ...
            reject(
                new Error(
                    `packaged OpenCode child exited early (${code ?? signal ?? "unknown"})\n${stderr}`,
                ),
            );
        };
...
function waitForChildExit(child) {
    ...
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            ...
            reject(
                new Error(
                    "packaged OpenCode child did not exit after disconnect",
                ),
            );
        }, 5_000);
        child.once("exit", () => {
