...
try {
    ...
    assertOutput(
        before,
        /status: stopped/u,
        "instance was not initially stopped",
    );

    const instanceStarted = runCli(
        ["instance", "start", "aromatic-pc"],
        fixture.env,
    );
    ...
    const pwd = runCli(
        [
            ...
            fixture.workspace,
            "bash_run",
            JSON.stringify({ command: "pwd", timeoutMs: 30_000 }),
        ],
        fixture.env,
    );
    process.stdout.write(pwd.stdout);
    assert.equal(
        pwd.stdout.includes(fixture.workspace),
        true,
        "pwd output omitted the workspace",
    );
    ...
    const echo = runCli(
        [
            ...
            "bash_run",
            JSON.stringify({
                command: "echo portable-devshell",
                timeoutMs: 30_000,
            }),
        ],
        fixture.env,
    ...
    assertOutput(
        logs,
        /portable-devshell/u,
        "instance logs omitted tool output",
    );
    ...
    assert.equal(
        rows.some(
            (row) =>
                row.collection === "toolCalls" &&
                row.payload.includes('"toolName":"bash_run"'),
        ),
        true,
    );
    assert.equal(
        rows.some(
            (row) =>
                row.collection === "events" &&
                row.payload.includes('"type":"toolCall.completed"'),
        ),
        true,
    );
    assert.equal(
        rows.some(
            (row) =>
                row.collection === "logs" &&
                row.payload.includes("portable-devshell"),
        ),
        true,
    );
